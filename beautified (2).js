...
(function() {
    ...
    L.chatBootstrapVersion = "1.4.0-universal.1";
    L.universalBundle = true;
    if (!L.__menuQueue) L.__menuQueue = [];
    if (!L.__subMenuQueue) L.__subMenuQueue = [];
    L.registerMenu = L.registerMenu || function(key, cb) {
        L.__menuQueue.push({
            key: key,
            cb: cb
        })
    };
    L.registerSubMenu = L.registerSubMenu || function(key, cb) {
        L.__subMenuQueue.push({
            key: key,
            cb: cb
        })
    };
...
class ToastifyInjector {
            ...
            if (rootNode.getElementsByClassName("chasm-toastify-track").length != rootNode.childNodes.length) {
                for (const element of Array.from(rootNode.getElementsByClassName("chasm-toastify-track"))) {
                    if (element.hasAttribute("completed")) {
                        ...
                        element.removeAt = current + 1e3
                    }
                }
            }
        }
        for (const element of rootNode.getElementsByClassName("chasm-toastify-track")) {
    ...
    doToastifyAlert(message, expires = 3000) {
        if (!document.getElementById("chasm-pulse-style")) {
            const style = document.createElement("style");
            style.id = "chasm-pulse-style";
            style.textContent = `         @keyframes chasm-toast-pulse {             0% { opacity: 0.3; transform: scale(0.9); }             50% { opacity: 1; transform: scale(1.1); }             100% { opacity: 0.3; transform: scale(0.9); }         }`;
            document.head.appendChild(style);
        } /*         [점멸등 조건 규칙 설정]        인젝션 완료, 오류, 쿨타임 대기중, 공간 부족, 후보 없음 등 인젝터 계열은 무점멸로 처리하고,        에리 검수 및 자동 추출/정리 상태일 때만 은은하게 노란 알림등이 깜빡이도록 완벽하게 걸러냅니다.     */
        const isInjector = message.includes("인젝트") || message.includes("쿨타임") || message.includes("공간 부족") || message.includes("후보 없음") || message.includes("오류");
        const isWaitingState = !isInjector && (message.includes("대기중") || message.includes("검수") || message.includes("분석") || message.includes("기억") || message.includes("재정렬") || message.includes("묻는") || message.includes("추출") || message.includes("갱신") || message.includes("정리") || message.includes("가져오는") || message.includes("탐색") || message.includes("수정"));
        const toastId = isInjector ? "chasm-injector-toast" : "chasm-refiner-toast";
        ...
        wrapperNode.className = "chasm-private-toast-wrapper";
        wrapperNode.style.cssText = `         position: fixed;         left: 50%;         transform: translate(-50%, -50%) scale(0.9);         opacity: 0;         transition: transform 0.2s cubic-bezier(0.175, 0.885, 0.32, 1.2), opacity 0.15s ease, top 0.2s ease;         z-index: 999999999 !important;         pointer-events: none;         display: flex;         justify-content: center;         align-items: center;         width: fit-content;         height: fit-content;     `;
        const containerNode = document.createElement("div"); /* 요청하신 다크 세련 테마 색상 대입: 배경 #23343A, 테두리 #88B9C8 */
        containerNode.style.cssText = `         background-color: #23343A;         border: 1.5px solid #88B9C8;         padding: 9px 14px;         border-radius: 9999px;         display: inline-flex;         justify-content: center;         align-items: center;         box-shadow: 0 4px 16px rgba(0,0,0,0.3);         box-sizing: border-box;     `; /* 대기 진행형 상태일 때만 텍스트 왼쪽에 깜빡이는 밝은 노란색(#F9F871) 동그라미 추가 */
        if (isWaitingState) {
            const dotNode = document.createElement("div");
            dotNode.style.cssText = `             width: 7px;             height: 7px;             border-radius: 50%;             background-color: #F9F871;             margin-right: 7px;             flex-shrink: 0;             animation: chasm-toast-pulse 1.2s infinite ease-in-out;             border: 0.5px solid #88B9C8;         `;
            containerNode.append(dotNode);
        }
        const textNode = document.createElement("p");
        textNode.textContent = message; /* 요청하신 아주 눈에 띄고 이쁜 파스텔 화이트 #EFFCFF 글자색 대입 */
        textNode.style.cssText = `         color: #EFFCFF;         text-align: center;         font-size: 12px;         font-weight: 400;         line-height: 100%;         margin: 0;         padding: 0;         white-space: nowrap;         user-select: none;     `;
        containerNode.append(textNode);
        wrapperNode.append(containerNode);
        document.body.appendChild(wrapperNode);
        const adjustPositions = () => {
            const inj = document.getElementById("chasm-injector-toast");
            const ref = document.getElementById("chasm-refiner-toast");
            if (inj && ref) {
                inj.style.top = "calc(50% - 22px)";
                ref.style.top = "calc(50% + 22px)";
            } else {
                if (inj) inj.style.top = "50%";
                if (ref) ref.style.top = "50%";
            }
        };
        adjustPositions();
        setTimeout(() => {
...
class CrackStoryStatus {
    ...
    static of (container) {
        return new CrackStoryStatus(CrackGenre.of(container.genre), container.imageCount, Visibility.of(container.visibility), container.status, container.categories, container.target?.name, container.chatType?.name, new CrackMappedImage(container.profileImage), new Date(container.createdAt), new Date(container.updatedAt), container.isAdult, container.isConvertedToAdult)
    }
}
...
class CrackStoryInfo {
    ...
    static of (container) {
        return new CrackStoryInfo(container._id, container.initialMessages, container.defaultCrackerModel, container.chatModelId, UserInfo.of(container.creator), container.name, CrackStoryDescription.of(container), CrackArticleStatistics.of(container), CrackStoryStatus.of(container), CrackPromptTemplate.of(container.promptTemplate), CrackComment.of(container.representativeComment), Array.from(container.startingSets).map(item => CrackStoryStartingSet.of(item)), container.shareUrl)
    }
}
...
class CrackChatRoom {
    ...
    static of (container) {
        return new CrackChatRoom(container._id, container.userId, container.title, container.lastMessage, container.model, container.chatModelId, container.hasUserNote, new Date(container.createdAt), new Date(container.updatedAt), container.isSummaryUpdated, container.isAutoRecommendUserNextMessage, container.chatProfile?._id, container.story ? CrackStorySessionInfo.of(container.story) : null, container.character ? CrackCharacterSessionInfo.of(container.character) : null)
    }
}
class CrackPersona {
    id;
...
class _CrackChatRoomApi {
    ...
    async extractLogs(chatId, {
        maxCount: maxCount = -1,
        delay: delay = 20,
        naturalOrder: naturalOrder = true
    } = {}) {
        const logs = [];
    ...
    async fetchLastMessage(chatId) {
        try {
            const next = (await this.iterateLogs(chatId, {
                maxCount: 1
            }).next()).value;
            if (next) return next
        } catch (error) {
    ...
    async changeChatModel(chatId, modelId) {
        const result = await this.#network.authFetch("PATCH", `https://crack-api.wrtn.ai/crack-gen/v3/chats/${chatId}`, {
            chatModelId: modelId
        });
        if (result instanceof Error) {
            return result
    ...
    async send(chatId, message, {
        ...
        onMessageSent: onMessageSent = undefined,
        timeout: timeout = 1e5
    } = {}) {
        try {
    ...
    async #emitMessage(currentSocket, timeout, chatId, message, lastMessage, socketCloser, onMessageSent) {
        return await new Promise(async (resolve, reject) => {
            ...
            try {
                currentSocket.emit("send", {
                    chatId: chatId,
                    message: message,
                    prevMessageId: lastMessage.id
                }, async sendResponse => {
                    if (sendResponse.result === "success") {
                        currentSocket.once("characterMessageGenerated", async response => {
                            socketCloser();
                            if (!isResolved) {
                                isResolved = true;
                                clearTimeout(taskId)
    ...
    async sendUserMessage(chatId, message, {
        socket: socket = undefined,
        timeout: timeout = 1e5,
        onMessageSent: onMessageSent = undefined
    } = {}) {
    ...
    async sendBotMessage(chatId, message, {
        socket: socket = undefined,
        timeout: timeout = 1e5,
        onMessageSent: onMessageSent = undefined
    } = {}) {
...
class _SocketIoUtil {
    ...
    async enable() {
        ...
        const {
            io: io
        } = await import("https://cdn.socket.io/4.8.1/socket.io.esm.min.js");
        this.#socketIo = io
    }
    async prepareIo() {
...
function isAttributeEquals(element, elementToCompare, attributeName, compareUndefined = true) {
    if (!compareUndefined && (!element.hasAttribute(attributeName) || !elementToCompare.hasAttribute(attributeName))) {
        return false
    }
    return element.getAttribute(attributeName) === elementToCompare.getAttribute(attributeName)
}
class GenericUtil {
    ...
    static assert(item) {
        if (item === undefined || item === null) {
            throw new Error("ASSERTION FAILED; null or undefined object detected at non-null logic")
        }
        return item
    }
    static isValid(item) {
    ...
    withLicenseCredential(menuName) {
        this.createMenu(menuName ?? "프레임워크에 대하여", modal => {
            modal.replaceContentPanel(panel => {
                panel.addTitleText(DECENTRAL_VERSION).addText("decentralized.js는 IGX 팀에서 제작되었습니다.").addText("이 프레임워크는 임베딩 가능한 탈중앙화된 모달 프레임워크입니다.").addTitleText("SVG 아이콘 디자인").addText("decentralized.js의 모든 아이콘은 SVGRepo에서 가져왔습니다.").addText("- 설정 아이콘 (https://www.svgrepo.com/svg/458353/setting-line)").addText("- 닫기 아이콘 (https://www.svgrepo.com/svg/494725/close)").addText("- 메뉴 아이콘 (https://www.svgrepo.com/svg/522418/menu)").addText("- 스위치 소스 코드 참조 (https://www.daleseo.com/css-toggle-switch/)");
...
class DecentrallizedModal {
    ...
    triggerSelect(preSelected) {
        if (preSelected) {
            if (preSelected.length === 1) {
                ...
                this.#menuItems.get(preSelected[0])?.__subMenus?.get(preSelected[1])?.onDisplay(this)
            } else {
                this.#selectedMenu.length = 0;
    ...
    init(isDarkTheme) {
        ...
        this.__container = setupClassNode("div", "decentral-modal-container decentral-color-container", node => {
        ...
        verticalPanel.appendChild((this.__contentPanel = new ContentPanel(`decentral-content-${this.baseId}`, "여기에 텍스트 입력", DECENTRAL_DEFAULT_ICON_SVG, () => {
    ...
    replaceContentPanel(lambda, title, iconSvg = undefined) {
        const element = assert(document.getElementById(`decentral-content-${this.baseId}`));
        this.__contentPanel = new ContentPanel(`decentral-content-${this.baseId}`, title, iconSvg ?? DECENTRAL_DEFAULT_ICON_SVG, () => {
            this.__mobileMenuPanel?.open()
        }, () => {
            this.close()
        });
        ...
        element.id = "";
        assert(element.parentElement).insertBefore(this.__contentPanel.asHTML(), element);
        element.remove()
    }
    refreshMenuPanel() {
        ...
        assert(this.__menuPanel?.currentMenu()?.parentElement).insertBefore(mainMenu.asHTML(), assert(this.__menuPanel?.currentMenu()));
        this.__menuPanel?.currentMenu()?.remove();
        assert(this.__mobileMenuPanel?.currentMenu()?.parentElement).append(mobileMenu.asHTML(), assert(this.__mobileMenuPanel?.currentMenu()));
...
class BaseMenuPanel extends HTMLComponentConvertable {
    ...
    runSelected() {
        ...
        if (this.selectedMenu.length === 2) {
            this.menus.get(this.selectedMenu[0])?.__subMenus?.get(this.selectedMenu[1])?.onDisplay(this.modal)
        }
    }
...
class MenuPanel extends BaseMenuPanel {
    ...
    createMenuContainer(itemName, menuItem, setActiveSelected) {
        const menuContainer = setupClassNode("div", "decentral-menu-element-container");
        const menuText = setupClassNode("span", "decentral-menu-element", node => {
    ...
    createSubMenuContainer(itemName, menuItem, menuContainer, menuText, setActiveSelected) {
        const subMenuContainer = setupClassNode("div", "decentral-sub-menu-container");
        for (const [subItemName, subItem] of menuItem.__subMenus) {
            const subMenuNode = setupClassNode("span", "decentral-sub-menu-element", node => {
            ...
            if (!setActiveSelected(false) && this.selectedMenu.length === 2 && this.selectedMenu[0] === itemName && this.selectedMenu[1] === subItemName) {
...
class MobileMenuPanel extends BaseMenuPanel {
    ...
    asHTML() {
        const container = this.#menu = setupClassNode("div", "decentral-mobile-menu-container");
        let isElementActiveSelected = false;
        for (const [itemName, menuItem] of this.menus) {
            const menuContainer = this.createMenuContainer(itemName, menuItem, isActive => {
                isElementActiveSelected = isActive;
    ...
    createMenuContainer(itemName, menuItem, setActiveSelected) {
        const menuContainer = setupClassNode("div", "decentral-mobile-menu-element-container");
        const menuText = setupClassNode("span", "decentral-mobile-menu-element", node => {
            node.textContent = itemName;
    ...
    createSubMenuContainer(itemName, menuItem, menuContainer, menuText, setActiveSelected) {
        const subMenuContainer = setupClassNode("div", "decentral-mobile-sub-menu-container");
        for (const [subItemName, subItem] of menuItem.__subMenus) {
            const subMenuNode = setupClassNode("span", "decentral-mobile-sub-menu-element", node => {
                node.textContent = subItemName;
                const expectedSubmenu = [itemName, subItemName];
...
class ComponentAppender extends HTMLComponentConvertable {
    ...
    addGrid(titleText, isLongField, lambda) {
        this.parentElement.appendChild(createGridElement(titleText, isLongField, lambda));
        return this
    }
    addTitleText(text, {
        initializer: initializer = undefined,
        __proxy: __proxy = this
    } = {
        initializer: undefined,
        __proxy: this
    }) {
        __proxy.parentElement.append(createLongFlatGridElement(null, node => {
            node.append(setupClassNode("p", "decentral-text-title", textNode => {
    ...
    addText(text, {
        ...
        this.parentElement.append(createLongFlatGridElement(null, node => {
            node.append(setupClassNode("p", "decentral-text-plain", textNode => {
                textNode.innerText = text;
                if (initializer) {
                    initializer(textNode)
                }
    ...
    constructInputGrid(id, titleText, isLongField, {
        defaultValue: defaultValue = undefined,
        onInit: onInit = undefined,
        onChange: onChange = undefined
    } = {}) {
        let inputNode = setupClassNode("input", "decentral-text-field decentral-modifiable-component", inputField => {
            inputField.id = id;
        ...
        this.parentElement.append(createGridElement(titleText, isLongField, (node, title, suffix) => {
            node.append(inputNode);
            onInit?.(inputNode, title, suffix)
        }));
        return inputNode
    }
    addInputGrid(id, titleText, isLongField, {
        defaultValue: defaultValue = undefined,
        onInit: onInit = undefined,
        onChange: onChange = undefined
    } = {}) {
    ...
    constructTextAreaGrid(id, titleText, {
        defaultValue: defaultValue = undefined,
        onInit: onInit = undefined,
        onChange: onChange = undefined
    } = {}) {
        let textNode = setupClassNode("textarea", "decentral-text-area decentral-modifiable-component", area => {
            area.id = id;
        ...
        this.parentElement.append(createGridElement(titleText, true, (node, title, suffix) => {
            node.append(textNode);
            onInit?.(textNode, title, suffix)
        }));
        return textNode
    }
    constructBoxedTextAreaGrid(id, titleText, description, {
        defaultValue: defaultValue = undefined,
        onInit: onInit = undefined,
        onChange: onChange = undefined
    } = {}) {
    ...
    constructLoggingArea(id, titleText, {
        defaultValue: defaultValue = undefined,
        onInit: onInit = undefined
    } = {}) {
        let textNode = setupClassNode("textarea", "decentral-logging-area", area => {
            area.id = id;
    ...
    addBoxedButton(id, titleText, description, {
        onInit: onInit = undefined,
        onTrigger: onTrigger = undefined
    } = {}) {
        const buttonNode = setupClassNode("button", "decentral-button", button => {
            button.id = id;
    ...
    constructButton(id, titleText, short, {
        onInit: onInit = undefined,
        onTrigger: onTrigger = undefined
    } = {}) {
        ...
        this.parentElement.append(createGridElement(null, short, node => {
            node.append(buttonNode)
        }));
        return buttonNode
    }
    addShortButton(id, titleText, {
        onInit: onInit = undefined,
        onTrigger: onTrigger = undefined
    } = {}) {
    ...
    constructBoxedField(title, description, {
        onInit: onInit = undefined
    } = {}) {
        let topNode;
        this.parentElement.append(topNode = createLongSemiFlatGridElement(null, node => {
            node.append(setupClassNode("div", "decentral-boxed-field", area => {
                area.append(setupClassNode("div", "element-text-container", field => {
                    field.append(setupClassNode("p", "element-title", text => {
                        text.innerText = title
                    }));
                    field.append(setupClassNode("p", "element-description", text => {
                        text.innerText = description
                    }))
                }));
                area.append(setupClassNode("div", "element-input-container", onInit))
            }))
        }));
        return topNode
    }
    addBoxedField(title, description, {
        onInit: onInit = undefined
    } = {}) {
        this.parentElement.append(createLongSemiFlatGridElement(null, node => {
            node.append(setupClassNode("div", "decentral-boxed-field", area => {
                area.append(setupClassNode("div", "element-text-container", field => {
                    field.append(setupClassNode("p", "element-title", text => {
                        text.innerText = title
                ...
                area.append(setupClassNode("div", "element-input-container", container => {
                    onInit?.(container)
                }))
            }))
        }));
    ...
    addLongBox(padded, {
        ...
        this.parentElement.append(createLongSemiFlatGridElement(null, node => {
            node.append(setupClassNode("div", padded ? "decentral-padded-boxed-field" : "decentral-boxed-field", onInit))
        }));
        return this
    }
    constructLongBoxedField(title, description, {
        onInit: onInit = undefined
    } = {}) {
        let topNode = createLongSemiFlatGridElement(null, node => {
            node.append(setupClassNode("div", "decentral-boxed-field", area => {
                area.append(setupClassNode("div", "element-text-container-full", field => {
                    field.append(setupClassNode("p", "element-title", text => {
                        text.innerText = title
                    }));
                    field.append(setupClassNode("p", "element-description", text => {
                    ...
                    field.append(setupClassNode("div", "element-input-container-long", onInit))
                }))
            }))
        });
        this.parentElement.append(topNode);
    ...
    constructSwitchBox(id, title, description, {
        defaultValue: defaultValue = false,
        onInit: onInit = undefined,
        onChange: onChange = undefined
    } = {}) {
        let inputNode = setupClassNode("input", "element-switch decentral-modifiable-component", switcher => {
            switcher.id = id;
        ...
        this.addBoxedField(title, description, {
            onInit: node => {
                node.append(inputNode = setupClassNode("div", "element-input-container", container => {
    ...
    addSwitchBox(id, title, description, {
        defaultValue: defaultValue = false,
        onInit: onInit = undefined,
        onChange: onChange = undefined
    } = {}) {
    ...
    __addNumberBox(id, title, description, type, {
        ...
        let inputNode = setupClassNode("div", "element-input-container", container => {
            container.append(setupClassNode("input", (type === 0 ? "element-small-input" : type === 1 ? "element-medium-input" : "element-large-input") + " decentral-modifiable-component", inputField => {
                inputField.id = id;
    ...
    constructShortNumberBox(id, title, description, {
        defaultValue: defaultValue = 0,
        min: min = 0,
        max: max = 1e3,
        onInit: onInit = undefined,
    ...
    addShortNumberBox(id, title, description, {
        defaultValue: defaultValue = 0,
        min: min = 0,
        max: max = 1e3,
        onInit: onInit = undefined,
    ...
    constructMediumNumberBox(id, title, description, {
        defaultValue: defaultValue = 0,
        min: min = 0,
        max: max = 1e3,
        onInit: onInit = undefined,
    ...
    addMediumNumberBox(id, title, description, {
        defaultValue: defaultValue = 0,
        min: min = 0,
        max: max = 1e3,
        onInit: onInit = undefined,
    ...
    constructNumberBox(id, title, description, {
        defaultValue: defaultValue = 0,
        min: min = 0,
        max: max = 1e3,
        onInit: onInit = undefined,
    ...
    addNumberBox(id, title, description, {
        defaultValue: defaultValue = 0,
        min: min = 0,
        max: max = 1e3,
        onInit: onInit = undefined,
    ...
    constructBoxedInputGrid(id, title, description, {
        defaultValue: defaultValue = undefined,
        onInit: onInit = undefined,
        onChange: onChange = undefined
    } = {}) {
        let inputNode = setupClassNode("input", "decentral-text-field", inputField => {
            inputField.id = id;
            inputField.setAttribute("type", "text");
    ...
    addBoxedInputGrid(id, title, description, {
        defaultValue: defaultValue = undefined,
        onInit: onInit = undefined,
        onChange: onChange = undefined
    } = {}) {
    ...
    constructSelectBox(titleText, initialText, initialId, isLong) {
        ...
        const title = setupFullNode("div", "decentral-option", "width: 100%; display: flex; flex-direction: row; align-items: center;", option => {
            option.append(setupStyleNode("span", "height: fit-content;", textNode => {
                textNode.textContent = initialText
            }));
            option.append(setupStyleNode("div", "margin-left: auto;", iconNode => {
            ...
            option.onclick = event => {
                ...
                if (topNode.hasAttribute("list-enabled")) {
                    ...
                    this.parentElement?.parentElement?.parentElement?.parentElement?.append(this.createOuterClickDetection(() => {
                        topNode.removeAttribute("list-enabled");
        ...
        return {
            ...
            listGroup: () => Array.from(topNode.getElementsByClassName("decentral-option")).map(it => it.getAttribute("decentral-selected")).filter(it => it != undefined),
            setSelected: target => {
            ...
            getSelected: () => topNode.getAttribute("decentral-selected"),
            findSelected: () => {
                for (const element of topNode.getElementsByClassName("decentral-option")) {
                    if (element.getAttribute("decentral-option-id") === topNode.getAttribute("decentral-selected")) {
                        return element
            ...
            addGroup: (text, lambda) => {
                const node = setupClassNode("div", "decentral-option-group", group => {
...
class ContentPanel extends ComponentAppender {
    __verticalContainer = setupClassNode("div", "decentral-vertical-container", () => {});
    ...
    constructor(id, title, svg, mobileOpenAction, closeAction) {
        ...
        const icon = setupClassNode("div", "decentral-modal-title-icon", node => {});
        const gridWrapper = setupClassNode("div", "decentral-grid-container", () => {});
        this.__verticalContainer.append(setupClassNode("div", "decentral-modal-title-container", node => {
            node.append(icon);
            node.append(setupClassNode("p", "decental-modal-title-text", node => {
                node.innerText = title
            }));
            node.append(setupClassNode("div", "decentral-modal-button-container", node => {
                node.append(setupClassNode("div", "decentral-mobile-menu-button", svgNode => {
                    svgNode.innerHTML = DECENTRAL_MENU_ICON_SVG;
                    svgNode.id = `${id}-menu`;
                    mobileOpenAction && (svgNode.onclick = mobileOpenAction)
                }));
                node.append(setupClassNode("div", "decentral-close-button", svgNode => {
...
function createGridElement(titleText, isLongField, lambda) {
    return setupClassNode("div", isLongField ? "decentral-grid-element-long" : "decentral-grid-element", node => {
        if (titleText) {
            const suffix = setupClassNode("div", "decentral-element-title-suffix");
            const title = setupClassNode("div", "decentral-element-title", elementTitle => {
                elementTitle.append(setupNode("p", node => {
...
function createLongSemiFlatGridElement(titleText, lambda) {
    ...
    function showStatusBadge(text) {
        ...
        if (mountTarget && _statusBadge.parentNode !== mountTarget) {
            try {
                mountTarget.appendChild(_statusBadge)
            } catch (_) {}
        }
        ...
        if (_statusAutoHideTimer) {
            clearTimeout(_statusAutoHideTimer);
            _statusAutoHideTimer = null
        }
        ...
        if (_statusDot && !_statusDot.__pulseTimer) {
            ...
            _statusDot.__pulseTimer = setInterval(function() {
                __on = !__on;
                try {
                    _statusDot.style.opacity = __on ? "1" : "0.35"
                } catch (_) {}
            }, 500)
        }
    }
    ...
    function hideStatusBadge() {
        ...
        if (_statusDot && _statusDot.__pulseTimer) {
            try {
                clearInterval(_statusDot.__pulseTimer)
            } catch (_) {}
            _statusDot.__pulseTimer = null;
            try {
                _statusDot.style.opacity = "1"
            } catch (_) {}
        }
    }
    ...
    function createApiInput(config, prefix, nd, onChange) {
        ...
        btnFirebase.onclick = () => {
            config[apiTypeKey] = "firebase";
            updateBtns();
            triggerSave()
        };
        typeRow.appendChild(btnKey);
        typeRow.appendChild(btnVertex);
        typeRow.appendChild(btnFirebase);
        ...
        locInput.onchange = () => {
            config[locKey] = locInput.value || "global";
            triggerSave()
        };
        const projInput = document.createElement("input");
        projInput.value = config[projKey] || "";
        projInput.placeholder = "Project ID";
        projInput.style.cssText = S;
        projInput.onchange = () => {
            config[projKey] = projInput.value;
            triggerSave()
        };
        const ld = document.createElement("div");
        ld.style.flex = "1";
        ld.appendChild(locInput);
        const pd = document.createElement("div");
        pd.style.flex = "1";
        pd.appendChild(projInput);
        locRow.appendChild(ld);
        locRow.appendChild(pd);
        ...
        const fbEmbInput = document.createElement("input");
        fbEmbInput.type = "text";
        fbEmbInput.value = config[fbEmbKey] || "";
        fbEmbInput.placeholder = "AIzaSy...";
        fbEmbInput.setAttribute("autocomplete", "off");
        fbEmbInput.style.cssText = S + "-webkit-text-security:disc;";
        fbEmbInput.onchange = () => {
            config[fbEmbKey] = fbEmbInput.value.trim();
            triggerSave()
        };
...
(function() {
    ...
    const TIMELINE_COMPRESSION_LEVELS = {
        full: "full",
        compact: "compact",
        micro: "micro"
    };
    ...
    const DEFAULTS = {
        ...
        temporalWeight: .18,
        activeEntityWeight: .25,
        relationshipGraphWeight: .22,
        unresolvedWeight: .28,
        maintenanceWeight: .12,
    ...
    function parseServiceAccountJson(jsonStr) {
        try {
            ...
            if (!obj.client_email || !obj.private_key) return {
                ok: false,
                error: "client_email 또는 private_key 누락"
            };
            return {
                ok: true,
    ...
    async function getVertexAccessToken(sa, cacheKey = "default") {
        if (!_tokenCaches[cacheKey]) _tokenCaches[cacheKey] = {
            token: null,
            expiry: 0
        };
        ...
        const cryptoKey = await crypto.subtle.importKey("pkcs8", pemToArrayBuffer(sa.privateKey), {
    ...
    function loadFirebaseSdk() {
        ...
        _fbSdkPromise = new Promise((resolve, reject) => {
            ...
            _w.addEventListener("crack-ext-fbsdk-ready", () => {
                clearTimeout(to);
                resolve(_w.__crackExtFirebaseSdk)
            }, {
                once: true
            });
    ...
    async function warmupFirebase(firebaseScript, model = "gemini-3-flash-preview") {
        try {
            ...
            if (!_fbAiCache[aiKey]) _fbAiCache[aiKey] = sdk.getAI(_fbAppCache[appName], {
                backend: new sdk.VertexAIBackend(fb_loc)
            });
            return true
        } catch (e) {
            return false
    ...
    async function callGeminiApi(prompt, opts = {}) {
        const {
            ...
            firebaseLocation: firebaseLocation = "global",
        ...
        if (isFirebase) {
            ...
            if (!cfg || !cfg.apiKey || !cfg.projectId) {
                const liveScript = _w.__LoreInj && _w.__LoreInj.settings && _w.__LoreInj.settings.config ? _w.__LoreInj.settings.config.autoExtFirebaseScript : "";
            ...
            if (!cfg || !cfg.apiKey || !cfg.projectId) {
                ...
                return {
                    text: null,
                    status: 0,
                    error: "Firebase 스크립트 형식 오류 (firebaseConfig = {...} 형태 붙여넣기 필요; len=" + len + ", head=" + head + ")",
                    retries: 0
                }
            }
            ...
            try {
                ...
                if (!app) {
                    app = sdk.initializeApp(cfg, appName);
                    _fbAppCache[appName] = app
                }
                const aiKey = appName + "|" + fb_loc;
                ...
                const fbSafety = [{
                    category: sdk.HarmCategory.HARM_CATEGORY_HATE_SPEECH,
                    threshold: sdk.HarmBlockThreshold.OFF
                }, {
                    category: sdk.HarmCategory.HARM_CATEGORY_SEXUALLY_EXPLICIT,
                    threshold: sdk.HarmBlockThreshold.OFF
                }, {
                    category: sdk.HarmCategory.HARM_CATEGORY_HARASSMENT,
                    threshold: sdk.HarmBlockThreshold.OFF
                }, {
                    category: sdk.HarmCategory.HARM_CATEGORY_DANGEROUS_CONTENT,
                    threshold: sdk.HarmBlockThreshold.OFF
                }];
                const modelKey = aiKey + "|" + model + "|" + simpleHash(JSON.stringify(fbGenConfig));
                let gm = _fbModelCache[modelKey];
                if (!gm) {
                    gm = sdk.getGenerativeModel(ai, {
                        model: model,
                        safetySettings: fbSafety,
                        generationConfig: fbGenConfig
                    });
                    _fbModelCache[modelKey] = gm
                }
                ...
                return {
                    text: null,
                    status: 0,
                    error: "Firebase: " + (fbErr.message || String(fbErr)),
                    retries: 0
                }
            }
            ...
            if (!fbKey) return {
                text: null,
                status: 0,
                error: "Firebase Web API Key 누락",
                retries: 0
            };
            if (!firebaseProjectId) return {
                text: null,
                status: 0,
                error: "Firebase projectId 누락",
                retries: 0
            };
        ...
        for (let attempt = 0; attempt <= maxRetries; attempt++) {
            try {
                ...
                if (!r.ok) {
                    ...
                    if (r.status === 429 && attempt < maxRetries) {
                        const waitMs = Math.pow(2, attempt + 1) * 1e3 + Math.random() * 500;
            ...
            if (attempt < maxRetries) {
                const waitMs = Math.min(8e3, 1e3 * Math.pow(2, attempt)) + Math.random() * 500;
                await new Promise(res => setTimeout(res, waitMs))
            }
        }
        return {
    ...
    async function embedTexts(texts, opts = {}) {
        const {
            ...
            vertexLocation: vertexLocation = "global",
            vertexProjectId: vertexProjectId = "",
            firebaseEmbedKey: firebaseEmbedKey = "",
            firebaseKey: firebaseKey = "",
            firebaseProjectId: firebaseProjectId = "",
            firebaseLocation: firebaseLocation = "global",
            model: model = DEFAULTS.embeddingModel,
            dimensions: dimensions = DEFAULTS.embeddingDimensions,
            taskType: taskType = DEFAULTS.embeddingTaskType,
            cacheKey: cacheKey = "embed",
            costContext: costContext = null
        } = opts;
        ...
        if (isFirebase) {
            ...
            return embedTexts(arr, {
                ...opts,
                apiType: "key",
                key: firebaseEmbedKey,
    ...
    function saveSettings(key, config) {
        try {
            _ls.setItem(key, JSON.stringify(config))
        } catch (e) {}
    }
    ...
    Object.assign(ns, {
        ...
        LOCAL_MIGRATION_VERSION: LOCAL_MIGRATION_VERSION,
        TIMELINE_EVENT_TYPE: TIMELINE_EVENT_TYPE,
        TIMELINE_SCHEMA_VERSION: TIMELINE_SCHEMA_VERSION,
        TIMELINE_COMPRESSION_LEVELS: TIMELINE_COMPRESSION_LEVELS,
        SAFETY: SAFETY,
        PLATFORM: PLATFORM,
        DEFAULTS: DEFAULTS,
        getDB: getDB,
        gmFetch: gmFetch,
        parseServiceAccountJson: parseServiceAccountJson,
        getVertexAccessToken: getVertexAccessToken,
        callGeminiApi: callGeminiApi,
        embedText: embedText,
        embedTexts: embedTexts,
        warmupFirebase: warmupFirebase,
...
(function() {
    ...
    if (!C || !C.__kernelLoaded) {
        console.error("[LoreCore:platform] kernel 미로드");
        return
    }
    if (C.__platformLoaded) return;

    function crackUtil() {
        return _w.CrackUtil || (typeof CrackUtil !== "undefined" ? CrackUtil : null)
    }

    function getCurUrl() {
        return _w.location.pathname
    }
    ...
    async function fetchLogs(count) {
        ...
        try {
            const CU = crackUtil();
            if (!CU) return [];
            const items = await CU.chatRoom().extractLogs(chatId, {
                maxCount: count
            });
    ...
    Object.assign(C, {
        ...
        fetchLogs: fetchLogs,
        fetchAllMemories: fetchAllMemories,
        fetchPersonaName: fetchPersonaName,
...
(function() {
    ...
    const C = _w.__LoreCore;
    if (!C || !C.__kernelLoaded) {
        console.error("[LoreCore:memory] kernel 미로드");
        return
    }
    ...
    function calcReinjectionScore(turnsSinceLastMention, entryType, config, entry) {
        ...
        if (turnsSinceLastMention <= aiMem) {
            ...
            needsReinjection = 1 - Math.exp(-overLimit * .5)
        }
    ...
    function detectActiveCharacters(recentMsgs, allEntries) {
        ...
        const characters = allEntries.filter(e => e.type === "identity" || e.type === "character").map(e => {
            ...
            return {
                entry: e,
                names: names.map(n => (n || "").toLowerCase()).filter(Boolean)
            }
        });
        const active = [];
            ...
            for (const n of c.names) {
                ...
                if (isCJK) {
                    ...
                    try {
                        ...
                        if (new RegExp(`(^|[^a-z0-9])${esc}([^a-z0-9]|$)`, "i").test(pool)) {
                            hit = true;
                            break
                        }
                    } catch {
    ...
    async function checkFirstEncounter(char1, char2) {
        const db = getDB();
        let enc = await db.encounters.where({
            char1: char1,
            char2: char2
        }).first();
        if (!enc) enc = await db.encounters.where({
            char1: char2,
            char2: char1
        }).first();
        return enc || null
    ...
    async function findReunionPairs(activeNames, currentTurn, minGap) {
        ...
        for (let i = 0; i < activeNames.length; i++) {
            for (let j = i + 1; j < activeNames.length; j++) {
                ...
                if (diff >= gap) out.push({
                    pair: [activeNames[i], activeNames[j]],
                    gap: diff
                })
            }
        }
    ...
    function extractSceneKeywords(recentMsgs) {
        ...
        function _wordHit(w) {
            ...
            try {
                return new RegExp("(^|[^a-z0-9])" + w.replace(/[.*+?^${}()|[\]\\]/g, "\\$&") + "([^a-z0-9]|$)", "i").test(_last2Low)
            } catch (e) {
                return _last2Low.includes(w.toLowerCase())
            }
        }
    ...
    function _arr(x) {
        return Array.isArray(x) ? x.filter(Boolean).map(v => String(v).trim()).filter(Boolean) : []
    }

    function _uniq(a) {
        return Array.from(new Set(_arr(a)))
    }
    ...
    function stableTimelineEventId(entry) {
        ...
        const seed = [entry.packName || "", entry.title || entry.name || "", _arr(entry.participants).join("|"), entry.location || "", when.anchor || "", when.turnStart || entry.eventTurn || entry.timeline?.eventTurn || "", when.turnEnd || ""].join("::");
    ...
    function normalizeTimelineEvent(entry, ctx = {}) {
        ...
        e.when = e.when && typeof e.when === "object" ? {
            ...e.when
        } : {};
        ...
        e.when.confidence = e.when.confidence != null ? e.when.confidence : .7;
    ...
    function inferEntryEntities(entry) {
        ...
        const out = [].concat(_arr(entry.entities)).concat(isTimelineEvent(entry) ? _arr(entry.participants).concat(_arr(entry.linkedLore), _arr(entry.location ? [entry.location] : [])) : []).concat(_arr(entry.parties)).concat(_arr(d.parties)).concat(_arr(entry.subjects)).concat(_arr(entry.objects)).concat(_arr(entry.locations));
    ...
    function graphOverlapScore(entry, activeNames) {
        ...
        for (const a of active)
            if (ents.some(e => e === a || e.includes(a) || a.includes(e))) hits++;
    ...
    function relationshipGraphScore(entry, activeNames) {
        ...
        let score = 0;
        const t = entry.type;
        if (t === "rel" || t === "relationship") score += .45;
        if (entry.callState || entry.call || entry.callHistory) score += .25;
        score += graphOverlapScore(entry, activeNames) * .35;
        const rels = Array.isArray(entry.relations) ? entry.relations.length : 0;
        if (rels) score += Math.min(.2, rels * .05);
    ...
    function unresolvedPriorityScore(entry) {
        ...
        let score = 0;
        if (entry.type === "promise" || entry.type === "prom") score += .55;
        if (/pending|open|unresolved|미해결|보류|진행|약속/.test(status)) score += .35;
        if (entry.cond || d.condition) score += .2;
        if (Array.isArray(entry.eventHistory) && entry.eventHistory.length) score += .1;
        return Math.min(1, score)
    }
    ...
    function temporalRecencyScore(entry, currentTurn) {
        ...
        return Math.min(1, recency + unresolved * .35)
    }
    ...
    function temporalRecallCueScore(text) {
        ...
        if (!raw) return 0;
        const cues = ["그때", "전에", "기억", "지난번", "처음", "이후", "그 후", "그날", "예전", "아까", "방금", "지난", "앞서", "이전", "나중", "다시", "재회", "약속", "remember", "before", "after", "last time", "first time", "back then", "previously"];
        let score = 0;
        for (const cue of cues) {
            ...
            if (hit) score += cue.length >= 4 ? .22 : .16
        }
        return Math.min(1, score)
    }
    ...
    function temporalTokenize(text) {
        const raw = String(text || "").toLowerCase();
        const parts = raw.split(/[\s,.!?;:"'()[\]{}<>\/\\|]+/).map(x => x.trim()).filter(x => x.length >= 2);
    ...
    function temporalTermOverlap(entry, pool) {
        ...
        const direct = Math.min(1, matched.length * .18);
        ...
        const fuzzy = Math.min(.45, tokenHits * .06);
        return {
            score: Math.min(1, direct + fuzzy),
            matched: _uniq(matched).slice(0, 6)
        }
    }
    ...
    function temporalAnchorMatchScore(entry, pool) {
        ...
        for (const a of anchors) {
            ...
            if (low.length >= 2 && lowPool.includes(low)) score = Math.max(score, .7);
            const toks = temporalTokenize(low);
            if (toks.some(t => lowPool.includes(t))) score = Math.max(score, .35)
        }
        return Math.min(1, score)
    }
    ...
    function resolveTemporalRecall(userInput, recentMsgs, entries, opts = {}) {
        ...
        for (const e of entries || []) {
            ...
            let score = cue * .22 + overlap.score * .38 + anchor * .22 + entity * .1 + recency * .05 + unresolved * .08;
            if (!cue && overlap.score < .22 && anchor < .35) score *= .35;
        ...
        for (const e of entries) {
            ...
            if (nicknames && typeof nicknames === "object") {
                for (const [key, value] of Object.entries(nicknames)) {
                    ...
                    setCallState(matrix, states, pair.from, pair.to, {
                        currentTerm: value
                    }, value)
                }
            }
    ...
    function formatHonorificMatrix(result, budget) {
        const src = result && typeof result === "object" && result.matrix ? result : {
            matrix: result || {},
            prevMap: {},
            states: {}
        };
        ...
        for (const [from, targets] of Object.entries(matrix)) {
            for (const [to, hon] of Object.entries(targets)) {
                const k = `${from}→${to}`;
                const state = states && states[k] ? states[k] : normalizeCallState({
                    currentTerm: hon,
                    previousTerms: prevMap && prevMap[k] ? [prevMap[k]] : []
                }, hon);
    ...
    async function saveEntryVersion(entry, reason) {
        ...
        try {
            ...
            console.warn("[LoreCore:memory] saveEntryVersion 실패:", e.message)
        }
    }
    ...
    function isAnchor(entry) {
        return !!(entry && entry.anchor === true)
    }
    Object.assign(C, {
        calcForgottenScore: calcForgottenScore,
        calcReinjectionScore: calcReinjectionScore,
        getHalfLife: getHalfLife,
        isAnchor: isAnchor,
        detectActiveCharacters: detectActiveCharacters,
        isRelatedToActive: isRelatedToActive,
        checkFirstEncounter: checkFirstEncounter,
        recordFirstEncounter: recordFirstEncounter,
        findUnmetPairs: findUnmetPairs,
        findReunionPairs: findReunionPairs,
        getWorkingMemory: getWorkingMemory,
        updateWorkingMemory: updateWorkingMemory,
        extractSceneKeywords: extractSceneKeywords,
        formatSceneTag: formatSceneTag,
        inferEntryEntities: inferEntryEntities,
        normalizeTemporalGraph: normalizeTemporalGraph,
        temporalGap: temporalGap,
        graphOverlapScore: graphOverlapScore,
        isTimelineEvent: isTimelineEvent,
        stableTimelineEventId: stableTimelineEventId,
        normalizeTimelineEvent: normalizeTimelineEvent,
        relationshipGraphScore: relationshipGraphScore,
        unresolvedPriorityScore: unresolvedPriorityScore,
        maintenanceRecallScore: maintenanceRecallScore,
        temporalRecencyScore: temporalRecencyScore,
        buildTemporalHint: buildTemporalHint,
        buildRelationDeltaHint: buildRelationDeltaHint,
        formatTemporalHints: formatTemporalHints,
        temporalRecallCueScore: temporalRecallCueScore,
        resolveTemporalRecall: resolveTemporalRecall,
        buildHonorificMatrix: buildHonorificMatrix,
        formatHonorificMatrix: formatHonorificMatrix,
        saveEntryVersion: saveEntryVersion,
...
(function() {
    ...
    const C = _w.__LoreCore;
    if (!C || !C.__kernelLoaded) {
        console.error("[LoreCore:format] kernel 미로드");
        return
    }
    ...
    function cfFull(e) {
        ...
        if (fullSummary) {
            const sum = charLen(fullSummary) > 90 ? [...fullSummary].slice(0, 87).join("") + "..." : fullSummary;
            line += " " + sum
        }
        if (d.nicknames && typeof d.nicknames === "object") {
            const pairs = Object.entries(d.nicknames);
            if (pairs.length > 0) {
                const nk = pairs.map(([k, v]) => {
                    const m = k.match(/^(.+?)→(.+?)$/);
                    return m ? m[1] + "->" + m[2] + ":" + v : k + ":" + v
                }).join(", ");
                line += " Call:" + nk
            }
        }
    ...
    function cfCompact(e) {
        ...
        if (compactSummary) {
            const sum = charLen(compactSummary) > 45 ? [...compactSummary].slice(0, 42).join("") + "..." : compactSummary;
            line += " " + sum
        }
        if (d.nicknames && typeof d.nicknames === "object") {
            const pairs = Object.entries(d.nicknames);
    ...
    function cfMicro(e) {
        ...
        const d = e.detail || {};
        const microSummary = summaryTier(e, "micro");
        const val = d.current_status || d.status || (microSummary ? [...microSummary].slice(0, 25).join("") : e.type);
        let hon = "";
        if (d.nicknames && typeof d.nicknames === "object") {
            const vals = Object.values(d.nicknames);
            if (vals.length > 0) hon = "/" + vals[0]
        }
    ...
    function buildTemporalRecallBlock(items, opts = {}) {
        const sourceRows = (items || []).map(x => x && x.entry ? x.entry : x).filter(e => e && (!C.isTimelineEvent || C.isTimelineEvent(e)));
        ...
        const levelOrder = preferredLevel === "full" ? ["full", "compact", "micro"] : preferredLevel === "compact" ? ["compact", "micro"] : ["micro"];
        const importanceOf = (e, idx) => {
            ...
            return imp * 3 + emo * 2 + hook * 4 + selected * 12 - idx * .1
        };
        const selectedIds = new Set(Array.isArray(decision.eventIds) ? decision.eventIds.map(String) : []);
        const rows = sourceRows.map((e, idx) => ({
            e: e,
            idx: idx,
            id: String(e.eventId || e.id || e.name || ""),
            priority: importanceOf(e, idx)
        })).sort((a, b) => {
        ...
        const header = mode === "specific_event" ? "[시간축 회상 — 직접 언급된 과거 사건]" : mode === "unresolved_hook" ? "[시간축 회상 — 이어질 미해결 사건]" : "[시간축 회상]";
        const lines = [header];
        const included = [];
            ...
            if (picked) {
                ...
                if (pickedLevel !== preferredLevel) {
                    compressionActions.push({
                        eventId: id,
                        action: pickedLevel === "micro-trimmed" ? "trim" : "downgrade",
                        from: preferredLevel,
                        to: pickedLevel
                    })
                }
    ...
    function buildLoreBudgetPlan(entries, budget, config = {}, opts = {}) {
        const activeNames = opts.activeNames || [];
        let ordered = entries.map((e, i) => ({
            e: e,
            i: i,
            p: entryPriority(e, i, activeNames)
        })).sort((a, b) => b.p - a.p);
        let bundledCount = 0;
        if (config.bundleByEntity !== false && ordered.length > 1) {
            ...
            for (const g of sortedGroups)
                for (const r of g.rows) bundled.push(r);
        ...
        for (const row of ordered) {
            ...
            if (picked) {
                lines.push(picked);
                included.push(row.e);
                used += charLen(picked) + 1;
                if (pickedLevel !== preferred) downgraded.push({
                    name: row.e.name,
                    from: preferred,
                    to: pickedLevel
                })
    ...
    function planInjectionBudget(opts) {
        const {
            userInput: userInput = "",
            maxInputChars: maxInputChars = 2e3,
            entries: entries = [],
            activeNames: activeNames = [],
            unmetPairs: unmetPairs = [],
            sceneTag: sceneTag = "",
            firstEncounterBlock: firstEncounterBlock = "",
            reunionTags: reunionTags = "",
            honorifics: honorifics = "",
            temporalHints: temporalHints = "",
            temporalRecallBlock: temporalRecallBlock = "",
            config: config = {},
        ...
        const result = {
            ...
            dropped: [],
            budgetPlan: {},
            finalChars: userChars,
            reason: ""
        };
        if (available < 20) {
            result.reason = "no_space_after_user_and_wrapper";
            return result
        }
        const sections = [];
        ...
        let lore = buildLoreBudgetPlan(entries, loreBudget, config, {
            activeNames: activeNames,
            unmetPairs: unmetPairs
        });
        ...
        while (finalChars > maxInputChars && loreBudget > 0) {
            ...
            lore = buildLoreBudgetPlan(entries, loreBudget, config, {
                activeNames: activeNames,
                unmetPairs: unmetPairs
            });
        ...
        if (finalChars > maxInputChars) {
            lore = {
                ...
                level: "critical-only",
                downgraded: [],
                dropped: entries.map(e => ({
                    name: e.name,
                    reason: "critical_sections_only"
                })),
                budgetPlan: {
                    loreBudget: 0,
                    used: 0,
        ...
        if (finalChars > maxInputChars) {
            result.reason = "cancel_over_limit";
            return result
        }
        const sectionChars = {};
        ...
        return {
            ...
            budgetPlan: {
                ...
                availableAfterCritical: available,
    ...
    function adaptiveFormat(opts) {
        ...
        if (!rawEntries.length || budget < 30) return {
            text: "",
            included: [],
            usedChars: 0,
            level: "none",
        ...
        if (config.bundleByEntity !== false) {
            ...
            for (const k of order)
                for (const e of groups.get(k)) ordered.push(e);
            entries = ordered;
        ...
        if (honorifics && config.honorificMatrixEnabled !== false) {
            const hB = Math.min(Math.floor(remaining * .2), 80);
            let hT = honorifics;
            if (charLen(hT) > hB) hT = [...hT].slice(0, hB - 3).join("") + "...";
            parts.push(hT);
            remaining -= charLen(hT) + 1
        }
        if (config.firstEncounterWarning !== false && unmetPairs.length) {
            const fB = Math.floor(remaining * .1);
            let fUsed = 0;
            for (const [a, b] of unmetPairs) {
                const tag = "[Unmet:" + a + "&" + b + "]";
                const tl = charLen(tag);
                if (fUsed + tl > fB) break;
                parts.push(tag);
                fUsed += tl + 1
            }
            remaining -= fUsed
        ...
        if (cmpMode === "auto" || cmpMode === "full") {
            for (let i = 0; i < entries.length; i++) {
                const e = entries[i];
                const tag = i < entries.length * .25 || i === 0 ? "[!] " : "";
                const line = tag + cfFull(e);
                const len = charLen(line);
        ...
        if (included.length === 0 && entries.length > 0) {
            level = "compact";
            for (const e of entries) {
                let line = cfCompact(e);
                let len = charLen(line);
                if (loreUsed + len + 1 > remaining) {
                    line = cfMicro(e);
                    len = charLen(line);
                    level = "micro"
                }
    ...
    function budgetFormat(entries, budget, config) {
        ...
        const formatter = {
            full: formatEntryFull,
            compact: formatEntryCompact,
            micro: formatEntryMicro
        } [level];
        const lines = [];
        let used = 0;
        for (const e of entries) {
    ...
    function assembleInjection(opts) {
        ...
        if (honorifics && config.workingMemoryEnabled !== false) {
            const hBudget = Math.min(remaining * .3, config.honorificMatrixChars || DEFAULTS.honorificMatrixChars);
    ...
    Object.assign(C, {
        ...
        formatEntryAtLevel: formatEntryAtLevel,
        formatTimelineEventAtLevel: formatTimelineEventAtLevel,
        buildTemporalRecallBlock: buildTemporalRecallBlock,
        buildLoreBudgetPlan: buildLoreBudgetPlan,
        planInjectionBudget: planInjectionBudget,
        formatEntryFull: formatEntryFull,
        formatEntryCompact: formatEntryCompact,
        formatEntryMicro: formatEntryMicro,
        budgetFormat: budgetFormat,
        assembleInjection: assembleInjection,
        formatFirstEncounterBlock: formatFirstEncounterBlock,
        formatReunionTag: formatReunionTag,
        __formatLoaded: true
    });
...
(function() {
    ...
    const C = _w.__LoreCore;
    if (!C || !C.__kernelLoaded) {
        console.error("[LoreCore:search] kernel 미로드");
        return
    }
    if (!C.__memoryLoaded) {
        console.error("[LoreCore:search] memory 미로드");
        return
    }
        ...
        for (const e of entries || []) {
            ...
            for (const raw of triggers) {
                ...
                if (t.includes("&&")) {
                    ...
                    if (parts.length && parts.every(p => pool.includes(p))) {
                        hit = true;
                        matched = t;
                        score = 1;
                        break
                    ...
                    if (!similar) continue;
                    const pattern = t.slice(1).toLowerCase();
                    if (!pattern) continue;
                    if (pool.includes(pattern)) {
                        hit = true;
                        matched = t;
                        score = .9;
                        break
    ...
    async function hybridSearch(userInput, recentMsgs, entries, config, apiOpts) {
        ...
        const embWeight = embEnabled ? cfg.embeddingWeight != null ? cfg.embeddingWeight : .4 : 0;
        const trigWeight = 1 - embWeight;
        const threshold = cfg.embeddingThreshold != null ? cfg.embeddingThreshold : DEFAULTS.embeddingThreshold != null ? DEFAULTS.embeddingThreshold : .3;
        const turnCounter = cfg.turnCounter != null ? cfg.turnCounter : null;
        const activeBoost = cfg.activeCharBoost != null ? cfg.activeCharBoost : 1;
        const inactivePenalty = cfg.inactiveCharPenalty != null ? cfg.inactiveCharPenalty : 1;
        const decayEnabled = cfg.decayEnabled !== false;
        const temporalEnabled = cfg.temporalGraphEnabled !== false;
        const activeEntityWeight = cfg.activeEntityWeight != null ? cfg.activeEntityWeight : DEFAULTS.activeEntityWeight || .25;
        const relationshipGraphWeight = cfg.relationshipGraphWeight != null ? cfg.relationshipGraphWeight : DEFAULTS.relationshipGraphWeight || .22;
        const temporalWeight = cfg.temporalWeight != null ? cfg.temporalWeight : DEFAULTS.temporalWeight || .18;
        const unresolvedWeight = cfg.unresolvedWeight != null ? cfg.unresolvedWeight : DEFAULTS.unresolvedWeight || .28;
        const periodicRecallEnabled = cfg.periodicRecallEnabled !== false;
        const maintenanceWeight = periodicRecallEnabled ? cfg.maintenanceWeight != null ? cfg.maintenanceWeight : DEFAULTS.maintenanceWeight || .12 : 0;
        const timelineRetrievalEnabled = cfg.timelineRetrievalEnabled !== false;
        const timelineRecallWeight = cfg.timelineRecallWeight != null ? cfg.timelineRecallWeight : DEFAULTS.timelineRecallWeight || .32;
        const timelineNoCuePenalty = periodicRecallEnabled ? cfg.timelineNoCuePenalty != null ? cfg.timelineNoCuePenalty : DEFAULTS.timelineNoCuePenalty || .35 : 0;
        ...
        const trigMap = {};
        for (const h of trigHits) trigMap[h.entry.id] = {
            score: h.triggerScore,
            matched: h.matchedTrigger
        };
        const embMap = {};
        ...
        if (embEnabled && hasKey && typeof C.embedText === "function") {
            try {
                ...
                const qText = ((userInput || "") + " " + tail).slice(0, 2e3);
                ...
                const queryVec = await C.embedText(qText, Object.assign({}, apiOpts, {
                    taskType: qTaskType
                }));
                if (queryVec) {
                    ...
                    for (const eb of allEmbs) {
                        ...
                        if (eb.schemaVersion && eb.schemaVersion < 2) boost *= .95;
                ...
                console.warn("[LoreCore] 쿼리 임베딩 실패:", e && e.message)
            }
        }
        const activeNames = cfg.activeCharDetection !== false ? detectActiveCharacters(recentMsgs || [], enabled) || [] : [];
        const temporalRecallMap = {};
        let temporalRecallHasCue = false,
            temporalRecallCount = 0;
        ...
        const scored = enabled.map(e => {
            ...
            let lmt = e.lastMentionedTurn != null ? e.lastMentionedTurn : lastMentionMap && lastMentionMap[e.id] != null ? lastMentionMap[e.id] : null;
            ...
            if (decayEnabled && turnCounter != null && lmt != null) {
                ...
                score = score * (1 + reScore * .5)
            }
            if (decayEnabled && lmt == null && (tScore > 0 || eScore > 0)) {
                score = score * 1.15
            }
            if (e.rootId && e.isCurrentArc === false) score *= .3;
            ...
            if (temporalEnabled) {
                ...
                if (timelineRetrievalEnabled && C.isTimelineEvent && C.isTimelineEvent(e)) {
                    ...
                    if (tr) {
                        ...
                        if (!temporalRecallHasCue && tScore <= 0 && eScore <= 0 && components.timelineRecall < .45) recallBoost = 0;
                        score += recallBoost
                    }
                }
            }
            if (e.anchor === true && periodicRecallEnabled && score < .2) score = .2;
            if (e.anchor === true && temporalEnabled && periodicRecallEnabled) score += .4;
            return {
                entry: e,
                score: score,
                tScore: tScore,
        ...
        return {
            ...
            searchStats: {
                ...
                temporalRecallCount: temporalRecallCount,
                temporalRecallHasCue: temporalRecallHasCue
            }
        }
    }
    async function smartRerank(query, candidates, recentMsgs, apiOpts, config) {
        ...
        const blendWeight = cfg.rerankBlendWeight != null ? cfg.rerankBlendWeight : .5;
        const minLlmScore = cfg.rerankMinLlmScore != null ? cfg.rerankMinLlmScore : 2;
        const anchorBoost = cfg.rerankAnchorBoost != null ? cfg.rerankAnchorBoost : 1;
        const truncated = candidates.slice(0, maxCandidates);
        const recentText = typeof recentMsgs === "string" ? recentMsgs : Array.isArray(recentMsgs) ? recentMsgs.slice(-4).map(m => (m.role || "") + ": " + (m.message || "").slice(0, 100)).join("\n") : "";
        const summaryOf = e => {
            const s = e.summary;
        ...
        const prompt = promptTpl ? promptTpl.replace("{context}", recentText).replace("{query}", query || "").replace("{candidates}", listText) + '\n\nReturn either {"scores":[5,4,...]} or [{"i":1,"s":5},...] as JSON only.' : "명시적 답변 제외: 오직 JSON만 출력.\n장면과 직접 관련성 기준 5점 척도 (5=핵심, 1=무관).\n\n입력:\n" + query + "\n\n최근 대화:\n" + recentText + "\n\n후보:\n" + listText + '\n\n출력: {"scores":[5,4,...]}';
        try {
            const res = await callGeminiApi(prompt, Object.assign({}, apiOpts, {
                model: apiOpts && apiOpts.model || "gemini-3-flash-preview",
            ...
            const hMin = Math.min.apply(null, hScores),
                hMax = Math.max.apply(null, hScores);
            ...
            if (kept.length === 0) {
                console.warn("[LoreCore:rerank] 전체 후보가 무관 판정 → 원본 순서 우선");
                return candidates
            }
            kept.sort((a, b) => b.score - a.score);
            const minKept = kept[kept.length - 1].score;
            const tail = candidates.slice(maxCandidates).map(s => Object.assign({}, s, {
                origScore: s.score,
                score: Math.min(s.score, minKept * .99)
            }));
            return kept.concat(tail)
        } catch (e) {
            console.warn("[LoreCore] 리랭크 실패, 기본 순서 사용:", e && e.message);
            return candidates
        }
    }
    Object.assign(C, {
        bigramSimilarity: bigramSimilarity,
        buildScanPool: buildScanPool,
...
(function() {
    ...
    const C = _w.__LoreCore;
    if (!C || !C.__kernelLoaded) {
        console.error("[LoreCore:embedding] kernel 미로드");
        return
    }
    ...
    function buildEmbeddingText(entry, field = "summary") {
        ...
        const timeline = entry.timeline && typeof entry.timeline === "object" ? [entry.timeline.sceneLabel, entry.timeline.relativeOrder, entry.timeline.observedRecency, entry.timeline.eventTurn ? "t" + entry.timeline.eventTurn : ""].filter(Boolean).join(" ") : "";
        const events = Array.isArray(entry.eventHistory) ? entry.eventHistory.slice(-4).map(ev => ev && ev.summary).filter(Boolean).join(" ") : "";
        const hooks = [entry.state, detail.current_status, detail.status, entry.cond, detail.condition].filter(Boolean).join(" ");
        const aliases = [].concat(Array.isArray(entry.aliases) ? entry.aliases : []).concat(Array.isArray(detail.aliases) ? detail.aliases : []).concat(detail.nicknames && typeof detail.nicknames === "object" ? Object.values(detail.nicknames) : []);
        return [entry.name, entry.type, aliases.join(" "), (entry.triggers || []).join(" "), entry.embed_text || "", parties.join(" "), entities.join(" "), hooks, flattenCallState(entry), timeline, events, summaryText(entry, "full"), summaryText(entry, "compact"), summaryText(entry, "micro")].filter(Boolean).join(" ").replace(/\s+/g, " ").trim()
    }
    ...
    async function cleanupStaleEmbeddings(packName, apiOpts = {}) {
        ...
        for (const eb of all) {
            ...
            if (!entry) {
                del = true;
                missingEntry++
            } else if (eb.packName && eb.packName !== entry.packName) {
                del = true;
                wrongPack++
            } else {
                const field = eb.field || "summary";
                const expectedHash = embeddingSourceHash(entry, field);
                if ((eb.sourceHash || eb.hash) !== expectedHash) {
                    del = true;
                    staleHash++
                } else if (targetModel && eb.model && eb.model !== targetModel) {
                    del = true;
                    wrongModel++
                }
            }
    ...
    async function ensureEmbedding(entry, apiOpts) {
        ...
        const targetModel = apiOpts.model || DEFAULTS.embeddingModel;
        const existing = await db.embeddings.where({
            entryId: entry.id,
            field: field
        }).first();
        const taskTypeMismatch = docTaskType != null && existing && existing.taskType != null && existing.taskType !== docTaskType;
        const summaryFresh = existing && (existing.sourceHash || existing.hash) === hash && !(existing.model && existing.model !== targetModel) && !taskTypeMismatch && existing.schemaVersion === EMB_SCHEMA_VERSION && existing.packName === entry.packName;
        if (!summaryFresh) {
            ...
            const data = {
                ...embeddingMeta(entry, field, text, {
                    ...apiOpts,
                    model: targetModel
                }, docTaskType),
                vector: vec
        ...
        if ((entry.type === "promise" || entry.type === "prom") && condText) {
            ...
            const existingCond = await db.embeddings.where({
                entryId: entry.id,
                field: "condition"
            }).first();
            if (existingCond && (existingCond.sourceHash || existingCond.hash) === condHash) {
                ...
                const needsRegenCond = existingCond.model && existingCond.model !== targetModel || condTaskTypeMismatch || existingCond.schemaVersion !== EMB_SCHEMA_VERSION || existingCond.packName !== entry.packName;
                if (!needsRegenCond) return
            }
            const condVec = await embedText(condText, {
                ...apiOpts,
                taskType: docTaskType
            });
            const condData = {
                ...embeddingMeta(entry, "condition", condText, {
                    ...apiOpts,
                    model: targetModel
                }, docTaskType),
                vector: condVec
    ...
    async function embedPack(packName, apiOpts, onProgress) {
        ...
        const embMap = {};
        for (const eb of allEmbs) {
            if (eb.field === "summary") embMap[eb.entryId] = eb
        }
        const pending = entries.filter(e => {
            const text = buildEmbeddingText(e, "summary");
            const hash = simpleHash(text);
            const existing = embMap[e.id];
            return !existing || (existing.sourceHash || existing.hash) !== hash || existing.model !== targetModel || existing.schemaVersion !== EMB_SCHEMA_VERSION || existing.taskType != null && docTaskType != null && existing.taskType !== docTaskType || existing.packName !== e.packName
        });
        let done = 0;
        for (let i = 0; i < pending.length; i += 5) {
            ...
            try {
                ...
                for (let j = 0; j < batch.length; j++) {
                    ...
                    const data = {
                        ...embeddingMeta(batch[j], "summary", texts[j], {
                            ...apiOpts,
                            model: targetModel
                        }, docTaskType),
                        vector: vecs[j]
    ...
    Object.assign(C, {
        ...
        cleanupStaleEmbeddings: cleanupStaleEmbeddings,
        invalidateEntryEmbeddings: invalidateEntryEmbeddings,
        ensureEmbedding: ensureEmbedding,
        embedPack: embedPack,
        convertLegacyPack: convertLegacyPack,
...
(function() {
    ...
    const C = _w.__LoreCore;
    if (!C || !C.__kernelLoaded) {
        console.error("[LoreCore:pricing] kernel 미로드");
        return
    }
    ...
    function getPricing(model) {
        ...
        for (const k of keys)
            if (m.startsWith(k)) return PRICING[k];
        return null
    }
    ...
    function _persistCumul() {
        try {
            _w.localStorage.setItem(CUMUL_KEY, JSON.stringify(_cumulCache))
        } catch (_) {}
    }
    ...
    function recordApiCost(ev) {
        ...
        const bf = cumul.byFeature[rec.feature] = cumul.byFeature[rec.feature] || {
            usd: 0,
            count: 0
        };
        bf.usd += Number(rec.usd) || 0;
        bf.count++;
        cumul.byModel = cumul.byModel || {};
        const bm = cumul.byModel[rec.model] = cumul.byModel[rec.model] || {
            usd: 0,
            count: 0
        };
        bm.usd += Number(rec.usd) || 0;
        bm.count++;
        _persistCumul();
        return rec
    }
    ...
    function clearCumulativeCost() {
        _cumulCache = {};
        try {
            _w.localStorage.removeItem(CUMUL_KEY)
        } catch (_) {}
    }
    ...
    function clearCostEvents() {
        _cache = [];
        try {
            _w.localStorage.removeItem(STORAGE_KEY)
        } catch (_) {}
    }
    Object.assign(C, {
        PRICING: PRICING,
        computeCost: computeCost,
        recordApiCost: recordApiCost,
        getCostEvents: getCostEvents,
        clearCostEvents: clearCostEvents,
        normalizeModel: normalizeModel,
        getCumulativeCost: getCumulativeCost,
        clearCumulativeCost: clearCumulativeCost,
...
(function() {
    ...
    const C = _w.__LoreCore;
    if (!C || !C.__kernelLoaded) {
        console.error("[LoreCore:importer] kernel 미로드");
        return
    }
    ...
    function normalizeApiOpts(apiOpts = {}) {
        ...
        const liveCfg = _w.__LoreInj && _w.__LoreInj.settings && _w.__LoreInj.settings.config ? _w.__LoreInj.settings.config : {};
        ...
        if (!out.model) out.model = liveCfg.autoExtModel === "_custom" ? liveCfg.autoExtCustomModel || "gemini-3-flash-preview" : liveCfg.autoExtModel || "gemini-3-flash-preview";
    ...
    function normalizeCallState(entry, turn) {
        ...
        for (const d of deltas) {
            ...
            out[key] = {
                ...
                confidence: d.confidence != null ? d.confidence : .75,
    ...
    function normalizeLoreEntry(entry, opts = {}) {
        ...
        if (String(e.type || "").toLowerCase() === (C.TIMELINE_EVENT_TYPE || "timeline_event") && C.normalizeTimelineEvent) {
            e = C.normalizeTimelineEvent(e, {
                turn: turn,
                currentTurn: opts.currentTurn || turn
            })
        }
        ...
        if (!e.embed_text) {
            ...
            const temporalText = String(e.type || "").toLowerCase() === (C.TIMELINE_EVENT_TYPE || "timeline_event") ? [e.title, e.location, (e.actions || []).join(" "), (e.hooks || []).join(" "), (e.recallTriggers || []).join(" "), e.when?.anchor, (e.linkedLore || []).join(" ")].filter(Boolean).join(" ") : "";
    ...
    async function importFromText(text, packName, apiOpts, opts = {}) {
        ...
        if (allEntries.length > 0) {
            ...
            if (!pack) await db.packs.put({
                name: packName,
                entryCount: 0,
                project: ""
            });
            for (let e of allEntries) {
                e = normalizeLoreEntry(e, {
                    source: "imported"
                });
                e.packName = packName;
                e.project = "";
            ...
            await db.packs.update(packName, {
                entryCount: count
            })
        }
        ...
        C.__lastImportReport = {
            added: allEntries.length,
            chunks: chunks.length,
            ok: okCount,
            empty: emptyCount,
            failed: failedCount,
    ...
    async function fetchExternalText(url, opts = {}) {
        ...
        const timeoutMs = Number(opts.timeoutMs) > 0 ? Number(opts.timeoutMs) : 15e3;
        const onProgress = typeof opts.onProgress === "function" ? opts.onProgress : null;
        const emit = info => {
        ...
        try {
            const resp = await withTimeout(gmFetch(normalized, {
                method: "GET",
            ...
            errors.push("GM direct " + (e.message || String(e)))
        }
        emit({
            phase: "fetch:try",
            method: "fetch",
        ...
        try {
            ...
            const r = await withTimeout(fetchPromise, "fetch").catch(err => {
                if (ac) {
                    try {
                        ac.abort()
                    } catch (_) {}
        ...
        const fallbackUrls = ["https://api.allorigins.win/raw?url=" + encodeURIComponent(normalized), "https://api.codetabs.com/v1/proxy/?quest=" + encodeURIComponent(normalized), "https://corsproxy.io/?" + encodeURIComponent(normalized)];
        for (let i = 0; i < fallbackUrls.length; i++) {
            ...
            try {
                const resp = await withTimeout(gmFetch(fu, {
                    method: "GET",
                    headers: {
                ...
                errors.push("proxy#" + (i + 1) + " " + (e.message || String(e)))
            }
        }
        emit({
            phase: "fetch:fail",
    ...
    function detectDuplicatesInSummary(entries, memories) {
        ...
        for (const e of entries) {
            ...
            if (nameInSummary && keywordsInSummary.length >= 2) duplicates.push({
                entryId: e.id,
                name: e.name,
                confidence: .8
            });
            else if (nameInSummary) duplicates.push({
                entryId: e.id,
                name: e.name,
                confidence: .5
            })
    ...
    Object.assign(C, {
        ...
        detectDuplicatesInSummary: detectDuplicatesInSummary,
        normalizeLoreEntry: normalizeLoreEntry,
        normalizeSummaryValue: normalizeSummaryValue,
        mergeLoreSummary: mergeLoreSummary,
...
(function() {
    ...
    const TOPICS = {
        factual: {
            label: "1. 명시적 사실 모순",
            desc: "[Lore]·[Summaries]와 정면 충돌",
            group: "logic"
        },
        presence: {
            label: "2. 존재/부재 모순",
            desc: "없어야 할 캐릭터가 등장하거나 반대",
            group: "logic"
        },
        nickname: {
            label: "3. 호칭 불일치",
            desc: "저장된 호칭·직함과 다른 부름",
            group: "logic"
        },
        state: {
            label: "4. 상태 모순",
            desc: "사망/부상/실종 등 상태 모순",
            group: "logic"
        },
        promise: {
            label: "5. 약속 모순",
            desc: "확정된 약속이나 서사 흐름 위반",
            group: "logic"
        },
        reunion: {
            label: "6. 재회 위반",
            desc: "이미 아는 사이가 처음 보는 것처럼 행동",
            group: "logic"
        },
        honorific: {
            label: "7. 호칭 연속성",
            desc: "최신 안정 호칭 상태와 명시적 충돌만 검수",
            group: "logic"
        },
        impersonation: {
            label: "8. 유저 사칭",
            desc: "AI가 유저 캐릭터의 행동/생각을 서술",
            group: "logic"
        },
        truncSentence: {
            label: "A. 문장 끊김 복구",
...
(function() {
    ...
    function getMessageContainer(el) {
        if (!el) return null;
        return el.closest?.('[data-message-id], [data-id], [data-testid*="message"], [class*="message"], article, section, li') || el.closest?.("div") || el
    }
    ...
    function findMessageContainerById(messageId) {
        ...
        for (const el of candidates) {
            const vals = [el.getAttribute("data-message-id"), el.getAttribute("data-id"), el.getAttribute("id"), el.getAttribute("href"), el.getAttribute("data-testid")].filter(Boolean);
    ...
    function renderMarkdownHTML(mdText) {
        ...
        html = html.replace(/@@LRFENCE(\d+)@@(<br>)?/g, function(_m, idx) {
            return fences[+idx]
        });
        return html
    }
    ...
    function rememberAssistantMessage(messageId, text) {
        ...
        if (container && messageId) {
            try {
                container.setAttribute("data-lore-refiner-message-id", String(messageId))
            } catch (_) {}
        }
        return container
    }
    ...
    function tryStoreUpdate(rootEl, messageId, newText) {
        ...
        const isMsgRef = v => {
            ...
            try {
                return v._id === messageId || v.id === messageId || v.messageId === messageId || v.msgId === messageId
            } catch (_) {
                return false
            }
        };
            ...
            try {
                ...
                if (val instanceof Map) {
                    for (const v of val.values())
                        if (containsMsgDeep(v, depth + 1, seen)) return true;
                    return false
                }
        ...
        const cloneWithMsgReplaced = (val, depth, seen) => {
            ...
            if (isMsgRef(val)) {
                if (!replacementMsg) {
                    try {
                        replacementMsg = Object.assign({}, val, {
                            content: newText
                        })
                    } catch (_) {
                        replacementMsg = val
            ...
            if (targetEl) {
                ...
                if (rerenderHits === 0) {
                    ...
                    const isProviderType = t => {
                        try {
                            return t && typeof t === "object" && (t.$$typeof === Symbol.for("react.provider") || !!t._context)
                        } catch (_) {
                            return false
                        }
                    };
                        ...
                        while (stack.length && opBudget.count < opBudget.max) {
                            ...
                            try {
                                if (isProviderType(f.type) && f.memoizedProps && f.memoizedProps.value) {
                                    if (tryStoreLikeDispatch(f.memoizedProps.value)) {
                                        storeBHits++;
                                        rerenderHits++;
                                        updated = true
                                    }
                            ...
                            while (h && hd < 60) {
                                try {
                                    if (h.memoizedState && typeof h.memoizedState === "object") {
                                        if (tryStoreLikeDispatch(h.memoizedState)) {
                                            storeBHits++;
                                            rerenderHits++;
                                            updated = true
                                        }
    ...
    function runPath0Mutation(messageId, newText) {
        ...
        try {
            Object.defineProperty(summary, "_propsHitFibers", {
                value: propsHitFibers,
                enumerable: false
            })
        } catch (_) {}
        ...
        const mutate = (val, depth, localSeen, ctx) => {
            ...
            if (isArr) {
                for (let i = 0; i < val.length; i++) {
                    ...
                    try {
                        mutate(child, depth + 1, localSeen, ctx)
                    } catch (_) {
                        summary.errors++
                    }
                }
        ...
        for (const root of roots) {
            ...
            while (stack.length && visited < VISIT_CAP) {
                ...
                while (h && i < 60) {
                    ...
                    try {
                        mutate(h.memoizedState, 0, new WeakSet, null)
                    } catch (_) {
                        summary.errors++
                    }
                    i++;
                ...
                try {
                    mutate(f.memoizedProps, 0, new WeakSet, {
                        fiber: f,
                        slot: "memoizedProps"
                    });
                    summary.propsScanned++
                } catch (_) {
                    summary.errors++
                }
                try {
                    mutate(f.pendingProps, 0, new WeakSet, {
                        fiber: f,
                        slot: "pendingProps"
                    });
                    summary.propsScanned++
        ...
        try {
            ...
            try {
                ["mouseenter", "mouseover", "mousemove"].forEach(t => {
                    host.dispatchEvent(new MouseEvent(t, {
                        bubbles: true,
                        cancelable: true,
                        view: window
                    }))
            ...
            const labelOf = el => {
                if (!el) return "";
                const parts = [el.textContent, el.getAttribute && el.getAttribute("aria-label"), el.getAttribute && el.getAttribute("title"), el.getAttribute && el.getAttribute("data-testid")].filter(Boolean);
            ...
            const editBtn = findButton(host, /(수정|편집|edit)/i) || findButton(host.parentElement, /(수정|편집|edit)/i);
            ...
            setTimeout(() => {
                try {
                    const cancelBtn = findButton(host, /(취소|cancel|닫기|close)/i) || findButton(document, /(취소|cancel)/i);
                    if (cancelBtn) {
                        try {
                            cancelBtn.click();
                            return
...
(function() {
    ...
    const _w = typeof unsafeWindow !== "undefined" ? unsafeWindow : window;
    const R = _w.__LoreRefiner = _w.__LoreRefiner || {};
    if (!R.__promptsLoaded) {
        console.error("[Refiner:core] prompts 미로드");
        return
    }
    if (!R.__domLoaded) {
        console.error("[Refiner:core] dom 미로드");
        return
    }
    ...
    function _currentChatKey() {
        try {
            return Core && Core.getCurrentChatId && Core.getCurrentChatId() || "global"
        } catch (_) {
            return "global"
        }
    }
    ...
    function saveProcessedFingerprints(chatKeyOverride) {
        ...
        try {
            _ls.setItem(_PROCESSED_KEY_PREFIX + chatKey, JSON.stringify(Array.from(set)))
        } catch (_) {}
    }
    ...
    async function refineMessage(assistantText, force, enqueueCallback) {
        ...
        let targetLog = null;
        try {
            targetLog = await CrackUtil.chatRoom().findLastBotMessage(chatRoomId)
        } catch (_) {}
        ...
        if (!releaseRefineLock) {
            R.lastState = {
                state: "skipped",
                detail: "duplicate refiner call blocked",
                at: Date.now(),
                queue: R.refineQueue ? R.refineQueue.length : 0,
                busy: !!R.workerBusy
            };
            Core.hideStatusBadge();
            return
        ...
        try {
            ...
            function stripInjectedOOC(msg, role) {
                ...
                return msg.replace(/\*\*OOC:[\s\S]*?\*\*/g, "").replace(/\[System:[\s\S]*?\[\/System\]/g, "").replace(/\(Narrator's note:[\s\S]*?\(End note\)/g, "").replace(/\/\*\*[\s\S]*?\*\*\//g, "").replace(/Remember these established facts[\s\S]*?(?=\n\n|$)/g, "").trim()
            }
            ...
            const prompt = promptTpl.replace("{lore}", loreText).replace("{memory}", memoryText).replace("{context}", contextText).replace("{message}", assistantText).replace("{passWord}", passWord);
            const _refModel = (config.refinerModel === "_custom" ? config.refinerCustomModel : config.refinerModel) || (config.autoExtModel === "_custom" ? config.autoExtCustomModel : config.autoExtModel || "gemini-3-flash-preview");
            let _refElapsedMs = 0,
                _refCost = null;
            try {
                ...
                if (isPass) {
                    ...
                    setTimeout(Core.hideStatusBadge, 2e3);
                ...
                try {
                    ...
                    if (LogCallback) LogCallback(url, {
                        time: (new Date).toLocaleTimeString(),
                        original: assistantText,
                        result: "Parsing Error: " + text.slice(0, 50),
                        isError: true,
                        model: _refModel,
                        elapsedMs: _refElapsedMs,
                        cost: _refCost
                ...
                if (parsed && (parsed.replacements || parsed.refined_text)) {
                    ...
                    setTimeout(Core.hideStatusBadge, 3e3);
                    const applyRefinement = async newText => {
                        try {
                            ...
                            if (lastBot && !(lastBot instanceof Error)) {
                                ...
                                let editJson = null;
                                try {
                                    editText = editResult.text ? await editResult.text() : ""
                                } catch (_) {}
                                try {
                                    editJson = editText ? JSON.parse(editText) : null
                                } catch (_) {}
                                ...
                                if (serverOk) {
                                    ...
                                    let targetEl = null;
                                    try {
                                        targetEl = R.findMessageContainerById && R.findMessageContainerById(serverMessageId) || R.findDeepestMatchingElement && R.findDeepestMatchingElement(oldPlain) || null
                                    } catch (_) {}
                                    ...
                                    const verifyAndPatchVisible = async delayMs => {
                                        ...
                                        let visible = false;
                                        try {
                                            visible = R.waitForVisibleText ? await R.waitForVisibleText(serverText, serverMessageId, 900) : R.isTextVisible ? R.isTextVisible(serverText, serverMessageId) : storeOk
                                        } catch (_) {
                                            visible = !!storeOk
                                        }
                                        if (!visible && R.refreshMessageInDOM && !hasCodeFence) {
                                            ...
                                            try {
                                                fallbackResult = R.refreshMessageInDOM(originalForDom, serverText, serverMessageId)
                                            } catch (_) {}
                                            ...
                                            _w.__LR_LAST_DOM_FALLBACK = {
                                                skipped: true,
                                                status: "skipped_codeblock",
                                        ...
                                        if (!visible && R.nudgeMessageNativeRender) {
                                            let nudged = false;
                                            try {
                                                nudged = !!R.nudgeMessageNativeRender(serverMessageId)
                                            } catch (_) {}
                                            _w.__LR_LAST_NATIVE_NUDGE = nudged;
                                            if (nudged) {
                                                try {
                                                    visible = R.waitForVisibleText ? await R.waitForVisibleText(serverText, serverMessageId, 900) : R.isTextVisible ? R.isTextVisible(serverText, serverMessageId) : visible
                                                } catch (_) {}
                                            }
                                        }
                    ...
                    if (config.refinerAutoMode) {
                        ...
                        if (existingPopup) {
                            ...
                            holdRefineLock = true;
                            R.showRefineConfirm(parsed.reason, correctedText, newText => {
                                applyRefinement(newText).finally(releaseRefineLock)
                            }, () => {
                                releaseRefineLock();
                                if (enqueueCallback) setTimeout(enqueueCallback, 100)
                            })
    ...
    R.getProcessedFingerprints = function() {
        return _loadChat(_currentChatKey())
    };
...
(function() {
    ...
    const R = _w.__LoreRefiner = _w.__LoreRefiner || {};
    if (!R.__coreLoaded) {
        console.error("[Refiner:queue] core 미로드");
        return
    }
    ...
    R.workerBusy = false;
    R.workerStartTime = 0;
    const WORKER_TIMEOUT = 9e4;

    function enqueueRefine(text, msgId) {
        R.lastState = {
            state: "queued",
            detail: "queued for refine",
            at: Date.now(),
            queue: refineQueue.length + 1,
            busy: !!R.workerBusy
        };
    ...
    async function processQueue() {
        ...
        R.lastState = {
            state: "running",
            detail: "calling refiner api",
            at: Date.now(),
            queue: refineQueue.length,
            busy: true
        };
        ...
        try {
            await Promise.race([R.refineMessage(item.text, false, processQueue), new Promise((_, rej) => setTimeout(() => rej(new Error("refineMessage 60초 타임아웃")), 6e4))])
        } catch (e) {
            R.Core && R.Core.hideStatusBadge();
            R.lastState = {
                state: "error",
        ...
        R.workerBusy = false;
        if (!R.lastState || R.lastState.state === "running") R.lastState = {
            state: "idle",
            detail: "done",
            at: Date.now(),
            queue: refineQueue.length,
...
(function() {
    ...
    const R = _w.__LoreRefiner = _w.__LoreRefiner || {};
    if (!R.__queueLoaded) {
        console.error("[Refiner:observer] queue 미로드");
        try {
            _w.__LoreInj?.markFailed?.("refinerObserver", "queue missing")
        } catch (_) {}
        return
    }
    ...
    function isChatPath() {
        ...
        return /\/characters\/[a-f0-9]+\/chats\/[a-f0-9]+/.test(location.pathname) || /\/stories\/[a-f0-9]+\/episodes\/[a-f0-9]+/.test(location.pathname) || /\/u\/[a-f0-9]+\/c\/[a-f0-9]+/.test(location.pathname)
    }
    async function checkLatestMessage() {
        const config = R.ConfigGetter();
        if (!config.refinerEnabled) {
            R.Core && R.Core.hideStatusBadge();
            setRefinerState("off", "disabled");
            return
        }
        ...
        if (!chatId) {
            R.Core && R.Core.hideStatusBadge();
            setRefinerState("idle", "no chat id");
            return
        }
        try {
            ...
            if (_needsWarmup) {
                ...
                _needsWarmup = false;
                if (msgId) {
                    R.getProcessedFingerprints().add(msgId);
                    R.saveProcessedFingerprints()
                }
                return
            }
            if (msgId !== lastAssistantMsgId) {
                ...
                if (contentLen === lastMsgLength && lastMsgLength > 0) {
                    idleCount++;
                    if (idleCount >= 2 && Date.now() - lastChangeTime > 4e3) {
    ...
    function setupObserver() {
        ...
        _watchdogInterval = setInterval(() => {
            ...
            if (_waitingSince && Date.now() - _waitingSince > 45e3) {
                _waitingSince = 0;
                R.Core && R.Core.hideStatusBadge();
    ...
    R.setNeedsWarmup = function() {
        _needsWarmup = true
    };
    R.getRefinerState = function() {
        return R.lastState || {
            state: "idle",
            detail: "",
            at: 0,
...
(function() {
    ...
    function fail(dep, msg) {
        console.error(msg);
        try {
            _w.__LoreInj?.markFailed?.("refiner", dep + " missing")
        } catch (_) {}
    ...
    R.manualRefine = async function(text, msgId) {
        ...
        try {
            await R.refineMessage(text, true, R.processQueue)
        } catch (e) {
            console.error("[Refiner] manual fail:", e);
            throw e
        }
    };
    R.__loaded = true
...
(function() {
    ...
    function showInjectToast(orig, mod, isErr) {
        try {
            const ti = _w.document.__toastifyInjector || new ToastifyInjector();
            if (isErr) {
                ti.doToastifyAlert("🚨로어 인젝트 오류", 3000);
                return;
            }
            if (orig !== mod) {
                ti.doToastifyAlert("✨로어 인젝트 완료!", 3000);
            } else {
                const L = _w.__LoreInj;
                if (L && L.getChatKey && L.getInjLog) {
                    const logs = L.getInjLog(L.getChatKey());
                    if (logs && logs.length > 0) {
                        const lastReason = logs[0].reason;
                        if (lastReason === "cooldown_filtered_all" || lastReason === "delta_skipped" || lastReason === "cooldown") {
                            ti.doToastifyAlert("⏳인젝트 쿨타임 대기중", 3000);
                        }
                    }
                }
            }
                        ...
                        if (!arr[1].message.includes("OOC:")) {
                            ...
                            (async () => {
                                try {
                                    ...
                                    showInjectToast(orig, mod, false)
                                } catch (e) {
                                    console.error("[Lore] WS err:", e);
                                    _origWsSend.call(ws, data);
                                    showInjectToast(orig, orig, true)
                                }
                            })();
                            return
                        }
                    ...
                    if (bodyText) {
                        ...
                        if (body) {
                                ...
                                for (let i = body.messages.length - 1; i >= 0; i--) {
                                    if (body.messages[i].role === "user" && typeof body.messages[i].content === "string") {
                                        if (!body.messages[i].content.includes("OOC:")) {
                                            ...
                                            origForToast = original;
                                            try {
                                                body.messages[i].content = await _injectFn(original);
                                                modForToast = body.messages[i].content;
                                                if (original !== body.messages[i].content) injected = true;
                                                showInjectToast(origForToast, modForToast, false)
                                            } catch (e) {
                                                showInjectToast(original, original, true)
                                            }
                                        }
                                        break
                                    }
                                    ...
                                    if (body.variables[key] !== undefined && typeof body.variables[key] === "string") {
                                        if (!body.variables[key].includes("OOC:")) {
                                            ...
                                            try {
                                                ...
                                                modForToast = body.variables[key];
                            ...
                            if (injected) {
                                ...
                                if (isReq) args[0] = new Request(args[0], {
                                    body: newBodyText
                                });
                                else {
                                    args[1] = args[1] || {};
                                    args[1].body = newBodyText
                                }
                            }
                        }
                    }
...
(function() {
    ...
    const OOC_FORMATS = {
        ...
        instruction: {
            name: "직접 지시",
            prefix: "Remember these established facts and reflect them naturally:",
            suffix: "",
            desc: "명시적 지시형 모델"
        },
        custom: {
    ...
    Object.assign(_w.__LoreInj, {
        VER: VER,
        OOC_FORMAT_VERSION: OOC_FORMAT_VERSION,
        OOC_FORMATS: OOC_FORMATS,
        DEFAULT_AUTO_EXTRACT_PROMPT_WITHOUT_DB: DEFAULT_AUTO_EXTRACT_PROMPT_WITHOUT_DB,
        DEFAULT_AUTO_EXTRACT_PROMPT_WITH_DB: DEFAULT_AUTO_EXTRACT_PROMPT_WITH_DB,
        AUTO_EXTRACT_PROMPT_VERSION: AUTO_EXTRACT_PROMPT_VERSION,
        LEGACY_AUTO_EXTRACT_PROMPTS_WITH_DB: LEGACY_AUTO_EXTRACT_PROMPTS_WITH_DB,
        DEFAULT_AUTO_EXTRACT_SCHEMA: DEFAULT_AUTO_EXTRACT_SCHEMA,
        DEFAULT_AUTO_EXTRACT_PATCH_SCHEMA: DEFAULT_AUTO_EXTRACT_PATCH_SCHEMA,
        DEFAULT_TEMPORAL_EXTRACT_PROMPT: DEFAULT_TEMPORAL_EXTRACT_PROMPT,
        DEFAULT_TEMPORAL_EXTRACT_SCHEMA: DEFAULT_TEMPORAL_EXTRACT_SCHEMA,
...
(async function() {
    ...
    if (!(_w.__LoreInj && _w.__LoreInj.__constLoaded)) {
        console.error("[LoreInj:3] const 미로드");
        return
    }
    if (_w.__LoreInj.__settingsLoaded) return;
    if (_w.__loreInjectorVersion) {
        try {
            CrackToast.warning("로어 인젝터: " + _w.__loreInjectorVersion + " 실행중. 중복방지로 비활성화.")
        } catch (e) {}
        return
    }
    ...
    if (!C) {
        console.error("[Lore] Core 미감지. 작동 불가.");
        return
    }
    ...
    async function ensureHeavyRuntimeInit(reason = "") {
        ...
        _heavyRuntimeInitPromise = (async () => {
            ...
            if (R && !R.__loreInjectorRuntimeInitDone) {
                R.init(C, () => settings.config, (url, logItem) => {
                    ...
                    try {
                        const ti = document.__toastifyInjector || new ToastifyInjector();
                        ti.doToastifyAlert(msg, 3000)
                    } catch (e) {}
    ...
    function getApiConfigSnapshot(config) {
        const keys = ["autoExtApiType", "autoExtKey", "autoExtVertexJson", "autoExtVertexLocation", "autoExtVertexProjectId", "autoExtFirebaseScript", "autoExtFirebaseEmbedKey", "autoExtModel", "autoExtCustomModel", "autoExtReasoning", "autoExtBudget", "embeddingModel", "rerankModel", "refinerModel", "refinerCustomModel", "temporalRecallJudgeModel", "temporalRecallJudgeCustomModel", "temporalRecallJudgeReasoning"];
        const out = {};
        keys.forEach(k => {
    ...
    function parseJsonLoose(text) {
        ...
        if (firstBracket >= 0) {
            ...
            if (lastClose > firstBracket) {
                try {
                    return JSON.parse(t.slice(firstBracket, lastClose + 1))
                } catch {}
            }
        }
        return null
    ...
    async function restoreSnapshot(snapshotId) {
        ...
        await db.transaction("rw", db.packs, db.entries, async () => {
            ...
            await db.packs.update(snap.packName, {
                entryCount: snap.data.length
            })
        });
        try {
            _ls.removeItem("lore-local-migration-version");
            if (settings && settings.config) {
                settings.config.localMigrationVersion = "";
                settings.save()
            }
            await runLocalMigration()
        } catch (e) {
            console.warn("[LoreInj:3] restore 후 마이그레이션 재실행 실패:", e)
        }
        return true
    }
    ...
    function convertLegacyEventToTimeline(entry, opts = {}) {
        ...
        const _entSrc = Array.isArray(entry.entities) && entry.entities.length ? entry.entities : Array.isArray(entry.parties) ? entry.parties : entry.detail && Array.isArray(entry.detail.parties) ? entry.detail.parties : [];
        ...
        out.recallTriggers = Array.isArray(entry.recallTriggers) && entry.recallTriggers.length ? entry.recallTriggers : Array.from(new Set([...Array.isArray(entry.triggers) ? entry.triggers : [], ...Array.isArray(entry.hooks) ? entry.hooks : []].filter(Boolean)));
        ...
        if (!out.when || typeof out.when !== "object") {
            out.when = {
                anchor: entry.when && entry.when.anchor || "",
                timeline: entry.timeline && entry.timeline.sceneLabel || ""
            }
        }
        out.imp = _clamp10(entry.imp != null ? entry.imp : entry.importance, 6);
        out.emo = _clamp10(entry.emo != null ? entry.emo : entry.emotional, 5);
        const _surBase = entry.sur != null ? entry.sur : entry.surprise != null ? entry.surprise : entry.confidence != null ? Number(entry.confidence) * 10 : null;
        out.sur = _clamp10(_surBase, 6);
        out.gs = out.imp + out.emo + out.sur;
        if (C.stableTimelineEventId) {
            try {
                out.eventId = out.eventId || C.stableTimelineEventId(out)
            } catch (_) {}
        }
    ...
    const defaultSettings = {
        enabled: true,
        position: "before",
        prefix: OOC_FORMATS.default.prefix,
        suffix: OOC_FORMATS.default.suffix,
        scanRange: 5,
        scanOffset: 2,
        maxEntries: 3,
        ...
        autoExtApiType: "key",
        autoExtVertexJson: "",
        autoExtVertexLocation: "global",
        autoExtVertexProjectId: "",
        autoExtFirebaseScript: "",
        autoExtFirebaseEmbedKey: "",
        autoExtKey: "",
        autoExtModel: "gemini-3-flash-preview",
        autoExtCustomModel: "",
        autoExtReasoning: "medium",
        autoExtBudget: 2048,
        autoExtPrefix: "",
        autoExtSuffix: "",
        autoExtIncludeDb: true,
        autoExtIncludePersona: true,
        autoExtPatchMode: true,
        autoExtDbDigestLimit: 40,
        temporalExtractEnabled: true,
        temporalExtractMode: "after_general",
        temporalCriticEnabled: false,
        temporalMaxEventsPerPass: 5,
        temporalExtractPrompt: DEFAULT_TEMPORAL_EXTRACT_PROMPT,
        temporalExtractSchema: DEFAULT_TEMPORAL_EXTRACT_SCHEMA,
        timelineRetrievalEnabled: true,
        timelineRecallWeight: .32,
        timelineNoCuePenalty: .35,
        timelineRecallPoolLimit: 12,
        temporalInjectionEnabled: true,
        temporalRecallChars: 450,
        temporalRecallNaturalChars: 260,
        temporalRecallMaxEvents: 2,
        temporalRecallExplicitMaxEvents: 3,
        temporalRecallReserveChars: 120,
        temporalCompressionEnabled: true,
        temporalCompressionMinChars: 40,
        temporalCompressionApiEnabled: false,
        temporalCompressionTargetChars: 140,
        temporalCompressionPreserveFields: ["participants", "location", "hooks"],
        temporalRecallJudgeEnabled: false,
        temporalRecallJudgeModel: "gemini-3.1-flash-lite-preview",
        temporalRecallJudgeCustomModel: "",
        temporalRecallJudgeReasoning: "minimal",
        temporalRecallJudgeTimeoutMs: 8e3,
        ...
        oocFormat: "default",
        oocPromptVersion: OOC_FORMAT_VERSION,
        autoExtractPromptVersion: AUTO_EXTRACT_PROMPT_VERSION,
        rerankEnabled: false,
        rerankModel: "gemini-3-flash-preview",
        rerankPrompt: C.DEFAULTS.rerankPrompt,
        refinerEnabled: false,
        refinerAutoMode: false,
        refinerPassKeyword: "PASS",
        refinerModel: "gemini-3.1-flash-lite-preview",
        refinerCustomModel: "",
        refinerContextTurns: 3,
        refinerCustomPrompt: defaultRefinerPrompt(),
        refinerLoreMode: "semantic",
        refinerMatchTurns: 5,
        refinerUseDynamic: true,
        refinerTopics: defaultRefinerTopics(),
        refinerPromptVersion: R ? R.PROMPT_VERSION : "",
        urlRefinerLogs: {},
    ...
    const settings = {
        ...
        save: function() {
            try {
                ...
                _ls.setItem("lore-injector-v5", JSON.stringify(this.config))
            } catch (e) {}
        },
        load: function() {
            try {
    ...
    function normalizeCallStateLocal(entry) {
        ...
        const add = (key, raw, meta = {}) => {
            ...
            out[key] = {
                ...
                confidence: obj.confidence != null ? obj.confidence : .7,
        ...
        if (entry.detail?.nicknames && typeof entry.detail.nicknames === "object") {
            Object.entries(entry.detail.nicknames).forEach(([k, v]) => add(k, v, {
                reason: "migrated from detail.nicknames"
            }))
        }
        if (entry.call && typeof entry.call === "object") {
            Object.entries(entry.call).forEach(([k, v]) => add(k, v, {
                reason: "migrated from call"
            }))
        }
        if (Array.isArray(entry.callHistory)) {
            ...
            Object.entries(byKey).forEach(([k, arr]) => {
                ...
                add(k, {
                    ...
                    currentTerm: last.term,
                    previousTerms: previousTerms,
                    reason: last.reason || "migrated from callHistory"
                })
            })
        }
        return out
    ...
    function normalizeEntryForMigration(entry, currentTurn) {
        ...
        const inject = entry.inject && typeof entry.inject === "object" ? {
            ...entry.inject
        } : {};
        ...
        const entities = Array.isArray(entry.entities) && entry.entities.length ? entry.entities : C.inferEntryEntities ? C.inferEntryEntities(entry) : [];
        ...
        if (entry.type === _tlType) {
            ...
            if ((!_impVal || _impVal <= 0) && entry.importance != null) {
                patch.imp = _clamp10(entry.importance, 6);
                oldFormat = true
            } else if (!_impVal || _impVal <= 0) {
                patch.imp = 6;
                oldFormat = true
            }
            if ((!_emoVal || _emoVal <= 0) && entry.emotional != null) {
                patch.emo = _clamp10(entry.emotional, 5);
                oldFormat = true
            } else if (!_emoVal || _emoVal <= 0) {
                patch.emo = 5;
                oldFormat = true
            }
            if ((!_surVal || _surVal <= 0) && entry.surprise != null) {
                patch.sur = _clamp10(entry.surprise, 6);
                oldFormat = true
            } else if ((!_surVal || _surVal <= 0) && entry.confidence != null) {
                patch.sur = _clamp10(Number(entry.confidence) * 10, 6);
                oldFormat = true
            } else if (!_surVal || _surVal <= 0) {
                patch.sur = 6;
                oldFormat = true
            }
        ...
        try {
            ...
            for (const e of entries) {
                const {
                    patch: patch,
                    oldFormat: oldFormat
                } = normalizeEntryForMigration(e, currentTurn);
            ...
            if (C.cleanupStaleEmbeddings) {
                const clean = await C.cleanupStaleEmbeddings(null, {
                    model: settings.config.embeddingModel || C.DEFAULTS.embeddingModel
                });
                status.staleEmbeddingsRemoved = clean.removed || 0
            }
            status.message = status.oldFormatDetected ? "Old lore format detected and locally migrated. Review recommended." : "Local migration check complete.";
    ...
    window.addEventListener("storage", e => {
        if (e.key === "lore-injector-v5") settings.load()
    });
    window.addEventListener("focus", () => {
        if (Date.now() - (settings._lastSaveTime || 0) > 3e3) settings.load()
    });
    ...
    function incrementTurnCounter(chatKey) {
        return C.incrementTurn(chatKey)
    }

    function recordEntryMention(chatKey, entryId) {
        return C.recordMention(chatKey, entryId)
    }
    ...
    async function getAutoExtPackForUrl(url) {
        ...
        while (true) {
            ...
            if (!inDb && !existing.includes(checkName)) {
                finalName = checkName;
                break
            }
            counter++
    ...
    function getExtLog(chatKey) {
        return settings.config.urlExtLogs?.[chatKey] || []
    }
    ...
    function addExtLog(chatKey, logItem) {
        ...
        settings.config.urlExtLogs[chatKey] = logs;
        settings.save()
    }

    function clearExtLog(chatKey) {
        if (!settings.config.urlExtLogs) settings.config.urlExtLogs = {};
        settings.config.urlExtLogs[chatKey] = [];
        settings.save()
    }

    function getInjLog(chatKey) {
        return settings.config.urlInjLogs?.[chatKey] || []
    }
    ...
    function addInjLog(chatKey, logItem) {
        ...
        settings.config.urlInjLogs[chatKey] = logs;
        settings.save()
    }

    function clearInjLog(chatKey) {
        if (!settings.config.urlInjLogs) settings.config.urlInjLogs = {};
        settings.config.urlInjLogs[chatKey] = [];
        settings.save()
    }
    ...
    Object.assign(_w.__LoreInj, {
        ...
        convertLegacyEventToTimeline: convertLegacyEventToTimeline,
        getChatKey: getChatKey,
        incrementTurnCounter: incrementTurnCounter,
        recordEntryMention: recordEntryMention,
        getTurnCounter: getTurnCounter,
        setTurnCounter: setTurnCounter,
        getCooldownMap: getCooldownMap,
        setCooldownLastTurn: setCooldownLastTurn,
        getAutoExtPackForUrl: getAutoExtPackForUrl,
        setAutoExtPackForUrl: setAutoExtPackForUrl,
        getUrlStateKey: getUrlStateKey,
        getActivePacksForUrl: getActivePacksForUrl,
        getDisabledEntriesForUrl: getDisabledEntriesForUrl,
        getExtLog: getExtLog,
        addExtLog: addExtLog,
        clearExtLog: clearExtLog,
        getInjLog: getInjLog,
        addInjLog: addInjLog,
        clearInjLog: clearInjLog,
        isEntryEnabledForUrl: isEntryEnabledForUrl,
        setPackEnabled: setPackEnabled,
        setEntryEnabled: setEntryEnabled,
        getApiConfigSnapshot: getApiConfigSnapshot,
        resetSettingsKeepApi: resetSettingsKeepApi,
        runLocalMigration: runLocalMigration,
        getMigrationStatus: getMigrationStatus,
        ensureHeavyRuntimeInit: ensureHeavyRuntimeInit,
...
(async function() {
    ...
    if (!(_w.__LoreInj && _w.__LoreInj.__settingsLoaded)) {
        console.error("[LoreInj:4] settings 미로드");
        return
    }
    ...
    function normalizeEntryForMerge(entry, turn) {
        if (C.normalizeLoreEntry) return C.normalizeLoreEntry(entry, {
            turn: turn,
    ...
    function mergeCallState(existing, incoming) {
        ...
        for (const [k, v] of Object.entries(incoming)) {
            if (!base[k]) base[k] = v;
            else if (typeof base[k] === "object" && typeof v === "object") base[k] = {
                ...base[k],
                ...v
            };
            else base[k] = v
    ...
    function selectExistingForContext(entries, context, limit) {
        ...
        const scoreOne = e => {
            ...
            const hay = [e.name, ...e.triggers || [], e.embed_text || "", ...e.entities || [], ...e.parties || [], ...e.detail && e.detail.parties || []].join(" ").toLowerCase();
            if (name && ctx.includes(name)) s += 120;
            for (const t of e.triggers || []) {
                if (!t || t.length < 2) continue;
                const parts = String(t).split("&&").map(p => p.trim().toLowerCase()).filter(Boolean);
                if (parts.length && parts.every(p => ctx.includes(p))) {
                    s += parts.length > 1 ? 80 : 30;
                    break
                }
            }
            if (e.anchor) s += 100;
            ...
            if (e.lastUpdated) s += Math.max(0, 30 - Math.floor((Date.now() - e.lastUpdated) / 864e5));
    ...
    function entryContentSignature(entry) {
        try {
            return JSON.stringify(stableForCompare(entry || {}))
        } catch (_) {
            return String(entry && entry.name || "") + "|" + String(entry && entry.type || "")
        }
    }
    ...
    async function applyExtractPatchOp(op, packName, chatKey) {
        ...
        try {
            if (C.saveEntryVersion) await C.saveEntryVersion(existing, "extract_patch")
        } catch (_) {}
        ...
        if (set.summary) existing.summary = C.mergeLoreSummary ? C.mergeLoreSummary(existing.summary, set.summary, existing.name, set.state || existing.state) : {
            ...existing.summary || {},
            ...set.summary
        };
        ...
        try {
            if (C.normalizeTemporalGraph) existing = C.normalizeTemporalGraph(existing, {
                currentTurn: getTurnCounter(chatKey),
                sceneId: chatKey
            })
        } catch (_) {}
    ...
    async function applyTemporalPatchOp(op, packName, chatKey) {
        ...
        const beforeSig = entryContentSignature(existing);
        try {
            if (C.saveEntryVersion) await C.saveEntryVersion(existing, "temporal_patch")
        } catch (_) {}
        const set = op.set || {};
        const append = op.append || {};
        if (set.title !== undefined) existing.title = set.title;
        if (set.name !== undefined) existing.name = set.name;
        if (set.when && typeof set.when === "object") existing.when = {
            ...existing.when || {},
            ...set.when
        };
        if (set.location !== undefined) existing.location = set.location;
        if (set.summary) existing.summary = C.mergeLoreSummary ? C.mergeLoreSummary(existing.summary, set.summary, existing.name) : {
            ...existing.summary || {},
            ...set.summary
        };
        const appendArr = key => {
    ...
    function normalizeTemporalCandidates(parsed, limit) {
        ...
        return arr.filter(e => e && typeof e === "object").filter(e => e.op !== "patch").map(e => {
    ...
    function temporalDigestForExtract(e) {
        const summary = e.summary && typeof e.summary === "object" ? {
            ...
            compact: String(e.summary || "").slice(0, 180),
            micro: ""
        };
        return {
            id: e.id,
            ...
            name: e.name,
            when: e.when ? {
                anchor: e.when.anchor || "",
                relative: e.when.relative || "",
                turnStart: e.when.turnStart || e.timeline?.eventTurn || 0
            } : undefined,
    ...
    async function runTemporalExtractPass(opts = {}) {
        if (settings.config.temporalExtractEnabled === false) return {
            count: 0,
            skipped: true
        };
        ...
        if (!promptTpl || !schema || !context) return {
            count: 0,
            skipped: true
        };
        ...
        try {
            ...
            const temporalApiOpts = {
                ...
                maxRetries: 1,
                timeoutMs: 12e4,
                maxOutputTokens: _patchOn ? 1024 : null,
                costContext: {
                    feature: "temporalExtract",
                    chatKey: chatKey || "global"
                }
            };
            const {
                res: res,
                parsed: parsed
            } = await callGeminiJsonWithRepair(prompt, temporalApiOpts, "Use patch objects only when a real timeline memory changes.");
    ...
    async function mergeExtractedData(entries, url) {
        ...
        for (const item of normalizeExtractItems(entries)) {
            ...
            if (e.type === TL_TYPE_MERGE && C.stableTimelineEventId && !e.eventId) {
                try {
                    e.eventId = C.stableTimelineEventId(e)
                } catch (_) {}
            }
            let existing;
            if (e.type === TL_TYPE_MERGE) {
                const _tlId = e.eventId || (C.stableTimelineEventId ? function() {
                    try {
                        return C.stableTimelineEventId(e)
                    } catch (_) {
                        return null
                    }
                }() : null);
                if (_tlId) {
                    existing = await db.entries.where("packName").equals(packName).and(x => {
                        ...
                        try {
                            return !!(C.stableTimelineEventId && C.stableTimelineEventId(x) === _tlId)
                        } catch (_) {
                            return false
                        }
                    }).first()
            ...
            if (existing) {
                ...
                try {
                    if (C.saveEntryVersion) await C.saveEntryVersion(existing, "extract_merge")
                } catch (ex) {}
                ...
                if (e.type === "rel" && Array.isArray(e.arc)) {
                    ...
                    for (const a of e.arc) {
                        ...
                        if (dup) Object.assign(dup, a);
                        else existing.arc.push(a)
                    }
                }
                ...
                if (e.type === "relationship" || e.type === "rel") {
                    ...
                    if (parties && parties.length >= 2) {
                        const [c1, c2] = parties;
                        try {
                            await C.recordFirstEncounter(c1, c2, {
                                turnApprox: getTurnCounter(chatKey),
                                timestamp: Date.now()
                            })
                        } catch (ex) {}
                    }
                ...
                try {
                    if (C.normalizeTemporalGraph) e = C.normalizeTemporalGraph(e, {
                        currentTurn: getTurnCounter(chatKey),
                        sceneId: chatKey
                    })
                } catch (_) {}
    ...
    let _extBadgeMsg = null,
        _extBadgeWatchdog = null;

    function extBadgeShow(msg) {
        _extBadgeMsg = msg;
        try {
            C.showStatusBadge(msg)
        } catch (e) {}
        ...
        _extBadgeWatchdog = setInterval(() => {
            ...
            try {
                C.showStatusBadge(_extBadgeMsg)
            } catch (e) {}
        }, 1500)
    }
    ...
    function extBadgeHide() {
        _extBadgeMsg = null;
        if (_extBadgeWatchdog) {
            clearInterval(_extBadgeWatchdog);
            _extBadgeWatchdog = null
        }
        try {
            C.hideStatusBadge()
    ...
    document.addEventListener("visibilitychange", () => {
        if (!document.hidden && _extBadgeMsg) {
            try {
                C.showStatusBadge(_extBadgeMsg)
            } catch (e) {}
        }
    ...
    async function runAutoExtract(isManual = false) {
        if (_extQ.running) {
            _extQ.pendingTurns++;
            if (isManual) _extQ.manualPending = true;
            return
        }
        _extQ.running = true;
        _extQ.pendingTurns = 0;
        _extQ.manualPending = false;
        extBadgeShow("에리가 대화 분석 중");
        try {
            await _doExtract(isManual)
        } finally {
            _extQ.running = false;
            extBadgeHide();
            if (_extQ.pendingTurns > 0 || _extQ.manualPending) {
                const nextManual = _extQ.manualPending;
                setTimeout(() => runAutoExtract(nextManual), 500)
            }
        }
    }
    async function _doExtract(isManual) {
        ...
        if (missing) {
            if (isManual) alert("API 설정 미완료.");
            return
        }
        const scanR = settings.config.autoExtScanRange || 6;
        const extraTurns = _extQ.pendingTurns || 0;
        const effectiveRange = scanR + extraTurns;
        const fetchCount = (effectiveRange + settings.config.autoExtOffset) * 2;
        let recentMsgs = await C.fetchLogs(fetchCount > 0 ? fetchCount : 20);
        if (!recentMsgs.length) {
            if (isManual) alert("대화 기록 없음.");
            return
        }
        ...
        try {
            ...
            const {
                res: res,
                parsed: parsed
            } = await callGeminiJsonWithRepair(prompt, apiOpts, "Do not output a single object. Wrap patch/add items in an array.");
            ...
            if (isManual) {
                const temporalMsg = settings.config.temporalExtractEnabled !== false ? " / 중요 장면 " + (temporalResult && temporalResult.count || 0) + "개" : "";
                const baseMsg = generalCount > 0 ? generalCount + "개 로어 추출 및 병합됨" : "새로운 일반 로어 없음";
    ...
    async function runBatchExtract(opts = {}) {
        ...
        let _batchTotalElapsedMs = 0,
            _batchTotalUsd = 0;
        let _batchHasUnknown = false,
            _batchHasEstimated = false,
            _batchCostKnown = false;
        for (let bi = 0; bi < batches.length; bi++) {
            ...
            for (let attempt = 0; attempt < maxAttempts && !ok; attempt++) {
                ...
                try {
                    ...
                    if (res && res.cost) {
                        if (res.cost.usd != null) {
                            _batchTotalUsd += Number(res.cost.usd) || 0;
                            _batchCostKnown = true
            ...
            if (ok) {
                ...
                addExtLog(chatKey, {
                    ...
                    msgs: msgs.length,
                    isManual: true,
                    status: "배치 " + (bi + 1) + "/" + batches.length + " 실패",
                    error: lastErr,
                    model: _batchModel
                })
            }
        ...
        addExtLog(chatKey, {
            ...
            msgs: totalMsgs,
            isManual: true,
            status: "전체 추출 완료 (성공 " + report.ok + " / 빈 " + report.empty + " / 실패 " + report.failed + " / " + report.totalBatches + "개 배치, 병합 " + report.entriesAdded + "건)",
            model: _batchModel,
            elapsedMs: _batchTotalElapsedMs,
            cost: {
                usd: _batchCostKnown ? _batchTotalUsd : null,
                estimated: _batchHasEstimated,
                hasUnknown: _batchHasUnknown,
    ...
    Object.assign(_w.__LoreInj, {
        mergeExtractedData: mergeExtractedData,
        runAutoExtract: runAutoExtract,
        runBatchExtract: runBatchExtract,
        runTemporalExtractPass: runTemporalExtractPass,
        extBadgeShow: extBadgeShow,
        extBadgeHide: extBadgeHide,
...
(async function() {
    ...
    try {
        ...
        if (!(_w.__LoreInj && _w.__LoreInj.__extractLoaded)) {
            console.error("[LoreInj:5] extract 미로드");
            return
        }
        ...
        function resolveActivePackState(url) {
            ...
            return {
                key: key,
                packs: packs,
                disabled: disabledByUrl[key] || []
            }
        }

        function buildTemporalJudgeCandidates(scored, limit) {
            return (scored || []).filter(s => s && s.entry && C.isTimelineEvent && C.isTimelineEvent(s.entry)).slice(0, Math.max(1, limit || 6)).map(s => {
                const e = s.entry;
                return {
        ...
        async function runTemporalRecallJudge(userInput, recentMsgs, scored, config, apiOpts) {
            ...
            const recentText = Array.isArray(recentMsgs) ? recentMsgs.slice(-4).map(m => (m.role || "") + ": " + String(m.message || "").slice(0, 180)).join("\n") : "";
            ...
            const prompt = promptTpl.replace("{schema}", schema).replace("{context}", (recentText + "\nuser: " + String(userInput || "").slice(0, 500)).trim()).replace("{candidates}", JSON.stringify(candidates, null, 2));
            try {
                ...
                const _judgeCall = C.callGeminiApi(prompt, {
                    ...
                    responseMimeType: "application/json",
                    maxRetries: 1,
                    timeoutMs: Math.max(8e3, _judgeTimeoutMs + 2e3),
                    maxOutputTokens: 512,
                    thinkingConfig: {
                        thinkingLevel: config.temporalRecallJudgeReasoning || "minimal"
                    },
                    costContext: {
                        feature: "judge",
                        chatKey: getChatKey() || "global"
                    },
                    signal: _judgeAbortCtrl ? _judgeAbortCtrl.signal : undefined
                });
                let _judgeTimer = null;
                const res = _judgeTimeoutMs > 0 ? await Promise.race([_judgeCall, new Promise((_, rej) => {
                    _judgeTimer = setTimeout(() => {
                        try {
                            if (_judgeAbortCtrl) _judgeAbortCtrl.abort()
                        } catch (_) {}
                        rej(new Error("temporal_judge_timeout"))
                    }, _judgeTimeoutMs)
                })]).finally(() => {
                    if (_judgeTimer) {
                        clearTimeout(_judgeTimer);
                        _judgeTimer = null
                    }
                }) : await _judgeCall;
                if (!res || !res.text) return {
                    error: res?.error || "empty_response",
                    fallback: true
                };
        ...
        function applyTemporalJudge(scored, decision) {
            ...
            for (const s of scored || []) {
                ...
                if (!decision.recall) {
                    if (s.components?.timelineRecall || s.temporalRecall) s.score *= .55;
                    continue
                }
                if (selected.has(id)) {
        ...
        function buildTemporalInjectionPlan(scored, decision, config, ctx = {}) {
            ...
            else picked = rows.filter(s => hasTemporalSignal(s) && (s.temporalRecall || Number(s.components?.timelineRecall || 0) >= .12)).slice(0, naturalMax);
            ...
            const budget = validDecision ? Math.max(120, Math.min(650, Number(decision.maxChars || config.temporalRecallChars || 450))) : Math.max(80, Math.min(config.temporalRecallChars || 450, config.temporalRecallNaturalChars || 260));
        ...
        async function inject(userInput) {
            ...
            const activePacksArr = typeof _w.__LoreInj.getActivePacksForUrl === "function" ? _w.__LoreInj.getActivePacksForUrl(_url) : resolveActivePackState(_url).packs || [];
            if (!activePacksArr.length) {
                addInjLog(chatKey, {
                    ...
                    count: 0,
                    note: "활성 로어팩 없음",
                    reason: "no_active_packs",
                    url: _url
                });
                return userInput
            }
            const allForPacks = await db.entries.where("packName").anyOf(activePacksArr).toArray();
            const disabledSet = new Set(typeof _w.__LoreInj.getDisabledEntriesForUrl === "function" ? _w.__LoreInj.getDisabledEntriesForUrl(_url) : resolveActivePackState(_url).disabled || []);
            ...
            const searchConfig = {
                ...
                scanRange: config.scanRange || 6,
                scanOffset: config.scanOffset || 0,
                strictMatch: config.strictMatch !== false,
                similarityMatch: config.similarityMatch === true,
                embeddingEnabled: config.embeddingEnabled || false,
                embeddingWeight: config.embeddingWeight || .4,
                decayEnabled: config.decayEnabled !== false,
                decayHalfLife: config.decayHalfLife || C.DEFAULTS.decayHalfLife,
                ...
                timelineRecallWeight: config.timelineRecallWeight != null ? config.timelineRecallWeight : C.DEFAULTS.timelineRecallWeight || .32,
                timelineNoCuePenalty: config.timelineNoCuePenalty != null ? config.timelineNoCuePenalty : C.DEFAULTS.timelineNoCuePenalty || .35,
                timelineRecallPoolLimit: config.timelineRecallPoolLimit || 12,
                aiMemoryTurns: config.aiMemoryTurns || 4,
                activeCharDetection: config.activeCharDetection !== false,
                activeCharBoost: config.activeCharBoostEnabled !== false ? C.DEFAULTS.activeCharBoost : 1,
                inactiveCharPenalty: config.activeCharBoostEnabled !== false ? C.DEFAULTS.inactiveCharPenalty : 1
            };
            let scored = [],
                activeNames = [],
                temporalJudgeDecision = null;
            try {
                ...
                if (C.resolveTemporalRecall && config.timelineRetrievalEnabled !== false) {
                    const resolved = C.resolveTemporalRecall(userInput, recentMsgs, enabled, {
                    ...
                    if (resolved && resolved.candidates && resolved.candidates.length && (config.periodicRecallEnabled !== false || resolved.hasCue)) {
                        ...
                        for (const c of resolved.candidates) {
                            ...
                            if (old) {
                                ...
                                old.score += c.score * (resolved.hasCue ? .75 : .35);
                                old.temporalRecall = c
                            } else {
                                const row = {
                                    entry: c.entry,
                                    score: c.score * (resolved.hasCue ? .9 : .45),
                                    tScore: 0,
                                    eSim: 0,
                                    matchedTrigger: (c.matchedTriggers || []).join(","),
                                    temporalRecall: c
                                };
                                scored.push(row);
                ...
                if (r.searchStats && config.embeddingEnabled) {
                    ...
                    st.to += r.searchStats.trigOnly;
                    st.eo += r.searchStats.embOnly;
                    st.b += r.searchStats.both;
                    st.n++;
                    ...
                    if (config.autoTuneEmbeddingWeight === true && st.n > 0 && st.n % 20 === 0) {
                        ...
                        if (total > 5) {
                            const embContrib = (st.eo + st.b * .5) / total;
                            const avgReflection = st.injLog.length > 0 ? st.injLog.reduce((a, b) => a + b, 0) / st.injLog.length : .5;
                            const feedbackAdj = avgReflection > .4 ? 0 : (.4 - avgReflection) * .3;
                            const target = Math.max(.15, Math.min(.65, embContrib + feedbackAdj));
                            const smoothed = .7 * (config.embeddingWeight || .4) + .3 * target;
                            settings.config.embeddingWeight = parseFloat(smoothed.toFixed(3));
                            settings.save()
                        }
                    }
            ...
            if (config.pendingPromiseBoost !== false) {
                for (const s of scored) {
                    if (s.entry.type === "promise" && s.entry.detail?.status === "pending") s.score = Math.max(s.score, .3)
                }
                scored.sort((a, b) => b.score - a.score)
            }
            for (const s of scored) {
                if (s.entry.rootId && !s.entry.isCurrentArc) s.score *= .3
            }
            scored.sort((a, b) => b.score - a.score);
            if (activeNames.length >= 2 && config.firstEncounterWarning !== false) {
                for (let i = 0; i < activeNames.length; i++) {
                    for (let j = i + 1; j < activeNames.length; j++) {
                        try {
                            await C.recordFirstEncounter(activeNames[i], activeNames[j], {
                                turnApprox: turnCounter
                            })
                        } catch (e) {}
                    }
                }
            ...
            let _recentInj = {};
            try {
                _recentInj = JSON.parse(_ls.getItem(_deltaKey) || "{}")
            } catch (e) {}
            ...
            const temporalPlan = buildTemporalInjectionPlan(_filteredScored, temporalJudgeDecision, config, {
                currentTurn: turnCounter
            });
            ...
            if (!topEntries.length && !temporalPlan.text) {
                if (!cooldownFilteredAll) {
                    ...
                    try {
                        const ti = document.__toastifyInjector || new ToastifyInjector();
                        ti.doToastifyAlert("⏳인젝트 후보 없음", 3000)
                    } catch (e) {}
                }
                return userInput
            }
            ...
            if (C.charLen(userInput) >= MAX_INPUT_CHARS - 20) {
                addInjLog(chatKey, {
                    ...
                    note: "공간부족",
                    budgetPlan: {
                        userChars: C.charLen(userInput),
                        maxInputChars: MAX_INPUT_CHARS
                    },
                    finalChars: C.charLen(userInput),
                    reason: "user_input_too_long"
                });
                try {
                    const ti = document.__toastifyInjector || new ToastifyInjector();
                    ti.doToastifyAlert("⏳공간 부족", 3000)
                } catch (e) {}
                return userInput
            }
            let honorifics = "";
            ...
            let unmetPairs = [];
            if (config.firstEncounterWarning !== false) try {
                unmetPairs = await C.findUnmetPairs(activeNames)
            } catch (e) {}
            ...
            const temporalHints = config.firstEncounterWarning !== false && C.formatTemporalHints ? C.formatTemporalHints(topEntries, {
                currentTurn: turnCounter,
                activeNames: activeNames,
                budget: Math.min(config.temporalHintChars || C.DEFAULTS.temporalHintChars || 120, 120)
            }) : "";
            ...
            if (!fmtResult.injected) {
                ...
                try {
                    const ti = document.__toastifyInjector || new ToastifyInjector();
                    if (fmtResult.reason === "user_input_too_long" || fmtResult.reason === "no_space_after_user_and_wrapper" || fmtResult.reason === "cancel_over_limit") {
                        ti.doToastifyAlert("⏳공간 부족", 3000)
                    } else {
                        ti.doToastifyAlert("⏳인젝트 후보 없음", 3000)
                    }
            ...
            try {
                const sk = "lore-hybrid-stats";
                const st = JSON.parse(_ls.getItem(sk) || "{}");
                st.lastInjected = allIncluded.map(e => e.name);
                _ls.setItem(sk, JSON.stringify(st))
            } catch (e) {}
            ...
            addInjLog(chatKey, {
                time: (new Date).toLocaleTimeString(),
                turn: turnCounter,
                matched: allIncluded.map(e => e.name),
                count: allIncluded.length,
                budget: fmtResult.budgetPlan?.loreBudget || 0,
                used: fmtResult.usedChars,
                level: fmtResult.level,
                activeChars: activeNames.slice(0, 5),
                userInputChars: _userLen,
                injectedChars: _injectedLen,
                totalChars: _finalChars,
...
(async function() {
    ...
    if (!(_w.__LoreInj && _w.__LoreInj.__injectLoaded)) {
        console.error("[LoreInj:6] inject 미로드");
        return
    }
    ...
    function stableMenuQueue(queue, prefix) {
        return (queue || []).map((item, index) => ({
            ...
            const ao = MENU_ORDER[a.key] ?? 1e4;
            const bo = MENU_ORDER[b.key] ?? 1e4;
    ...
    function mountQueuedMenus(modal) {
        ...
        menuQ.forEach(({
            ...
            try {
                ...
                console.error(`[LoreInj:6] 메뉴 등록 실패 (${key}):`, e);
                _w.__LoreInj?.markFailed?.("menu:" + key, e)
            }
        });
        subQ.forEach(({
            key: key,
            ...
            try {
                ...
                console.error(`[LoreInj:6] 서브메뉴 등록 실패 (${key}):`, e);
                _w.__LoreInj?.markFailed?.("submenu:" + key, e)
            }
        })
    }
...
(async function() {
    ...
    while (!(_w.__LoreInj && _w.__LoreInj.__settingsLoaded) && Date.now() < deadline) await new Promise(r => setTimeout(r, 50));
    if (!(_w.__LoreInj && _w.__LoreInj.__settingsLoaded)) {
        console.error("[LoreInj:sub-main] settings 미로드");
        return
    }
        ...
        modal.createMenu("로어 설정", m => {
            m.replaceContentPanel(async panel => {
                const PRESETS = {
                    ...
                    minimal: {
                        name: "수동 검색",
                        desc: "자동 추출 OFF. 수동 추출만 사용. API 호출 최소.",
                        config: {
                            embeddingEnabled: true,
                            embeddingWeight: .35,
                    ...
                    advanced: {
                        name: "정밀",
                        desc: "5턴 자동 정리 + 후보 재정렬 + 응답 교정. 장문 RP용.",
                        config: {
                            embeddingEnabled: true,
                            embeddingWeight: .4,
                            autoExtEnabled: true,
                ...
                panel.addBoxedField("", "", {
                    onInit: nd => {
                        ...
                        const chip = (label, value, ok) => {
                            ...
                            v.textContent = value;
                            v.style.cssText = "font-size:12px;font-weight:bold;color:" + (ok ? "#4a9" : "#da8") + ";white-space:nowrap;overflow:hidden;text-overflow:ellipsis;";
                            c.appendChild(l);
                            c.appendChild(v);
                            grid.appendChild(c);
                            return v
                        ...
                        db.entries.toArray().then(entries => {
                            ...
                            entryValue.textContent = "확인 실패";
                            entryValue.style.color = "#d66"
                        })
                    }
                });
                panel.addBoxedField("", "", {
                    onInit: nd => {
                        ...
                        const t = document.createElement("div");
                        t.textContent = "빠른 설정";
                        t.style.cssText = "font-size:14px;color:#4a9;font-weight:bold;margin-bottom:8px;";
                        nd.appendChild(t);
                        const row = document.createElement("div");
                        row.style.cssText = "display:flex;gap:8px;flex-wrap:wrap;";
                        for (const [key, preset] of Object.entries(PRESETS)) {
                            const btn = document.createElement("button");
                            btn.style.cssText = "padding:10px 14px;font-size:12px;border-radius:6px;cursor:pointer;border:1px solid #333;background:#1a1a1a;color:#ccc;display:flex;flex-direction:column;gap:4px;text-align:left;flex:1;min-width:110px;";
                            const nm = document.createElement("div");
                            nm.textContent = preset.name;
                            nm.style.cssText = "font-weight:bold;color:#4a9;font-size:13px;";
                            const ds = document.createElement("div");
                            ds.textContent = preset.desc;
                            ds.style.cssText = "font-size:10px;color:#888;";
                            btn.appendChild(nm);
                            btn.appendChild(ds);
                            btn.onclick = () => {
                                if (!confirm("[" + preset.name + "] 프리셋 적용?")) return;
                                settings.config = JSON.parse(JSON.stringify(_w.__LoreInj.defaultSettings));
                ...
                panel.addBoxedField("", "", {
                    onInit: nd => {
                        C.setFullWidth(nd);
                        nd.appendChild(C.createToggleRow("로어 인젝션 활성화", "대화에 설정 정보를 자동 삽입함.", settings.config.enabled, v => {
                            settings.config.enabled = v;
                            settings.save()
                        }));
                        nd.appendChild(C.createToggleRow("적응형 로어 압축", "주입 공간 부족 시 텍스트를 자동으로 짧게 줄임.", settings.config.useCompressedFormat !== false, v => {
                            settings.config.useCompressedFormat = v;
                            settings.save()
                        }));
                        const cmpWrap = document.createElement("div");
                        cmpWrap.style.cssText = "display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;padding-left:10px;";
                        const cmpLbl = document.createElement("div");
                        cmpLbl.textContent = "압축 모드";
                        cmpLbl.style.cssText = "font-size:12px;color:#aaa;";
                        const cmpSel = document.createElement("select");
                        cmpSel.style.cssText = "width:120px;padding:4px;border:1px solid #333;border-radius:4px;background:#0a0a0a;color:#ccc;font-size:11px;";
                        [{
                            v: "auto",
                            l: "자동 (공간 맞춤)"
                        }, {
                        ...
                        cmpSel.onchange = () => {
                            settings.config.compressionMode = cmpSel.value;
                            settings.save()
                        };
                        cmpWrap.appendChild(cmpLbl);
                        cmpWrap.appendChild(cmpSel);
                        nd.appendChild(cmpWrap)
                    }
                });
                panel.addBoxedField("", "", {
                    onInit: nd => {
                        ...
                        const t = document.createElement("div");
                        t.textContent = "검색 & 감지";
                        t.style.cssText = "font-size:14px;color:#4a9;font-weight:bold;margin-bottom:8px;padding-bottom:6px;border-bottom:1px solid #333;";
                        nd.appendChild(t);
                        nd.appendChild(C.createToggleRow("의미로 찾기", "단어가 달라도 관련 로어 찾음. API/검색 준비 필요.", settings.config.embeddingEnabled, v => {
                            settings.config.embeddingEnabled = v;
                            settings.save()
                        }));
                        const emRow = document.createElement("div");
                        emRow.style.cssText = "display:flex;justify-content:space-between;align-items:center;gap:10px;width:100%;margin-bottom:8px;";
                        const emL = document.createElement("div");
                        emL.style.cssText = "display:flex;flex-direction:column;gap:4px;flex:1;";
                        const eml1 = document.createElement("div");
                        eml1.textContent = "의미 검색 모델";
                        eml1.style.cssText = "font-size:13px;color:#ccc;font-weight:bold;";
                        emL.appendChild(eml1);
                        const emSel = document.createElement("select");
                        emSel.style.cssText = "width:200px;padding:6px;border:1px solid #333;border-radius:4px;background:#0a0a0a;color:#ccc;font-size:12px;";
                        [{
                            v: "gemini-embedding-001",
                            l: "gemini-embedding-001"
                        }, {
                            v: "gemini-embedding-2-preview",
                            l: "gemini-embedding-2-preview"
                        }].forEach(o => {
                            const opt = document.createElement("option");
                            opt.value = o.v;
                            opt.textContent = o.l;
                        ...
                        emSel.onchange = () => {
                            settings.config.embeddingModel = emSel.value;
                            settings.save();
                            alert("모델 변경됨. 기존 로어 검색 준비 재실행 필요.")
                        };
                        emRow.appendChild(emL);
                        emRow.appendChild(emSel);
                        nd.appendChild(emRow);
                        nd.appendChild(C.createToggleRow("추출 후 검색 준비", "새 로어를 의미 검색용으로 자동 준비함.", settings.config.autoEmbedOnExtract !== false, v => {
                            settings.config.autoEmbedOnExtract = v;
                            settings.save()
                        }));
                        nd.appendChild(C.createToggleRow("오래된 정보도 가끔 넣기", "직접 관련이 약해도 중요한 과거 정보를 주기적으로 넣음.", settings.config.periodicRecallEnabled !== false, v => {
                            settings.config.periodicRecallEnabled = v;
                            settings.config.decayEnabled = v;
                            settings.save()
                        }));
                        nd.appendChild(C.createToggleRow("AI로 후보 다시 고르기", "검색 후보를 AI가 현재 장면 기준으로 다시 정렬함.", settings.config.rerankEnabled || false, v => {
                ...
                panel.addBoxedField("", "", {
                    onInit: nd => {
                        ...
                        t.style.cssText = "font-size:13px;color:#ccc;font-weight:bold;";
                        const d = document.createElement("div");
                        d.textContent = "메시지 기준 로어 삽입 위치.";
                        d.style.cssText = "font-size:11px;color:#888;";
                        left.appendChild(t);
                        left.appendChild(d);
                        const right = document.createElement("div");
                        ...
                        b1.textContent = "메시지 앞";
                        b1.onclick = () => {
                            settings.config.position = "before";
                            settings.save();
                            updateBtns()
                        };
                        b2.textContent = "메시지 뒤";
                        b2.onclick = () => {
                            settings.config.position = "after";
                            settings.save();
                            updateBtns()
                        };
                        updateBtns();
                ...
                panel.addBoxedField("", "", {
                    onInit: nd => {
                        ...
                        tr.textContent = "추가 정보 주입";
                        tr.style.cssText = "font-size:14px;color:#4a9;font-weight:bold;margin-bottom:8px;padding-bottom:6px;border-bottom:1px solid #333;";
                        nd.appendChild(tr);
                        nd.appendChild(C.createToggleRow("호칭 정보", "캐릭터 간 호칭 정보 함께 전달함.", settings.config.honorificMatrixEnabled !== false, v => {
                            settings.config.honorificMatrixEnabled = v;
                            settings.save()
                        }));
                        nd.appendChild(C.createToggleRow("첫 만남/재회 관리", "첫 만남/재회 여부 자동 전달함.", settings.config.firstEncounterWarning !== false, v => {
                ...
                panel.addBoxedField("", "", {
                    onInit: nd => {
                        ...
                        t1.textContent = "출력 포맷";
                        t1.style.cssText = "font-size:14px;color:#4a9;font-weight:bold;margin-bottom:8px;padding-bottom:6px;border-bottom:1px solid #333;";
                        nd.appendChild(t1);
                        const oocSel = document.createElement("select");
                        oocSel.style.cssText = "width:100%;padding:6px 8px;border:1px solid #333;border-radius:4px;background:#0a0a0a;color:#ccc;font-size:12px;box-sizing:border-box;margin-bottom:12px;";
                        for (const [k, v] of Object.entries(OOC_FORMATS)) {
                            const opt = document.createElement("option");
                            opt.value = k;
                            opt.textContent = v.name + " — " + v.desc;
                            oocSel.appendChild(opt)
                        }
                        oocSel.value = settings.config.oocFormat || "custom";
                        const pInp = document.createElement("input");
                        pInp.value = settings.config.prefix || "";
                        pInp.style.cssText = "width:100%;padding:6px 8px;border:1px solid #333;border-radius:4px;background:#0a0a0a;color:#ccc;font-size:12px;box-sizing:border-box;margin-bottom:12px;";
                        pInp.onchange = () => {
                            settings.config.prefix = pInp.value;
                            settings.save()
                        };
                        const sInp = document.createElement("input");
                        sInp.value = settings.config.suffix || "";
                        sInp.style.cssText = "width:100%;padding:6px 8px;border:1px solid #333;border-radius:4px;background:#0a0a0a;color:#ccc;font-size:12px;box-sizing:border-box;margin-bottom:20px;";
                        sInp.onchange = () => {
                            settings.config.suffix = sInp.value;
                            settings.save()
                        };
                        oocSel.onchange = () => {
                            const fmt = OOC_FORMATS[oocSel.value];
                            if (fmt && oocSel.value !== "custom") {
                                pInp.value = fmt.prefix;
                                sInp.value = fmt.suffix;
                                settings.config.prefix = fmt.prefix;
                                settings.config.suffix = fmt.suffix
                            }
                            settings.config.oocFormat = oocSel.value;
                            settings.save();
                            pInp.disabled = oocSel.value !== "custom";
                            sInp.disabled = oocSel.value !== "custom";
                            pInp.style.opacity = oocSel.value !== "custom" ? "0.6" : "1";
                            sInp.style.opacity = oocSel.value !== "custom" ? "0.6" : "1"
                        };
                        nd.appendChild(oocSel);
                        nd.appendChild(pInp);
                        nd.appendChild(sInp);
                        const isCustom = (settings.config.oocFormat || "custom") === "custom";
                        pInp.disabled = !isCustom;
                        sInp.disabled = !isCustom;
                        pInp.style.opacity = isCustom ? "1" : "0.6";
                        sInp.style.opacity = isCustom ? "1" : "0.6"
                    }
                });
                panel.addBoxedField("", "", {
                    onInit: nd => {
                        ...
                        resetBtn.textContent = "모든 설정 초기화 (DB 유지)";
...
(async function() {
    ...
    while (!(_w.__LoreInj && _w.__LoreInj.__settingsLoaded) && Date.now() < deadline) await new Promise(r => setTimeout(r, 50));
    if (!(_w.__LoreInj && _w.__LoreInj.__settingsLoaded)) {
        console.error("[LoreInj:sub-lore] settings 미로드");
        return
    }
    ...
    _w.__LoreInj.registerSubMenu("lore", function(modal) {
        modal.createSubMenu("로어 관리 (목록)", m => {
            const renderPanel = async panel => {
                const _url = C.getCurUrl();
                const activePacks = _w.__LoreInj.getActivePacksForUrl ? _w.__LoreInj.getActivePacksForUrl(_url) : settings.config.urlPacks?.[_url] || [];
                if (!activePacks.length) {
                    panel.addText("활성화된 팩이 없습니다. 파일 탭에서 활성화하세요.");
                    return
                }
                const entries = await db.entries.toArray();
                const filtered = entries.filter(e => activePacks.includes(e.packName));
                if (!filtered.length) {
                    panel.addText("활성 항목 없음.");
                    return
                }
                const byPack = {};
                filtered.forEach(e => {
                    (byPack[e.packName] = byPack[e.packName] || []).push(e)
                });
                for (const [pk, items] of Object.entries(byPack)) {
                    panel.addBoxedField("", "", {
                        onInit: nd => {
                            C.setFullWidth(nd);
                            nd.style.cssText += "background:#1a1a1a;border:1px solid #333;border-radius:4px;margin-bottom:12px;";
                            ...
                            const pkSw = document.createElement("div");
                            pkSw.style.cssText = "width:28px;height:14px;border-radius:7px;background:" + (allItemsEnabled ? "#285" : "#444") + ";position:relative;cursor:pointer;flex-shrink:0;margin-right:8px;";
                            const pkDot = document.createElement("div");
                            pkDot.style.cssText = "width:10px;height:10px;border-radius:50%;background:#fff;position:absolute;top:2px;left:" + (allItemsEnabled ? "16px" : "2px") + ";transition:left .2s;";
                            ...
                            const title = document.createElement("div");
                            title.style.cssText = "font-size:14px;font-weight:bold;color:#ccc;flex:1;";
                            title.textContent = pk + " (" + items.length + "개)";
                            const arrow = document.createElement("span");
                            arrow.textContent = "▼";
                            arrow.style.cssText = "font-size:12px;color:#888;";
                            headerRow.appendChild(pkSw);
                            ...
                            for (const e of items) {
                                ...
                                swWrap.onclick = ev => {
                                    ev.stopPropagation();
                                    const ns = !isEntryEnabledForUrl(e);
                                    setEntryEnabled(e, ns);
                                    swLabel.textContent = ns ? "ON" : "OFF";
                                    swLabel.style.color = ns ? "#4a7" : "#777";
                                    sw.style.background = ns ? "#285" : "#444";
                                    dot.style.left = ns ? "14px" : "2px"
                                };
                                ...
                                const embStatusSpan = document.createElement("span");
                                embStatusSpan.style.cssText = "margin-left:6px;font-size:10px;padding:1px 4px;border-radius:3px;";
                                const updateEmbStatus = async () => {
                                    try {
                                        const emb = await db.embeddings.where({
                                            entryId: e.id,
                                            field: "summary"
                                        }).first() || await db.embeddings.where("entryId").equals(e.id).first();
                                        if (emb) {
                                            const ch = C.embeddingSourceHash ? C.embeddingSourceHash(e, emb.field || "summary") : emb.hash || "";
                                            const targetModel = settings.config.embeddingModel || "gemini-embedding-001";
                                            if ((emb.sourceHash || emb.hash) === ch && (!emb.packName || emb.packName === e.packName)) {
                                                const needsRegen = emb.model && emb.model !== targetModel || emb.schemaVersion && emb.schemaVersion < 2 || emb.taskType !== "RETRIEVAL_DOCUMENT" && targetModel.includes("embedding-001");
                                                if (needsRegen) {
                                                    embStatusSpan.textContent = "재생성";
                                                    embStatusSpan.style.background = "#3a3a1a";
                                                    embStatusSpan.style.color = "#d96"
                                                } else {
                                                    embStatusSpan.textContent = "임베딩";
                                                    embStatusSpan.style.background = "#1a3a2a";
                                                    embStatusSpan.style.color = "#4a9"
                                                }
                                            } else {
                                                embStatusSpan.textContent = "변경됨";
                                                embStatusSpan.style.background = "#3a3a1a";
                                                embStatusSpan.style.color = "#d96"
                                            }
                                        } else {
                                            embStatusSpan.textContent = "미임베딩";
                                            embStatusSpan.style.background = "#2a2a2a";
                                            embStatusSpan.style.color = "#888"
                                        }
                                    } catch (ex) {}
                                };
                                updateEmbStatus();
                                nameSpan.appendChild(embStatusSpan);
                                left.appendChild(swWrap);
                                ...
                                const embGenBtn = document.createElement("button");
                                embGenBtn.textContent = "임베딩";
                                embGenBtn.style.cssText = B + "color:#4a9;border-color:#264;";
                                embGenBtn.onclick = async ev => {
                                    ev.stopPropagation();
                                    embGenBtn.disabled = true;
                                    embGenBtn.textContent = "...";
                                    try {
                                        await C.ensureEmbedding(e, {
                                        ...
                                        embGenBtn.textContent = "OK";
                                        updateEmbStatus()
                                    } catch (err) {
                                        embGenBtn.textContent = "X";
                                        alert("실패:" + err.message)
                                    }
                                    setTimeout(() => {
                                        embGenBtn.textContent = "임베딩";
                                        embGenBtn.disabled = false
                                    }, 1500)
                                };
                                const copyBtn = document.createElement("button");
                                copyBtn.textContent = "복사";
                                copyBtn.style.cssText = B + "color:#4a9;border-color:#264;";
                                copyBtn.onclick = ev => {
                                    ev.stopPropagation();
                                    const clean = {
                                        ...e
                                    ...
                                    delete clean.id;
                                    delete clean.packName;
                                    delete clean.project;
                                    delete clean.enabled;
                                    navigator.clipboard.writeText(JSON.stringify(clean, null, 2)).then(() => alert("복사됨.")).catch(() => alert("실패"))
                                };
                                const editBtn = document.createElement("button");
                                editBtn.textContent = "수정";
                                editBtn.style.cssText = B + "color:#88c;border-color:#446;";
                                const delBtn = document.createElement("button");
                                delBtn.textContent = "삭제";
                                delBtn.style.cssText = B + "color:#a55;border-color:#633;";
                                const histBtn = document.createElement("button");
                                histBtn.textContent = "이력";
                                histBtn.style.cssText = B + "color:#da8;border-color:#642;";
                                const anchorBtn = document.createElement("button");
                                const _renderAnchor = () => {
                                    const on = !!e.anchor;
                                    anchorBtn.textContent = on ? "앵커" : "앵커";
                                    anchorBtn.title = on ? "앵커 해제 — 자동 추출 병합 시 보호 해제됨" : "앵커 지정 — summary/state/detail/call/inject 자동 덮어쓰기 차단, 재주입 우선도 최대";
                                    anchorBtn.style.cssText = B + (on ? "color:#fc4;border-color:#963;background:#2a1a00;" : "color:#777;border-color:#444;")
                                };
                                _renderAnchor();
                                anchorBtn.onclick = async ev => {
                                    ...
                                    try {
                                        ...
                                        alert("앵커 토글 실패: " + err.message);
                                        e.anchor = !e.anchor;
                                        _renderAnchor()
                                    }
                                };
                                right.appendChild(embGenBtn);
                                ...
                                histBtn.onclick = async ev => {
                                    ...
                                    try {
                                        if (!C.getEntryVersions) {
                                            historyContainer.innerHTML = '<div style="font-size:11px;color:#d66;">버전 이력 기능 미로드.</div>';
                                            return
                                        }
                                        const versions = await C.getEntryVersions(e.id);
                                        if (!versions || !versions.length) {
                                            historyContainer.innerHTML = '<div style="font-size:11px;color:#888;">저장된 버전 없음.</div>';
                                            return
                                        }
                                        historyContainer.innerHTML = "";
                                        const hdr = document.createElement("div");
                                        hdr.textContent = versions.length + "개 버전 (최신순)";
                                        hdr.style.cssText = "font-size:11px;color:#da8;margin-bottom:6px;font-weight:bold;";
                                        historyContainer.appendChild(hdr);
                                        for (const v of versions) {
                                            const vrow = document.createElement("div");
                                            vrow.style.cssText = "display:flex;justify-content:space-between;align-items:flex-start;padding:4px 0;border-bottom:1px dashed #222;gap:8px;";
                                            const info = document.createElement("div");
                                            info.style.cssText = "font-size:11px;color:#aaa;flex:1;min-width:0;";
                                            ...
                                            vbtns.style.cssText = "display:flex;gap:4px;flex-shrink:0;";
                                            const resBtn = document.createElement("button");
                                            resBtn.textContent = "복원";
                                            resBtn.style.cssText = "padding:3px 8px;font-size:10px;border-radius:3px;background:#258;color:#fff;border:none;cursor:pointer;";
                                            ...
                                            vrow.appendChild(info);
                                            vrow.appendChild(vbtns);
                                            historyContainer.appendChild(vrow)
                                        }
                                    } catch (err) {
                                        historyContainer.innerHTML = '<div style="font-size:11px;color:#d66;">오류: ' + err.message + "</div>"
                                    }
                                };
                                ...
                                delete editableObj.id;
                                delete editableObj.packName;
                                delete editableObj.project;
                                delete editableObj.enabled;
                                ta.value = JSON.stringify(editableObj, null, 2);
                                const btnRow = document.createElement("div");
                                btnRow.style.cssText = "display:flex;justify-content:flex-end;gap:6px;";
                                const saveBtn = document.createElement("button");
                                saveBtn.textContent = "저장";
                                saveBtn.style.cssText = B + "background:#285;border-color:#285;color:#fff;";
                                const cancelBtn = document.createElement("button");
                                cancelBtn.textContent = "닫기";
                                cancelBtn.style.cssText = B;
                                ...
                                nameSpan.onclick = toggleEdit;
                                editBtn.onclick = toggleEdit;
                                cancelBtn.onclick = toggleEdit;
                                saveBtn.onclick = async () => {
                                    try {
                                        ...
                                        try {
                                            if (C.invalidateEntryEmbeddings) await C.invalidateEntryEmbeddings(updated.id)
                                        } catch (_) {}
                                        alert("수정됨. 임베딩은 변경됨으로 표시되며 필요 시 재생성하세요.");
                                        Object.assign(e, updated);
                                        nameSpan.textContent = "[" + updated.type + "] " + updated.name;
                                        nameSpan.appendChild(embStatusSpan);
                                        updateEmbStatus();
                                        toggleEdit()
                                    } catch (err) {
                                ...
                                delBtn.onclick = async () => {
                                    if (confirm("[" + e.name + "] 삭제?")) {
                                        await db.entries.delete(e.id);
                                        try {
                                            await db.embeddings.where("entryId").equals(e.id).delete()
                                        } catch (ex) {}
                                        const count = await db.entries.where("packName").equals(e.packName).count();
                                        if (count <= 0) {
                                            await db.packs.delete(e.packName);
                                            row.remove();
                                            m.replaceContentPanel(renderLoreUI, "로어 관리")
                                        } else {
                                            await db.packs.update(e.packName, {
                                                entryCount: count
                                            });
                                            row.remove();
...
(async function() {
    ...
    while (!(_w.__LoreInj && _w.__LoreInj.__settingsLoaded) && Date.now() < deadline) await new Promise(r => setTimeout(r, 50));
    if (!(_w.__LoreInj && _w.__LoreInj.__settingsLoaded)) {
        console.error("[LoreInj:sub-merge] settings 미로드");
        return
    }
            ...
            const renderMerge = async panel => {
                ...
                panel.addBoxedField("", "", {
                    onInit: nd => {
                        C.setFullWidth(nd);
                        const t = document.createElement("div");
                        t.textContent = "중복 로어 병합";
                        t.style.cssText = "font-size:14px;color:#4a9;font-weight:bold;margin-bottom:4px;";
                        nd.appendChild(t);
                        const d = document.createElement("div");
                        d.innerHTML = '임베딩 유사도로 중복 후보를 찾아 그룹별 수동 승인. 캐릭터/관계/약속/장소/사건처럼 대분류가 다른 로어는 유사해도 한 엔트리로 접지 않고 같은 대분류 안에서만 병합 후보를 만든다.<br><span style="color:#da8;">※ 현재 페이지에서 활성화(ON)된 팩의 엔트리만 대상 — 다른 페이지나 비활성 팩의 로어는 건드리지 않음.</span>';
                        d.style.cssText = "font-size:11px;color:#888;margin-bottom:10px;line-height:1.5;";
                        nd.appendChild(d);
                        const row = document.createElement("div");
                        ...
                        const mk = (label, getter, setter, min, max, step) => {
                            ...
                            const l = document.createElement("div");
                            l.textContent = label;
                            l.style.cssText = "font-size:12px;color:#888;margin-bottom:4px;";
                        ...
                        row.appendChild(mk("유사도 임계값", () => state.threshold, v => state.threshold = v, .7, .99, .01));
                        row.appendChild(mk("최대 글자수 (summary)", () => state.maxChars, v => state.maxChars = v, 200, 3e3, 50));
                        nd.appendChild(row);
                        const runBtn = document.createElement("button");
                        runBtn.textContent = "병합 후보 찾기";
                        ...
                        runStatus.style.cssText = "font-size:11px;color:#888;margin-top:6px;text-align:center;";
                        runBtn.onclick = async () => {
                            ...
                            try {
                                ...
                                if (entries.length < 2) {
                                    runStatus.textContent = "활성 엔트리 2개 미만.";
                                    runStatus.style.color = "#d66";
                                    return
                                }
                                ...
                                if (withEmb.length < 2) {
                                    runStatus.textContent = "임베딩 있는 엔트리 2개 미만. 먼저 파일 탭에서 임베딩 생성 필요.";
                                    runStatus.style.color = "#d66";
                                    return
                                }
                                const cos = (a, b) => {
                                ...
                                runStatus.textContent = "실패: " + e.message;
                                runStatus.style.color = "#d66"
                            }
                            runBtn.textContent = "병합 후보 찾기";
                            runBtn.disabled = false
                        };
                        nd.appendChild(runBtn);
                        nd.appendChild(runStatus);
                        const bulkRow = document.createElement("div");
                        bulkRow.style.cssText = "display:flex;gap:8px;align-items:center;margin-top:10px;padding:8px;background:#111;border:1px solid #333;border-radius:4px;";
                        const bulkLbl = document.createElement("div");
                        bulkLbl.textContent = "일괄 모드 변경";
                        bulkLbl.style.cssText = "font-size:11px;color:#888;white-space:nowrap;";
                        const bulkSel = document.createElement("select");
                        bulkSel.style.cssText = "flex:1;padding:6px;border:1px solid #333;border-radius:4px;background:#0a0a0a;color:#ccc;font-size:11px;";
                        [
                            ["", "-- 선택하여 모든 그룹에 일괄 적용 --"],
                            ["keep-longest", "가장 긴 항목 유지 + 키워드 합집합 (안전)"],
                            ["llm-summarize", "LLM 요약 병합 (API 호출, 품질↑)"]
                        ].forEach(([v, l]) => {
                        ...
                        bulkSel.onchange = () => {
                            ...
                            const sels = document.querySelectorAll("select.lore-merge-mode-sel");
                            sels.forEach(s => {
                                s.value = bulkSel.value
                            });
                            bulkSel.value = ""
                        };
                        bulkRow.appendChild(bulkLbl);
                        bulkRow.appendChild(bulkSel);
                        nd.appendChild(bulkRow);
                        if (C.__lastMergeUndo && C.__lastMergeUndo.originals && C.__lastMergeUndo.originals.length) {
                            const undoBtn = document.createElement("button");
                            undoBtn.textContent = "직전 병합 취소 (" + C.__lastMergeUndo.originals.length + "개 엔트리 복원)";
                            ...
                            undoBtn.onclick = async () => {
                                ...
                                try {
                                    ...
                                    if (undo.mergedId !== undefined && undo.mergedId !== null) {
                                        ...
                                        try {
                                            await db.embeddings.where("entryId").equals(undo.mergedId).delete()
                                        } catch (_) {}
                                    }
                                    ...
                                    for (const pk of packs) {
                                        const cnt = await db.entries.where("packName").equals(pk).count();
                                        await db.packs.update(pk, {
                                            entryCount: cnt
                                        })
                                    }
                ...
                if (state.groups.length === 0) {
                    panel.addText("후보 없음. 임계값을 낮춰 재시도할 수 있음.");
                    return
                }
                for (const [gi, grp] of state.groups.entries()) {
                    panel.addBoxedField("", "", {
                        onInit: nd => {
                            ...
                            const hdr = document.createElement("div");
                            hdr.style.cssText = "display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;padding-bottom:6px;border-bottom:1px solid #333;";
                            const title = document.createElement("div");
                            title.textContent = "그룹 " + (gi + 1) + " — " + grp.entries.length + "개 / 유사도 " + (grp.sim * 100).toFixed(1) + "%";
                            ...
                            warn.textContent = typeWarn;
                            warn.style.cssText = "font-size:10px;color:" + (types.length > 1 ? "#d96" : "#888") + ";";
                            ...
                            for (const e of grp.entries) {
                                const row = document.createElement("div");
                                row.style.cssText = "padding:6px 0;border-bottom:1px dashed #222;font-size:11px;color:#aaa;";
                            ...
                            ctrl.style.cssText = "margin-top:10px;display:flex;gap:8px;align-items:center;";
                            const modeSel = document.createElement("select");
                            modeSel.className = "lore-merge-mode-sel";
                            modeSel.style.cssText = "padding:6px;border:1px solid #333;border-radius:4px;background:#0a0a0a;color:#ccc;font-size:11px;flex:1;";
                            [
                                ["keep-longest", "가장 긴 항목 유지 + 키워드 합집합 (안전)"],
                                ["llm-summarize", "LLM 요약 병합 (API 호출, 품질↑)"]
                            ].forEach(([v, l]) => {
                            ...
                            previewBtn.textContent = "프리뷰";
                            previewBtn.style.cssText = "padding:6px 12px;font-size:11px;border-radius:4px;background:transparent;border:1px solid #446;color:#88c;cursor:pointer;";
                            const execBtn = document.createElement("button");
                            execBtn.textContent = "병합 실행";
                            execBtn.style.cssText = "padding:6px 12px;font-size:11px;border-radius:4px;background:#285;border:none;color:#fff;cursor:pointer;font-weight:bold;";
                            ctrl.appendChild(modeSel);
                            ctrl.appendChild(previewBtn);
                            ctrl.appendChild(execBtn);
                            nd.appendChild(ctrl);
                            const preview = document.createElement("div");
                            preview.style.cssText = "margin-top:8px;padding:8px;background:#0a0a0a;border:1px solid #222;border-radius:4px;font-size:11px;color:#ccc;display:none;white-space:pre-wrap;word-break:break-all;max-height:300px;overflow-y:auto;";
                            ...
                            const buildPreview = async () => {
                                ...
                                try {
                                    ...
                                    preview.textContent = "X " + e.message;
                                    mergedDraft = null
                                }
                            };
                            ...
                            execBtn.onclick = async () => {
                                ...
                                if (types.length > 1) {
                                    if (!confirm("타입 불일치 (" + types.join(", ") + "). 계속?")) return
                                }
                                ...
                                try {
                                    ...
                                    for (const e of grp.entries) {
                                        try {
                                            if (C.saveEntryVersion) await C.saveEntryVersion(e, "pre_merge")
                                        } catch (_) {}
                                    }
                                    ...
                                    for (const e of toDelete) {
                                        await db.entries.delete(e.id);
                                        try {
                                            if (C.invalidateEntryEmbeddings) await C.invalidateEntryEmbeddings(e.id);
                                            else await db.embeddings.where("entryId").equals(e.id).delete()
                                        } catch (_) {}
                                    }
                                    try {
                                        if (C.invalidateEntryEmbeddings) await C.invalidateEntryEmbeddings(target.id);
                                        else await db.embeddings.where("entryId").equals(target.id).delete()
                                    } catch (_) {}
                                    ...
                                    try {
                                        ...
                                        if (hasApi) {
                                            ...
                                            embedMsg = "API 미설정 — 파일 탭에서 수동 임베딩 필요."
                                        }
                                    } catch (embErr) {
                                        embedMsg = "임베딩 재생성 실패: " + (embErr.message || embErr)
                                    }
...
(async function() {
    ...
    while (!(_w.__LoreInj && _w.__LoreInj.__settingsLoaded) && Date.now() < deadline) await new Promise(r => setTimeout(r, 50));
    if (!(_w.__LoreInj && _w.__LoreInj.__settingsLoaded)) {
        console.error("[LoreInj:sub-snapshot] settings 미로드");
        return
    }
            ...
            const renderSnapshotUI = async panel => {
                panel.addBoxedField("", "", {
                    onInit: async nd => {
                        C.setFullWidth(nd);
                        const t = document.createElement("div");
                        t.textContent = "스냅샷 복원";
                        t.style.cssText = "font-size:14px;color:#ccc;font-weight:bold;margin-bottom:8px;";
                        nd.appendChild(t);
                        const snaps = await db.snapshots.orderBy("timestamp").reverse().toArray();
                        if (!snaps.length) {
                            nd.appendChild(Object.assign(document.createElement("div"), {
                                textContent: "저장된 스냅샷이 없습니다.",
                        ...
                        for (const s of snaps) {
                            const row = document.createElement("div");
                            row.style.cssText = "display:flex;justify-content:space-between;align-items:center;padding:6px;background:#1a1a1a;border:1px solid #333;border-radius:4px;";
                            const info = document.createElement("div");
                            info.style.cssText = "display:flex;flex-direction:column;";
                            const sTitle = document.createElement("span");
                            sTitle.textContent = `[${s.packName}] ${s.label} (${s.data.length}개)`;
                            sTitle.style.cssText = "font-size:12px;color:#4a9;font-weight:bold;";
                            const sTime = document.createElement("span");
                            sTime.textContent = new Date(s.timestamp).toLocaleString();
                            sTime.style.cssText = "font-size:10px;color:#888;";
                            ...
                            rBtn.textContent = "복원";
                            rBtn.style.cssText = "padding:4px 8px;font-size:11px;border-radius:3px;background:#258;color:#fff;border:none;cursor:pointer;";
                            rBtn.onclick = async () => {
                                if (confirm(`[${s.packName}] 팩을 이 시점으로 복원할 것?\n기존 데이터는 덮어씌워집니다.`)) {
                            ...
                            dBtn.textContent = "삭제";
                            dBtn.style.cssText = "padding:4px 8px;font-size:11px;border-radius:3px;background:transparent;color:#d66;border:1px solid #d66;cursor:pointer;";
                            dBtn.onclick = async () => {
                                if (confirm("삭제?")) {
...
(async function() {
    ...
    while (!(_w.__LoreInj && _w.__LoreInj.__settingsLoaded) && Date.now() < deadline) await new Promise(r => setTimeout(r, 50));
    if (!(_w.__LoreInj && _w.__LoreInj.__settingsLoaded)) {
        console.error("[LoreInj:sub-file] settings 미로드");
        return
    }
            ...
            const renderPackUI = async panel => {
                panel.addBoxedField("", "", {
                    onInit: nd => {
                        C.setFullWidth(nd);
                        const title = document.createElement("div");
                        title.textContent = "로어 가져오기";
                        title.style.cssText = "font-size:14px;color:#ccc;font-weight:bold;margin-bottom:8px;";
                        nd.appendChild(title);
                        const row = document.createElement("div");
                        row.style.cssText = "display:flex;gap:8px;align-items:center;margin-bottom:8px;";
                        const nameInput = document.createElement("input");
                        nameInput.placeholder = "로어 이름";
                        nameInput.style.cssText = "flex:1;padding:6px 8px;border:1px solid #333;border-radius:4px;background:#0a0a0a;color:#ccc;font-size:12px;";
                        ...
                        importBtn.textContent = "JSON 파일 가져오기";
                        ...
                        fileInput.onchange = async ev => {
                            ...
                            try {
                                ...
                                for (let e of arr) {
                                    ...
                                    if (C.normalizeLoreEntry) e = C.normalizeLoreEntry(e, {
                                        source: "imported"
                                    });
                                ...
                                let pack = await db.packs.get(packName);
                                if (pack) await db.packs.update(packName, {
                                    entryCount: totalCount
                                });
                                else await db.packs.put({
                                    name: packName,
                                    entryCount: totalCount,
                                    project: settings.config.activeProject || ""
                                });
                                await setPackEnabled(packName, true);
                        ...
                        nd.appendChild(row);
                        const manualLbl = document.createElement("div");
                        manualLbl.textContent = "또는 직접 JSON 입력";
                        manualLbl.style.cssText = "font-size:12px;color:#888;margin:12px 0 4px;";
                        nd.appendChild(manualLbl);
                        ...
                        manualBtnRow.style.cssText = "display:flex;justify-content:flex-end;margin-top:6px;";
                        const manualBtn = document.createElement("button");
                        manualBtn.textContent = "수동 추가";
                            ...
                            try {
                                ...
                                for (let e of arr) {
                                    ...
                                    if (ex) {
                                        ...
                                        try {
                                            if (C.invalidateEntryEmbeddings) await C.invalidateEntryEmbeddings(ex.id)
                                        } catch (_) {}
                                    } else {
                                        await db.entries.add(e);
                                        cnt++
                                ...
                                const tc = await db.entries.where("packName").equals(pn).count();
                                let pk = await db.packs.get(pn);
                                if (pk) await db.packs.update(pn, {
                                    entryCount: tc
                                });
                                else await db.packs.put({
                                    name: pn,
                                ...
                                alert(arr.length + "개 처리 (신규 " + cnt + "개)");
                                manualTa.value = "";
                                m.replaceContentPanel(renderPackUI, "파일 관리")
                            } catch (err) {
                        ...
                        manualBtnRow.appendChild(manualBtn);
                        nd.appendChild(manualBtnRow)
                    }
                });
                panel.addBoxedField("", "", {
                    onInit: async nd => {
                        ...
                        for (const p of rawPacks) {
                            ...
                            if ((p.entryCount || 0) !== count) await db.packs.update(p.name, {
                                entryCount: count
                            });
                            packs.push({
                                ...p,
                        ...
                        if (!packs.length) {
                            ...
                            empty.textContent = "등록된 팩이 없습니다.";
                            empty.style.cssText = "color:#666;text-align:center;padding:20px;font-size:12px;";
                            nd.appendChild(empty);
                            return
                        }
                        const curUrl = C.getCurUrl();
                        const enabledPacks = _w.__LoreInj.getActivePacksForUrl ? _w.__LoreInj.getActivePacksForUrl(curUrl) : settings.config.urlPacks?.[curUrl] || [];
                        for (const pack of packs) {
                            const packDiv = document.createElement("div");
                            packDiv.style.cssText = "margin-bottom:8px;border:1px solid #333;border-radius:4px;overflow:hidden;";
                            const header = document.createElement("div");
                            header.style.cssText = "display:flex;justify-content:space-between;align-items:center;padding:10px 12px;background:#111;";
                            ...
                            const swWrap = document.createElement("div");
                            swWrap.style.cssText = "display:flex;align-items:center;gap:6px;cursor:pointer;";
                            const swLabel = document.createElement("span");
                            swLabel.textContent = isEnabled ? "ON" : "OFF";
                            swLabel.style.cssText = "font-size:10px;font-weight:bold;width:20px;text-align:center;color:" + (isEnabled ? "#4a7" : "#777") + ";";
                            const sw = document.createElement("div");
                            ...
                            swWrap.onclick = async () => {
                                ...
                                swLabel.textContent = nowEnabled ? "ON" : "OFF";
                                swLabel.style.color = nowEnabled ? "#4a7" : "#777";
                                sw.style.background = nowEnabled ? "#285" : "#444";
                                dot.style.left = nowEnabled ? "14px" : "2px"
                            };
                            leftSide.appendChild(swWrap);
                            const nameEl = document.createElement("span");
                            nameEl.textContent = pack.name + " (" + (pack.entryCount || 0) + "개)";
                            nameEl.style.cssText = "font-size:13px;color:#ccc;font-weight:bold;";
                            ...
                            embBtn.textContent = "임베딩";
                            embBtn.style.cssText = B + "color:#4a9;border-color:#264;";
                            embBtn.onclick = async () => {
                                const apiType = settings.config.autoExtApiType || "key";
                                const miss = apiType === "vertex" ? !settings.config.autoExtVertexJson : apiType === "firebase" ? !settings.config.autoExtFirebaseEmbedKey : !settings.config.autoExtKey;
                                if (miss) {
                                    alert(apiType === "firebase" ? "임베딩용 Gemini API 키 필요." : "API 설정 필요.");
                                    return
                                }
                                if (!confirm("[" + pack.name + "] 임베딩 생성?")) return;
                                embBtn.disabled = true;
                                const orig = embBtn.textContent;
                                try {
                                    const cnt = await C.embedPack(pack.name, {
                                        apiType: apiType,
                                        key: settings.config.autoExtKey,
                                        vertexJson: settings.config.autoExtVertexJson,
                                        vertexLocation: settings.config.autoExtVertexLocation || "global",
                                        ...
                                        embBtn.textContent = done + "/" + total
                                    });
                                    embBtn.textContent = "OK" + cnt;
                                    setTimeout(() => {
                                        embBtn.textContent = orig;
                                        embBtn.disabled = false
                                    }, 2e3)
                                } catch (e) {
                                    embBtn.textContent = "X";
                                    embBtn.disabled = false;
                                    alert("실패:" + e.message)
                                }
                            };
                            const cleanBtn = document.createElement("button");
                            cleanBtn.textContent = "정리";
                            cleanBtn.title = "API 호출 없이 stale embeddings 삭제";
                            cleanBtn.style.cssText = B + "color:#da8;border-color:#642;";
                            cleanBtn.onclick = async () => {
                                cleanBtn.disabled = true;
                                const orig = cleanBtn.textContent;
                                cleanBtn.textContent = "...";
                                try {
                                    const rpt = C.cleanupStaleEmbeddings ? await C.cleanupStaleEmbeddings(pack.name, {
                                        model: settings.config.embeddingModel || "gemini-embedding-001"
                                    }) : {
                                        removed: 0
                                    };
                                    cleanBtn.textContent = "정리 " + rpt.removed;
                                    alert("stale embedding 정리 완료: " + JSON.stringify(rpt))
                                } catch (e) {
                                    cleanBtn.textContent = "X";
                                    alert("정리 실패: " + e.message)
                                }
                                setTimeout(() => {
                                    cleanBtn.textContent = orig;
                            ...
                            delBtn.style.cssText = B + "color:#a55;border-color:#633;";
                            delBtn.onclick = async () => {
                                if (!confirm("[" + pack.name + "] 삭제?")) return;
                                const es = await db.entries.where("packName").equals(pack.name).toArray();
                                for (const e of es) await db.embeddings.where("entryId").equals(e.id).delete();
                                await db.entries.where("packName").equals(pack.name).delete();
                                await db.packs.delete(pack.name);
                                m.replaceContentPanel(renderPackUI, "파일 관리")
                            };
                            actions.appendChild(exportBtn);
                            actions.appendChild(embBtn);
...
(async function() {
    ...
    function buildGenerationApiOpts(overrides = {}, costContext = null) {
        const cfg = settings.config || {};
        const model = cfg.autoExtModel === "_custom" ? cfg.autoExtCustomModel : cfg.autoExtModel;
        const opts = {
        ...
        modal.createSubMenu("추출/변환 설정", m => {
            m.replaceContentPanel(panel => {
                panel.addBoxedField("", "", {
                    onInit: nd => {
                        C.setFullWidth(nd);
                        nd.appendChild(C.createToggleRow("자동 대화 정리", "정해진 턴마다 대화를 로어에 추가함.", settings.config.autoExtEnabled, v => {
                            settings.config.autoExtEnabled = v;
                            settings.save()
                        }));
                        nd.appendChild(C.createToggleRow("기존 로어 참고", "저장된 로어를 같이 참고해 중복 저장 줄임.", settings.config.autoExtIncludeDb, v => {
                            settings.config.autoExtIncludeDb = v;
                            settings.save()
                        }));
                        nd.appendChild(C.createToggleRow("변경분만 저장", "기존 로어 전체 대신 바뀐 부분만 받아 출력 토큰 줄임.", settings.config.autoExtPatchMode !== false, v => {
                            settings.config.autoExtPatchMode = v;
                            settings.save()
                        }));
                        nd.appendChild(C.createToggleRow("페르소나 정보 전송", "추출 시 페르소나 이름을 같이 보내 정확도 올림.", settings.config.autoExtIncludePersona, v => {
                            settings.config.autoExtIncludePersona = v;
                            settings.save()
                        }));
                        nd.appendChild(C.createToggleRow("중요 장면 기억하기", "중요 사건/약속을 나중에 떠올릴 수 있게 별도 정리함.", settings.config.temporalExtractEnabled !== false, v => {
                            settings.config.temporalExtractEnabled = v;
                            settings.save()
                        }));
                        const row1 = document.createElement("div");
                        row1.style.cssText = "display:flex;gap:12px;margin-bottom:8px;align-items:center;";
                        ...
                        row1.appendChild(makeInput("자동 정리 주기", "autoExtTurns", 8));
                        row1.appendChild(makeInput("읽을 최근 대화", "autoExtScanRange", 6));
                        row1.appendChild(makeInput("최근 제외", "autoExtOffset", 5));
                        row1.appendChild(makeInput("중요 장면 최대", "temporalMaxEventsPerPass", 5));
                        nd.appendChild(row1);
                        const injTitle = document.createElement("div");
                        injTitle.textContent = "삽입 옵션";
                        injTitle.style.cssText = "font-size:13px;color:#ccc;font-weight:bold;margin:12px 0 6px;padding-top:10px;border-top:1px solid #333;";
                        nd.appendChild(injTitle);
                        nd.appendChild(C.createToggleRow("삽입 쿨타임 사용", "같은 로어가 너무 자주 들어가지 않게 막음.", settings.config.cooldownEnabled !== false, v => {
                            settings.config.cooldownEnabled = v;
                            settings.save()
                        }));
                        nd.appendChild(C.createToggleRow("오래된 정보도 가끔 넣기", "직접 관련이 약해도 중요한 과거 정보를 주기적으로 넣음.", settings.config.periodicRecallEnabled !== false, v => {
                            settings.config.periodicRecallEnabled = v;
                        ...
                        const injRow = document.createElement("div");
                        injRow.style.cssText = "display:flex;gap:12px;margin:8px 0 12px;align-items:center;";
                        ...
                        row2.style.cssText = "display:flex;gap:12px;margin-bottom:12px;align-items:center;";
                        const f3 = document.createElement("div");
                        f3.style.flex = "1";
                        const l3 = document.createElement("div");
                        l3.textContent = "저장할 로어팩";
                        l3.style.cssText = "font-size:12px;color:#888;margin-bottom:4px;";
                        ...
                        const savePackName = () => {
                            ...
                            setAutoExtPackForUrl(C.getCurUrl(), val)
                        };
                        i3.oninput = savePackName;
                        i3.onchange = savePackName;
                        ...
                        s3.onchange = () => {
                            if (s3.value) {
                                ...
                                setAutoExtPackForUrl(C.getCurUrl(), s3.value);
                                s3.value = ""
                            }
                        };
                        inputWrap.appendChild(i3);
                        inputWrap.appendChild(s3);
                        f3.appendChild(l3);
                        f3.appendChild(inputWrap);
                        row2.appendChild(f3);
                        nd.appendChild(row2);
                        const btnRun = document.createElement("button");
                        btnRun.textContent = "수동 추출 실행";
                        ...
                        btnStatus.style.cssText = "font-size:11px;color:#888;margin-top:6px;text-align:center;line-height:1.4;";
                ...
                panel.addBoxedField("", "", {
                    onInit: nd => {
                        ...
                        jt.textContent = "과거 장면 불러오기 판단";
                        jt.style.cssText = "font-size:14px;color:#4a9;font-weight:bold;margin-bottom:6px;";
                        nd.appendChild(jt);
                        const jd = document.createElement("div");
                        jd.textContent = "사용 여부와 호출 범위만 여기서 조절함. 모델/생각 깊이는 API 설정에서 관리함.";
                        jd.style.cssText = "font-size:11px;color:#888;margin-bottom:10px;line-height:1.4;";
                        nd.appendChild(jd);
                        nd.appendChild(C.createToggleRow("AI로 참고 장면 고르기", "규칙 판단 뒤 AI가 필요한 과거 장면 한 번 더 고름.", settings.config.temporalRecallJudgeEnabled, v => {
                            settings.config.temporalRecallJudgeEnabled = v;
                            settings.save()
                        }));
                        const jrow = document.createElement("div");
                        jrow.style.cssText = "display:flex;gap:12px;margin-top:10px;";
                        const jmkNum = (label, key, defaultVal, min, max) => {
                            const f = document.createElement("div");
                            f.style.flex = "1";
                            const l = document.createElement("div");
                            l.textContent = label;
                            l.style.cssText = "font-size:11px;color:#999;margin-bottom:4px;";
                            const i = document.createElement("input");
                            i.type = "number";
                            i.value = settings.config[key] !== undefined ? settings.config[key] : defaultVal;
                        ...
                        jrow.appendChild(jmkNum("응답 제한 시간(ms)", "temporalRecallJudgeTimeoutMs", 8e3, 1e3, 6e4));
                ...
                panel.addBoxedField("", "", {
                    onInit: nd => {
                        ...
                        bTitle.textContent = "전체 로그 일괄 추출";
                        bTitle.style.cssText = "font-size:14px;color:#4a9;font-weight:bold;margin-bottom:8px;";
                        nd.appendChild(bTitle);
                        const bDesc = document.createElement("div");
                        bDesc.textContent = "긴 대화를 배치로 나눠 정리함. API 비용 큼. 초기 정리용.";
                        bDesc.style.cssText = "font-size:11px;color:#888;margin-bottom:10px;line-height:1.4;";
                        nd.appendChild(bDesc);
                        const bRow = document.createElement("div");
                        bRow.style.cssText = "display:flex;gap:12px;margin-bottom:8px;align-items:center;";
                        const mkNum = (label, getter, setter, defaultVal) => {
                            ...
                            i.type = "number";
                            const cur = getter();
                            i.value = cur !== undefined && cur !== null ? cur : defaultVal;
                            i.style.cssText = "width:100%;padding:6px;border:1px solid #333;border-radius:4px;background:#0a0a0a;color:#ccc;font-size:12px;box-sizing:border-box;";
                            const save = () => {
                                const v = parseInt(i.value);
                                if (!isNaN(v)) {
                        ...
                        bStatus.style.cssText = "font-size:11px;color:#888;margin-top:6px;text-align:center;line-height:1.5;";
                        bBtn.onclick = async () => {
                            if (!confirm("전체 로그를 배치로 분석함. API 비용 큼. 계속?")) return;
                            settings.save();
                            bBtn.disabled = true;
                            const orig = bBtn.textContent;
                            bBtn.textContent = "실행 중...";
                            bStatus.textContent = "전체 로그 가져오는 중";
                            bStatus.style.color = "#4a9";
                            ...
                            try {
                                const report = await _w.__LoreInj.runBatchExtract({
                                    ...
                                    onProgress: ev => {
                                        const sec = Math.floor((Date.now() - start) / 1e3);
                ...
                panel.addBoxedField("", "", {
                    onInit: nd => {
                        ...
                        urlInp.type = "text";
                        urlInp.placeholder = "URL 입력";
                        urlInp.style.cssText = S;
                        nd.appendChild(urlInp);
                        const nameInp = document.createElement("input");
                        nameInp.type = "text";
                        nameInp.placeholder = "팩 이름";
                        nameInp.style.cssText = S;
                        nd.appendChild(nameInp);
                        const rDiv = document.createElement("div");
                        rDiv.style.cssText = "font-size:12px;color:#888;margin-top:8px;";
                        const urlBtn = document.createElement("button");
                        urlBtn.textContent = "URL 변환";
                        urlBtn.style.cssText = "padding:8px 16px;font-size:12px;border-radius:4px;cursor:pointer;background:#285;color:#fff;border:none;font-weight:bold;";
                        urlBtn.onclick = async () => {
                            if (!urlInp.value.trim() || !nameInp.value.trim()) {
                                alert("URL과 팩이름 필요.");
                                return
                            }
                            urlBtn.disabled = true;
                            urlBtn.textContent = "변환중...";
                            const startMs = Date.now();
                            ...
                            try {
                                const cnt = await C.importFromUrl(urlInp.value.trim(), nameInp.value.trim(), buildGenerationApiOpts({}, {
                                    feature: "urlImport",
                                    chatKey: C.getCurrentChatId && C.getCurrentChatId() || "global"
                                }), {
                                    onProgress: ev => {
                                        if (!ev) return;
                                        switch (ev.phase) {
                                            ...
                                            case "fetch:try": {
                                                ...
                                                const sec = Math.max(1, Math.floor((ev.timeoutMs || 15e3) / 1e3));
                        ...
                        t2.innerHTML = '<div style="font-size:13px;color:#ccc;font-weight:bold;margin-top:16px;margin-bottom:8px;">텍스트 → 로어 팩</div>';
                        nd.appendChild(t2);
                        const ta = document.createElement("textarea");
                        ta.placeholder = "설정, 소설 텍스트 등";
                        ta.style.cssText = S + "height:100px;resize:vertical;";
                        nd.appendChild(ta);
                        const nameInp2 = document.createElement("input");
                        nameInp2.type = "text";
                        nameInp2.placeholder = "팩 이름";
                        nameInp2.style.cssText = S;
                        nd.appendChild(nameInp2);
                        const rDiv2 = document.createElement("div");
                        rDiv2.style.cssText = "font-size:12px;color:#888;margin-top:8px;";
                        const tBtn = document.createElement("button");
                        tBtn.textContent = "텍스트 변환";
                        tBtn.style.cssText = "padding:8px 16px;font-size:12px;border-radius:4px;cursor:pointer;background:#285;color:#fff;border:none;font-weight:bold;";
                        tBtn.onclick = async () => {
                            if (!ta.value.trim() || !nameInp2.value.trim()) {
                                alert("입력값 필요.");
                                return
                            }
                            tBtn.disabled = true;
                            tBtn.textContent = "변환중...";
                            const startMs = Date.now();
...
(async function() {
    ...
    _w.__LoreInj.registerSubMenu("refiner", function(modal) {
        modal.createSubMenu("AI 응답 교정", m => {
            m.replaceContentPanel(async panel => {
                if (!R) {
                    panel.addText("Refiner 라이브러리 없음.");
                    return
                }
                panel.addBoxedField("", "", {
                    onInit: nd => {
                        ...
                        const t = document.createElement("div");
                        t.textContent = "수동 검수";
                        t.style.cssText = "font-size:13px;color:#4a9;font-weight:bold;margin-bottom:4px;";
                        nd.appendChild(t);
                        const d = document.createElement("div");
                        d.textContent = "마지막 AI 응답을 지금 즉시 재검수. 이미 처리된 응답도 다시 돌릴 수 있음.";
                        d.style.cssText = "font-size:11px;color:#888;margin-bottom:8px;line-height:1.4;";
                        nd.appendChild(d);
                        const btnBox = document.createElement("div");
                        btnBox.style.cssText = "position:relative;";
                        const btn = document.createElement("button");
                        btn.textContent = "최근 AI 응답 재검수";
                        btn.style.cssText = "width:100%;padding:10px;background:#258;color:#fff;border:none;border-radius:4px;cursor:pointer;font-size:12px;font-weight:bold;";
                        const statusLine = document.createElement("div");
                        statusLine.style.cssText = "font-size:11px;margin-top:6px;text-align:center;line-height:1.4;min-height:16px;display:flex;align-items:center;justify-content:center;gap:6px;color:#888;";
                        btnBox.appendChild(btn);
                        btnBox.appendChild(statusLine);
                        nd.appendChild(btnBox);
                        ...
                        const makeDot = () => {
                            ...
                            d.style.cssText = "display:inline-block;width:8px;height:8px;border-radius:50%;background:#4a9;animation:lore-refiner-pulse 1s infinite;";
                            return d
                        };
                        btn.onclick = async () => {
                            if (!R.manualRefine) {
                                alert("Refiner 버전 낮음. Tampermonkey에서 스크립트 수동 업데이트 필요.");
                                return
                            }
                            ...
                            try {
                                ...
                                const sec = Math.floor((Date.now() - start) / 1e3);
                                statusLine.innerHTML = "";
                                statusLine.textContent = `완료 (${sec}초)`;
                                statusLine.style.color = "#4a9";
                                setTimeout(() => {
                                ...
                                clearInterval(tick);
                                statusLine.innerHTML = "";
                                statusLine.textContent = "실패: " + String(e.message || e).slice(0, 50);
                ...
                panel.addBoxedField("", "", {
                    onInit: nd => {
                        C.setFullWidth(nd);
                        nd.appendChild(C.createToggleRow("응답 교정 켜기", "AI 응답 시 로어 기반 자동 검수.", settings.config.refinerEnabled, v => {
                            settings.config.refinerEnabled = v;
                            settings.save();
                            if (v && R.setNeedsWarmup) R.setNeedsWarmup()
                        }));
                        nd.appendChild(C.createToggleRow("자동 반영 (팝업 없음)", "검수 결과를 팝업 없이 즉시 적용.", settings.config.refinerAutoMode, v => {
                            settings.config.refinerAutoMode = v;
                            settings.save()
                        }));
                        nd.appendChild(C.createToggleRow("상태 배지 표시", "진행 상태를 화면 우측에 띄움. 모바일에서 겹치면 끄기.", settings.config.statusBadgeEnabled !== false, v => {
                            settings.config.statusBadgeEnabled = v;
                            settings.save();
                            if (!v && C.hideStatusBadge) C.hideStatusBadge()
                        }));
                        ...
                        const renderLive = () => {
                            ...
                            if (!st) {
                                live.textContent = "상태: 대기";
                                return
                            }
                            const age = st.at ? Math.max(0, Math.floor((Date.now() - st.at) / 1e3)) : 0;
                            live.textContent = "상태: " + (st.state || "idle") + (st.detail ? " · " + st.detail : "") + " · " + age + "초 전"
                        };
                        renderLive();
                        try {
                            if (R.__refinerStatusUiTimer) clearInterval(R.__refinerStatusUiTimer)
                        } catch (_) {}
                        R.__refinerStatusUiTimer = setInterval(renderLive, 1e3);
                        nd.appendChild(live);
                        const S = "width:100%;padding:6px 8px;border:1px solid #333;border-radius:4px;background:#0a0a0a;color:#ccc;font-size:12px;box-sizing:border-box;margin-bottom:8px;";
                        const modeWrap = document.createElement("div");
                        modeWrap.style.cssText = "display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;";
                        const modeLbl = document.createElement("div");
                        modeLbl.textContent = "로어 검색 모드";
                        modeLbl.style.cssText = "font-size:13px;color:#ccc;font-weight:bold;";
                        const modeSel = document.createElement("select");
                        modeSel.style.cssText = "width:160px;padding:4px;border:1px solid #333;border-radius:4px;background:#0a0a0a;color:#ccc;font-size:12px;";
                        [{
                            v: "matchedOnly",
                            l: "키워드 매칭만"
                        }, {
                        ...
                        modeSel.onchange = () => {
                            settings.config.refinerLoreMode = modeSel.value;
                            settings.save()
                        };
                        modeWrap.appendChild(modeLbl);
                        modeWrap.appendChild(modeSel);
                        nd.appendChild(modeWrap);
                        const tplWrap = document.createElement("div");
                        tplWrap.style.cssText = "margin-bottom:8px;padding-top:8px;border-top:1px dashed #333;";
                        const tplLbl = document.createElement("div");
                        tplLbl.textContent = "검수 템플릿 선택";
                        tplLbl.style.cssText = "font-size:13px;color:#ccc;font-weight:bold;margin-bottom:4px;";
                        const tplSel = document.createElement("select");
                        tplSel.style.cssText = S;
                        const tplDesc = document.createElement("div");
                        tplDesc.style.cssText = "font-size:11px;color:#888;margin-bottom:8px;line-height:1.4;";
                        const customOpt = document.createElement("option");
                        customOpt.value = "custom";
                        customOpt.textContent = "직접 입력 (커스텀)";
                        tplSel.appendChild(customOpt);
                        if (R.TEMPLATES) {
                            Object.entries(R.TEMPLATES).forEach(([k, t]) => {
                                const opt = document.createElement("option");
                                opt.value = k;
                        ...
                        if (R.TOPICS) {
                            ...
                            dynOpt.value = "dynamic";
                            dynOpt.textContent = "주제별 선택 (체크박스)";
                            tplSel.appendChild(dynOpt)
                        }
                        tplWrap.appendChild(tplLbl);
                        tplWrap.appendChild(tplSel);
                        tplWrap.appendChild(tplDesc);
                        nd.appendChild(tplWrap);
                        ...
                        const topicsHdr = document.createElement("div");
                        topicsHdr.textContent = "검수 주제 선택 — 호칭은 최신 안정 상태 기준으로만 검사";
                        topicsHdr.style.cssText = "font-size:12px;color:#4a9;font-weight:bold;margin-bottom:8px;padding-bottom:6px;border-bottom:1px dashed #333;";
                        ...
                        const renderTopics = () => {
                            if (!settings.config.refinerTopics) {
                                const def = {};
                                Object.keys(R.TOPICS || {}).forEach(k => def[k] = true);
                            ...
                            Object.entries(R.TOPICS || {}).forEach(([k, meta]) => {
                                ...
                                lbl.style.cssText = "font-size:12px;color:#ccc;font-weight:bold;";
                                const dsc = document.createElement("div");
                                dsc.textContent = meta.desc;
                                dsc.style.cssText = "font-size:10px;color:#888;line-height:1.4;";
                                txt.appendChild(lbl);
                                txt.appendChild(dsc);
                                cb.onchange = () => {
                        ...
                        const wrap = document.createElement("div");
                        wrap.style.cssText = "display:flex;justify-content:space-between;align-items:center;gap:10px;width:100%;margin-bottom:12px;";
                        const left = document.createElement("div");
                        left.style.cssText = "display:flex;flex-direction:column;gap:4px;flex:1;";
                        const t = document.createElement("div");
                        t.textContent = "참조 대화 턴 수";
                        t.style.cssText = "font-size:13px;color:#ccc;font-weight:bold;";
                        left.appendChild(t);
                        const right = document.createElement("div");
                        const inp = document.createElement("input");
                        inp.type = "number";
                        inp.value = settings.config.refinerContextTurns !== undefined ? settings.config.refinerContextTurns : 1;
                        inp.min = 0;
                        inp.max = 20;
                        ...
                        inp.onchange = () => {
                            settings.config.refinerContextTurns = parseInt(inp.value) || 0;
                            settings.save()
                        };
                        ...
                        wrap.appendChild(right);
                        nd.appendChild(wrap);
                        const tLbl2 = document.createElement("div");
                        tLbl2.textContent = "프롬프트 미리보기";
                        tLbl2.style.cssText = "font-size:11px;color:#999;margin-bottom:4px;";
                        nd.appendChild(tLbl2);
                        const ta = document.createElement("textarea");
                        ta.value = settings.config.refinerCustomPrompt;
                        ta.style.cssText = S + "height:200px;font-family:monospace;resize:vertical;";
                        ta.readOnly = true;
                        nd.appendChild(ta);
                        if (R.TEMPLATES) {
                            tplSel.onchange = () => {
                            ...
                            Object.entries(R.TEMPLATES).forEach(([k, t]) => {
                                if (normalize(t.prompt) === curNorm) matched = k
                            });
                        ...
                        clearFpBtn.textContent = "처리 기록 큐 초기화";
                        clearFpBtn.style.cssText = "width:100%;padding:8px;margin-top:12px;background:#654;color:#fff;border:none;border-radius:4px;cursor:pointer;font-size:12px;";
                        clearFpBtn.onclick = () => {
                            R.clearProcessed();
                            alert("기록 삭제됨")
                        };
    ...
    _w.__LoreInj.registerSubMenu("log", function(modal) {
        modal.createSubMenu("실행 로그", m => {
            const renderLogs = panel => {
                const chatKey = getChatKey();
                const escHtml = v => String(v == null ? "" : v).replace(/[&<>"']/g, ch => ({
                    "&": "&amp;",
                    "<": "&lt;",
                    ">": "&gt;",
                    '"': "&quot;",
                ...
                const fmtCostUsd = n => "$" + (Math.abs(Number(n)) < .01 ? Number(n).toFixed(5) : Number(n).toFixed(4));
                const idFromUrl = url => {
                    const s = String(url || "");
                ...
                const renderCostLine = i => {
                    ...
                    if (i.elapsedMs) parts.push((i.elapsedMs / 1e3).toFixed(2) + "s");
                    if (i.cost) {
                        const c = i.cost;
                ...
                const iLog = getInjLog(chatKey);
                const eLog = getExtLog(chatKey);
                const cLog = JSON.parse(_ls.getItem("lore-contradictions") || "[]");
                const rLog = settings.config.urlRefinerLogs?.[chatKey] || [];
                const makeLogBox = (title, color, items, renderer) => {
                    panel.addBoxedField("", "", {
                        onInit: nd => {
                            C.setFullWidth(nd);
                            const hRow = document.createElement("div");
                            hRow.style.cssText = "display:flex;justify-content:space-between;align-items:center;padding-bottom:8px;border-bottom:1px solid #333;margin-bottom:8px;cursor:pointer;";
                            ...
                            if (items.length > 0) {
                                ...
                                btn.onclick = ev => {
                                    ...
                                    if (confirm("삭제?")) {
                                        renderer(true);
                                        m.replaceContentPanel(renderLogs, "로그")
                                    }
                                };
                            ...
                            if (!items.length) {
                                listCon.appendChild(Object.assign(document.createElement("div"), {
                                    textContent: "기록 없음.",
                ...
                const renderCostBox = panel => {
                    panel.addBoxedField("", "", {
                        onInit: nd => {
                            ...
                            titleSpan.textContent = "API 비용";
                            titleSpan.style.cssText = "font-size:14px;color:#cc6;font-weight:bold;";
                            ...
                            const fmtUsd = n => "$" + (Math.abs(n) < .01 ? n.toFixed(5) : n.toFixed(4));
                            const fmtTok = n => n >= 1e3 ? (n / 1e3).toFixed(1) + "k" : String(n);
                            const refresh = () => {
                                Object.entries(periodBtns).forEach(([k, b]) => {
                                    if (k === period) {
                                        b.style.background = "#cc6";
                                        b.style.color = "#222";
                                        b.style.borderColor = "#cc6"
                                    } else {
                                ...
                                const renderTable = (title, bucket, keyLabel, labelMap) => {
                                    ...
                                    for (const [k, v] of rows) {
                                        const usdCell = v.unknown === v.calls ? '<span style="color:#888;">—</span>' : fmtUsd(v.usd) + (v.unknown ? ' <span style="color:#888;font-size:10px;">(+' + v.unknown + " 제외)</span>" : "") + (v.est ? ' <span style="color:#888;font-size:10px;" title="char/4 추정">~</span>' : "");
                                ...
                                if (!events.length) {
                                    ...
                                    Object.keys(byChat).forEach(k => {
                                        chatLabels[k] = displayChatLabel(k)
                                    });
                ...
                makeLogBox("추출 기록", "#da8", eLog, (clear, i, nd) => {
                    if (clear) clearExtLog(chatKey);
                    else {
                        const r = document.createElement("div");
                        r.style.cssText = "margin-bottom:6px;border-bottom:1px dashed #222;padding-bottom:4px;font-size:12px;";
                        r.innerHTML = `<span style="color:${i.status==="실패"?"#d66":i.status==="성공"?"#4a9":"#ccc"}">[${i.time}] ${i.isManual?"수동":"자동"} - ${i.status} (${i.count||0}개)</span>${i.api?`<br><span style="font-size:10px;color:#69b;">API: ${i.api.status}${i.api.error?" | "+i.api.error:""}</span>`:""}${renderCostLine(i)}`;
                        nd.appendChild(r)
                    }
                });
                makeLogBox("교정 기록", "#ea5", rLog, (clear, i, nd) => {
                    if (clear) {
                        ...
                        const r = document.createElement("div");
                        r.style.cssText = "margin-bottom:6px;border-bottom:1px dashed #222;padding-bottom:4px;font-size:12px;";
                        r.innerHTML = `<span style="color:${i.isPass?"#4a9":i.isError?"#d66":"#ea5"};">[${i.time}] ${i.isPass?"통과":i.isError?"에러":"교정됨"}</span>${i.reason?`<br><span style="font-size:11px;color:#da7;">${i.reason}</span>`:""}${renderCostLine(i)}`;
                        nd.appendChild(r)
                    }
                });
                makeLogBox("모순 기록", "#d96", cLog, (clear, i, nd) => {
                    if (clear) _ls.removeItem("lore-contradictions");
                    else {
                        const r = document.createElement("div");
                        r.style.cssText = "margin-bottom:6px;border-bottom:1px dashed #222;padding-bottom:4px;font-size:12px;";
                        r.innerHTML = `<span style="color:#d96;font-weight:bold;">${i.name}</span><br><span style="color:#a55;">"${i.oldStatus}" → "${i.newStatus}"</span><br><span style="font-size:10px;color:#888;">${new Date(i.time).toLocaleString()} (~${i.turn}턴)</span>`;
                        nd.appendChild(r)
                    }
                })
            };
                    ...
                    onInit: nd => {
                        ...
                        clearAllBtn.onclick = async () => {
                            ...
                            try {
                                ...
                                try {
                                    if (db.workingMemory) await db.workingMemory.delete(curUrl)
                                } catch (_) {}
                                settings.save();
                        ...
                        for (const e of allEntries) {
                            ...
                            if (cooldownRem > 0 || reinjScore > .1 || turnsSince > 0) {
                                ...
                                statusList.push({
                                    ...
                                    type: e.type,
                                    pack: e.packName,
                                    cooldownRem: cooldownRem,
                                    turnsSince: turnsSince,
                                    reinjScore: reinjScore,
                                    eventTurn: evTurn,
                                    gap: gap,
    ...
    function addSelect(nd, label, value, groups, onChange, opts = {}) {
        ...
        if (opts.customKey) {
            ...
            customInput.onchange = () => {
                settings.config[opts.customKey] = customInput.value;
                settings.save();
                if (opts.onCustomChange) opts.onCustomChange(customInput.value)
            };
    ...
    function renderPromptSettings(panel) {
        panel.addBoxedField("", "", {
            onInit: nd => {
                ...
                tplHeader.style.cssText = "display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;";
                const tplTitle = document.createElement("div");
                tplTitle.textContent = "템플릿";
                tplTitle.style.cssText = "font-size:12px;color:#ccc;font-weight:bold;";
                const newTplBtn = document.createElement("button");
                newTplBtn.textContent = "+ 새 템플릿";
                newTplBtn.style.cssText = "font-size:11px;padding:3px 8px;border-radius:3px;background:#258;border:none;color:#fff;cursor:pointer;";
                tplHeader.appendChild(tplTitle);
                tplHeader.appendChild(newTplBtn);
                nd.appendChild(tplHeader);
                const tplSelectWrap = document.createElement("div");
                tplSelectWrap.style.cssText = "display:flex;gap:8px;margin-bottom:12px;align-items:center;";
                ...
                const tplRenameBtn = document.createElement("button");
                tplRenameBtn.textContent = "이름 변경";
                tplRenameBtn.style.cssText = "font-size:11px;padding:4px 8px;border-radius:3px;background:transparent;border:1px solid #446;color:#88c;cursor:pointer;";
                const tplDelBtn = document.createElement("button");
                tplDelBtn.textContent = "삭제";
                tplDelBtn.style.cssText = "font-size:11px;padding:4px 8px;border-radius:3px;background:transparent;border:1px solid #d66;color:#d66;cursor:pointer;";
                const tplResetBtn = document.createElement("button");
                tplResetBtn.textContent = "초기화";
                tplResetBtn.style.cssText = "font-size:11px;padding:4px 8px;border-radius:3px;background:transparent;border:1px solid #285;color:#4a9;cursor:pointer;margin-left:auto;";
                tplSelectWrap.appendChild(tplSelect);
                tplSelectWrap.appendChild(tplRenameBtn);
                tplSelectWrap.appendChild(tplDelBtn);
                tplSelectWrap.appendChild(tplResetBtn);
                ...
                const mkLabel = txt => {
                    const l = document.createElement("div");
                    l.textContent = txt;
                    l.style.cssText = "font-size:12px;color:#ccc;margin-bottom:4px;";
                    nd.appendChild(l)
                };
                mkLabel("출력 형식(JSON)");
                const taSchema = document.createElement("textarea");
                taSchema.style.cssText = promptStyle;
                nd.appendChild(taSchema);
                mkLabel("새 로어 추출 지시문");
                const ta1 = document.createElement("textarea");
                ta1.style.cssText = promptStyle;
                nd.appendChild(ta1);
                mkLabel("기존 로어 참고 지시문");
                const ta2 = document.createElement("textarea");
                ta2.style.cssText = promptStyle;
                nd.appendChild(ta2);
                ...
                newTplBtn.onclick = () => {
                    ...
                    settings.config.templates.push({
                        ...
                        promptWithoutDb: active.promptWithoutDb,
                        promptWithDb: active.promptWithDb
                    });
                ...
                tplRenameBtn.onclick = () => {
                    ...
                    const newName = prompt("템플릿 이름:", activeTpl.name);
                    if (newName) {
                        const idx = settings.config.templates.findIndex(t => t.id === activeTpl.id);
                        if (idx !== -1) {
                ...
                tplDelBtn.onclick = () => {
                    ...
                    const activeTpl = settings.getActiveTemplate();
                    if (activeTpl.isDefault) return;
                    if (confirm("[" + activeTpl.name + "] 템플릿 삭제?")) {
        ...
        panel.addBoxedField("", "", {
            onInit: nd => {
                ...
                title.style.cssText = "font-size:14px;color:#4a9;font-weight:bold;margin-bottom:8px;";
                nd.appendChild(title);
                addPromptArea(nd, "후보 재정렬 지시문", settings.config.rerankPrompt || C.DEFAULTS.rerankPrompt, v => {
                    settings.config.rerankPrompt = v;
                    settings.save()
                }, {
                    height: 110,
                    reset: () => C.DEFAULTS.rerankPrompt
                });
                addPromptArea(nd, "응답 교정 지시문", settings.config.refinerCustomPrompt || "", v => {
                    settings.config.refinerCustomPrompt = v;
                    settings.config.refinerUseDynamic = false;
                    settings.save()
                }, {
                    height: 180
            ...
            m.replaceContentPanel(panel => {
                panel.addBoxedField("", "", {
                    onInit: nd => {
                        C.setFullWidth(nd);
                        const t = document.createElement("div");
                        t.textContent = "API 연결";
                        t.style.cssText = "font-size:13px;color:#ccc;font-weight:bold;margin-bottom:8px;";
                        nd.appendChild(t);
                        ...
                        testRow.style.cssText = "margin:12px 0 16px;display:flex;gap:8px;align-items:center;";
                        ...
                        testBtn.onclick = async () => {
                            ...
                            const missing = apiType === "vertex" ? !settings.config.autoExtVertexJson : apiType === "firebase" ? !settings.config.autoExtFirebaseScript : !settings.config.autoExtKey;
                            if (missing) {
                                alert(apiType === "vertex" ? "Vertex JSON 필요." : apiType === "firebase" ? "Firebase 설정 필요." : "API 키 필요.");
                                return
                            }
                            ...
                            try {
                                ...
                                testResult.textContent = r.text ? "✅ 성공: " + r.text.trim().slice(0, 50) : "❌ 실패: " + r.error;
                                testResult.style.color = r.text ? "#4a9" : "#d66"
                            } catch (e) {
                                testResult.textContent = "❌ 오류: " + e.message;
                                testResult.style.color = "#d66"
                            }
                            testBtn.disabled = false
                        };
                        testRow.appendChild(testBtn);
                        testRow.appendChild(testResult);
                        nd.appendChild(testRow);
                        const modelHead = document.createElement("div");
                        modelHead.textContent = "모델 선택";
                        modelHead.style.cssText = "font-size:13px;color:#ccc;font-weight:bold;margin:14px 0 8px;padding-top:10px;border-top:1px solid #333;";
                        nd.appendChild(modelHead);
                        const modelNote = document.createElement("div");
                        modelNote.textContent = "API를 쓰는 기능별 모델을 여기서 한 번에 관리함. 프롬프트 내용은 프롬프트 관리 메뉴에서 수정함.";
                        modelNote.style.cssText = "font-size:11px;color:#888;margin-bottom:8px;line-height:1.4;";
                        nd.appendChild(modelNote);
                        addSelect(nd, "추출/정리용 모델", settings.config.autoExtModel || "gemini-3-flash-preview", [
                            ["Gemini 3.x", [
                            ...
                            settings.config.autoExtModel = v;
                            settings.save()
                        }, {
                            customKey: "autoExtCustomModel"
                        });
                        addSelect(nd, "후보 재정렬 모델", settings.config.rerankModel || "gemini-3-flash-preview", [
                            ["Gemini", [
                                ["3.1 Flash Lite (추천)", "gemini-3.1-flash-lite-preview"],
                                ["3.5 Flash", "gemini-3.5-flash"],
                                ["3.0 Flash", "gemini-3-flash-preview"],
                                ["2.5 Flash Lite", "gemini-2.5-flash-lite"]
                            ]]
                        ], v => {
                            settings.config.rerankModel = v;
                            settings.save()
                        });
                        const judgeCtl = addSelect(nd, "과거 장면 판단 모델", settings.config.temporalRecallJudgeModel || "gemini-3.1-flash-lite-preview", [
                            ["Gemini 3.x (추천)", [
                                ["3.1 Flash Lite (기본)", "gemini-3.1-flash-lite-preview"],
                                ["3.5 Flash", "gemini-3.5-flash"],
                                ["3.0 Flash", "gemini-3-flash-preview"]
                            ]],
                            ["Gemini 2.x", [
                            ...
                            settings.config.temporalRecallJudgeModel = v;
                            settings.save()
                        }, {
                            customKey: "temporalRecallJudgeCustomModel"
                        });
                        addSelect(nd, "응답 교정 모델", settings.config.refinerModel !== undefined ? settings.config.refinerModel : "", [
                            ["기본 LLM과 동일", [
                                ["기본 LLM 사용", ""]
                            ]],
                            ["Gemini 3.x", [
                            ...
                            settings.config.refinerModel = v;
                            settings.save()
                        }, {
                            customKey: "refinerCustomModel"
                        });
                        const rl = document.createElement("div");
                        rl.textContent = "생각 깊이";
                        rl.style.cssText = "font-size:11px;color:#999;margin:10px 0 4px;";
                        nd.appendChild(rl);
                        const rs = document.createElement("select");
                        rs.style.cssText = FIELD_STYLE;
                        [
                            ["Off", "off"],
                            ["Minimal (256)", "minimal"],
                            ["Low (1024)", "low"],
                        ...
                        rs.value = settings.config.autoExtReasoning || "medium";
                        nd.appendChild(rs);
                        const bl = document.createElement("div");
                        bl.textContent = "생각 예산";
                        bl.style.cssText = "font-size:11px;color:#666;margin-bottom:4px;margin-top:8px;" + (rs.value === "budget" ? "" : "display:none;");
                        const bi = document.createElement("input");
                        bi.type = "number";
                        bi.value = settings.config.autoExtBudget || 2048;
                        bi.style.cssText = FIELD_STYLE + (rs.value === "budget" ? "" : "display:none;");
                        bi.onchange = () => {
                            settings.config.autoExtBudget = parseInt(bi.value) || 2048;
                            settings.save()
                        };
                        rs.onchange = () => {
                            settings.config.autoExtReasoning = rs.value;
                            settings.save();
                            const isB = rs.value === "budget";
                            bl.style.display = isB ? "" : "none";
                            bi.style.display = isB ? "" : "none"
                        };
                        nd.appendChild(bl);
                        nd.appendChild(bi);
                        const jrl = document.createElement("div");
                        jrl.textContent = "과거 장면 판단 생각 깊이";
                        jrl.style.cssText = "font-size:11px;color:#999;margin:10px 0 4px;";
                        nd.appendChild(jrl);
                        const jrs = document.createElement("select");
                        jrs.style.cssText = FIELD_STYLE;
                        [
                            ["Minimal (권장)", "minimal"],
                            ["Low", "low"],
                            ["Medium", "medium"],
                        ...
                        proWarn.style.cssText = "font-size:10px;color:#d96;margin-top:4px;display:none;";
                        proWarn.textContent = "주의: Pro 모델은 minimal 미지원. low 이상 권장.";
                        nd.appendChild(proWarn);
                        const refreshProWarn = () => {
                            const m = judgeCtl.sel.value === "_custom" ? judgeCtl.customInput?.value || "" : judgeCtl.sel.value;
                            proWarn.style.display = String(m).includes("pro") && jrs.value === "minimal" ? "" : "none"
                        };
                        ...
                        jrs.onchange = () => {
                            settings.config.temporalRecallJudgeReasoning = jrs.value;
                            settings.save();
                            refreshProWarn()
                        };
                        refreshProWarn()
...
(async function() {
    ...
    async function showBootErrorBadge(gate) {
        ...
        btn.onclick = () => {
            ...
            const payload = {
                gate: gate,
                moduleStatus: L.moduleStatus || {},
                missingSubs: L.missingSubs || [],
                menuOrder: L.__menuOrder || null,
                route: L.route || null
            };
            alert(JSON.stringify(payload, null, 2).slice(0, 3e3))
        };
        document.body.appendChild(btn)
    }
    async function waitForModalManager(timeoutMs = 1e4) {
        const until = Date.now() + timeoutMs;
        while (Date.now() < until) {
            const MM = typeof ModalManager !== "undefined" ? ModalManager : _w.ModalManager || typeof window !== "undefined" && window.ModalManager || null;
            if (MM) return MM;
    ...
    if (_w.__LoreInjReady && typeof _w.__LoreInjReady.then === "function") {
        ...
        const L = _w.__LoreInj;
        if (!L || !_subs.every(k => L[k])) {
            console.error("[LoreInj:6-ui] 폴백 대기 타임아웃, UI 마운트 중단");
            await showBootErrorBadge({
                ok: false,
    ...
    if (!modal || typeof modal.createMenu !== "function") {
        ...
        await showBootErrorBadge({
            ok: false,
            reason: "modal.createMenu missing"
        });
        return
    }
    ...
    if (!document.getElementById("erie-lore-style")) {
        const loreStyle = document.createElement("style");
        loreStyle.id = "erie-lore-style";
        loreStyle.textContent = `     .erie-lore {       opacity: 0 !important;       height: 0 !important;       min-height: 0 !important;       max-height: 0 !important;       overflow: hidden !important;       margin: 0 !important;       padding: 0 !important;       border: none !important;       pointer-events: none !important;       position: absolute !important;     }`;
        document.head.appendChild(loreStyle);
    } /* 1. 로어 텍스트 투명화 스캐너 (3.0초 주기) */
    setInterval(() => {
        try {
            const markdowns = document.querySelectorAll('.wrtn-markdown:not([data-lore-processed="true"])');
            for (const md of markdowns) {
                if (md.closest('.ProseMirror') || md.closest('.__chat_input_textarea')) continue;
                const text = md.textContent || ""; /* OOC 컨텍스트 및 사용자가 명시한 System 태그 감지 */
                if (text.includes('[System:') || text.includes('System: Established world') || text.includes('<ooc_lore_context>') || text.includes('/**') || text.includes('(Narrator\'s note:')) {
                    const children = md.childNodes;
                    let isInside = false;
                    for (const child of children) {
                        const childText = child.textContent || "";
                        if (childText.includes('[System:') || childText.includes('System: Established world') || childText.includes('<ooc_lore_context>') || childText.includes('/**') || childText.includes('(Narrator\'s note:')) {
                            isInside = true;
                        }
                        if (isInside) {
                            if (child.nodeType === Node.ELEMENT_NODE) {
                                child.classList.add('erie-lore');
                            } else if (child.nodeType === Node.TEXT_NODE) {
                                const span = document.createElement('span');
                                span.classList.add('erie-lore');
                                span.textContent = child.textContent;
                                md.replaceChild(span, child);
                            }
                        } /* 닫는 태그 감지 시 해제 */
                        if (childText.includes('[/System]') || childText.includes('</ooc_lore_context>') || childText.includes('**/') || childText.includes('(End note)')) {
                            isInside = false;
                        }
                    }
                    md.setAttribute('data-lore-processed', 'true');
                }
            }
        } catch (e) {}
    }, 3000); /* 2. 7 in 1 통합팩 하단 툴바 가로채기 및 버튼 삽입 (0.5초 주기) */
    setInterval(() => {
        try {
            const tools = document.getElementById("custom-rp-tools");
            if (tools && !document.getElementById("lore-bottom-visual-btn")) {
                /* 너비 28px인 스페이서 빈 공간 찾기 */
                const spacers = Array.from(tools.children).filter(el => el.style && el.style.width === '28px'); /* 요구사항: 인덱스 2번 영역 즉 3번째 빈 공간 */
                if (spacers.length >= 3) {
                    const targetSpacer = spacers[2];
                    const btn = document.createElement("button");
                    btn.id = "lore-bottom-visual-btn";
                    btn.className = "crack-ext-lore-btn relative inline-flex items-center justify-center rounded-full text-sm font-medium transition-colors border border-border bg-card text-line-gray-1 hover:bg-secondary p-0 size-7";
                    btn.innerHTML = '<span style="font-size:15px; font-weight:normal; color:#BCD0D7;">♥</span>';
                    btn.title = "로어 인젝터";
                    btn.onclick = (e) => {
                        e.preventDefault();
                        try {
                            const mm = typeof ModalManager !== "undefined" ? ModalManager : (_w.ModalManager || window.ModalManager);
                            if (mm) mm.getOrCreateManager("c2").display(document.body.getAttribute("data-theme") !== "light");
                        } catch (err) {}
                    };
                    targetSpacer.replaceWith(btn);
                }
            }
        } catch (e) {}
    }, 500);
...
(function() {
    ...
    function settle(payload) {
        ...
        settled = true;
        try {
            _w.__LoreInjReady.__resolve(payload)
        } catch (_) {}
        try {
            _w.dispatchEvent(new CustomEvent("LoreInj:ready", {
                detail: payload
            }))
        } catch (_) {}
    }
    ...
    function check() {
        ...
        if (missingCore.length === 0 && missingSubs.length === 0) {
            ...
            settle({
                ok: true,
                ver: state.VER || L.chatBootstrapVersion,
                universal: true
            });
            return
        }
